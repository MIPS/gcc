...
;; Operand2 is the 2nd word of the function descriptor that has the TOC
;; Operand3 is either the static chain or 0
...
(define_insn_and_split "call_indirect_aix<ptrsize>"
  [(call (mem:SI (match_operand:P 0 "register_operand" "c,*l"))
   ...
   (use (match_operand:P 3 "optional_static_chain_operand" "rO,rO"))
   (clobber (match_operand:P 4 "memory_operand" "=m,m"))
   (clobber (reg:SI LR_REGNO))]
  "DEFAULT_ABI == ABI_AIX"
  "#"
  "&& reload_completed"
  [(set (match_dup 4) (reg:P 2))
   (set (reg:P 2) (match_dup 2))
   (parallel [(call (mem:SI (match_dup 0))
		    (match_dup 1))
	      (use (reg:P 2))
	      (use (match_dup 3))
	      (set (reg:P 2) (match_dup 4))
...
(define_insn "*call_indirect_aix<ptrsize>_internal"
  [(call (mem:SI (match_operand:P 0 "register_operand" "c,*l"))
	 (match_operand 1 "" "g,g"))
   (use (reg:P 2))
   (use (match_operand:P 2 "optional_static_chain_operand" "rO,rO"))
   (set (match_operand:P 3 "gpc_reg_operand" "+r,r")
	(match_operand:P 4 "memory_operand" "m,m"))
   (clobber (reg:P LR_REGNO))]
  "DEFAULT_ABI == ABI_AIX && reload_completed"
  "b%T0l\;<ptrload>%U4%X4 %3,%4"
  [(set_attr "type" "jmpreg")
   (set_attr "length" "8")])

;; Operand1 is the addresss of the function to call
;; Operand2 is the flag for System V.4 for unprototyped or FP registers
;; Operand3 is the 2nd word of the function descriptor that has the TOC
;; Operand4 is either the static chain or 0
;; Operand5 is the stack location to restore the TOC pointer from
...
(define_insn_and_split "call_value_indirect_aix<ptrsize>"
  [(set (match_operand 0 "" "")
   ...
   (use (match_operand:P 4 "optional_static_chain_operand" "rO,rO"))
   (clobber (match_operand:P 5 "memory_operand" "=m,m"))
   (clobber (reg:P LR_REGNO))]
  "DEFAULT_ABI == ABI_AIX"
  "#"
  "&& reload_completed"
  [(set (match_dup 5) (reg:P 2))
   (set (reg:P 2) (match_dup 3))
   (parallel [(set (match_dup 0)
		   (call (mem:SI (match_dup 1))
			 (match_dup 2)))
	      (use (reg:P 2))
	      (use (match_dup 4))
	      (set (reg:P 2) (match_dup 5))
	      (clobber (reg:P LR_REGNO))])]
  ""
  [(set_attr "type" "jmpreg")
   (set_attr "length" "16")])
...
(define_insn "*call_value_indirect_aix<ptrsize>_internal"
  [(set (match_operand 0 "" "")
   ...
   (use (reg:P 2))
   (use (match_operand:P 3 "optional_static_chain_operand" "rO,rO"))
   (set (match_operand:P 4 "gpc_reg_operand" "+r,r")
	(match_operand:P 5 "memory_operand" "m,m"))
   (clobber (reg:P LR_REGNO))]
  "DEFAULT_ABI == ABI_AIX && reload_completed"
  "b%T1l\;<ptrload>%U5%X5 %4,%5"
  [(set_attr "type" "jmpreg")
   (set_attr "length" "8")])
