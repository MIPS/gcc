...
; load/store instruction for extra fusion
(define_mode_attr fuse_ldf	[(SF "lfs")  (DF "lfd")])
(define_mode_attr fuse_stf	[(SF "stfs") (DF "stfd")])
...
;; extra fusion is selected, we also fuse floating point loads/stores.

;; Fused TOC support:
;; Replace simple GPR loads with a fused form.  This is done before register
;; allocation, so that we can avoid allocating a temporary base register that
;; won't be used, and that we try to load into base registers, and not register
;; 0.  Allow DImode to be loaded to FPR registers.

(define_split
  [(set (match_operand:INT1 0 "offsettable_reg_operand" "")
	(match_operand:INT1 1 "fusion_toc_mem_raw" ""))]
  "TARGET_FUSION_TOC_INT && can_create_pseudo_p ()"
...
(define_insn "*fusion_tocload_<mode>"
  [(set (match_operand:QHSI 0 "int_reg_operand" "=&b,??r")
	(match_operand:QHSI 1 "fusion_toc_mem_wrapped" "wG,wG"))
   (unspec [(const_int 0)] UNSPEC_FUSION_ADDIS)
   (use (match_operand:DI 2 "base_reg_operand" "r,r"))
   (clobber (match_scratch:DI 3 "=X,&b"))]
  "TARGET_FUSION_TOC_INT"
{
  if (base_reg_operand (operands[0], <MODE>mode))
    return emit_fusion_gpr_load (operands[0], operands[1]);

  return emit_fusion_extra_load (operands[0], operands[1], operands[3]);
}
  [(set_attr "type" "load")
   (set_attr "length" "8")])

(define_insn "*fusion_tocload_di"
  [(set (match_operand:DI 0 "int_reg_operand" "=&b,??r,?d")
	(match_operand:DI 1 "fusion_toc_mem_wrapped" "wG,wG,wG"))
   (unspec [(const_int 0)] UNSPEC_FUSION_ADDIS)
...

;; Advanced fusion testing
;; Merge addis with floating load/store to FPRs.
(define_peephole2
  [(set (match_operand:P 0 "base_reg_operand" "")
	(match_operand:P 1 "fusion_gpr_addis" ""))
   (set (match_operand:SFDF 2 "fpr_reg_operand" "")
  ...
  [(const_int 0)]
{
  expand_fusion_extra_load (operands);
  DONE;
})
...
(define_peephole2
  ...
  [(set (match_dup 0)
	(unspec:SDI [(match_dup 1)
		     (match_dup 2)] UNSPEC_FUSION_EXTRA))])
...
(define_peephole2
  ...
  [(set (match_dup 2)
	(unspec:SDI [(match_dup 1)
		     (match_dup 3)] UNSPEC_FUSION_EXTRA))])
  ...
  "TARGET_FUSION_EXTRA"
{
  ...
  return emit_fusion_extra_load (operands[0], operands[1], operands[2]);
}
  [(set_attr "type" "load")
   (set_attr "length" "8")])
...
(define_insn "fusion_gpr_<P:mode>_<GPR_FUSION:mode>_store"
  ...
  "TARGET_FUSION_EXTRA"
{
  return emit_fusion_extra_store (operands[0], operands[1], operands[2]);
}
  [(set_attr "type" "store")
   (set_attr "length" "8")])

(define_insn "fusion_fpr_<P:mode>_<FPR_FUSION:mode>_load"
  [(set (match_operand:FPR_FUSION 0 "fpr_reg_operand" "=<FPR_FUSION:Ff>")
	(unspec:FPR_FUSION
...
(define_insn "fusion_fpr_<P:mode>_<FPR_FUSION:mode>_store"
  [(set (match_operand:FPR_FUSION 0 "fusion_addis_mem_combo_store" "=wF")
	(unspec:FPR_FUSION
	 [(match_operand:FPR_FUSION 1 "fpr_reg_operand" "<FPR_FUSION:Ff>")]
	 UNSPEC_FUSION_EXTRA))
   (clobber (match_operand:P 2 "base_reg_operand" "=&b"))]
  "TARGET_FUSION_EXTRA"
{
...
(define_insn "*fusion_extra_<mode>_constant"
