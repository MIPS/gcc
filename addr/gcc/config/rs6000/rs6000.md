...
(define_insn "mov<mode>_hardfloat"
  ...
  "TARGET_HARD_FLOAT && TARGET_FPRS && TARGET_SINGLE_FLOAT
   && valid_move_insn_p (operands[0], operands[1])"
  "@
   mr %0,%1
   lwz%U1%X1 %0,%1
   stw%U0%X0 %1,%0
...
;; Return 1 if the operand must be loaded from memory easy.  This is used by a
...
;; have constants in order not interfere with reciprocal estimation.
...
(define_peephole2
  ...
  "!(TARGET_E500_DOUBLE && GET_CODE (operands[2]) == SUBREG)
   ...
   && valid_move_insn_p (operands[2], operands[1])"
  [(set (match_dup 2) (match_dup 1))])
...
;; For SF/DFmode that can go in both traditional floating point registers and
;; Altivec registers, create a TOC entry that does not put the LO_SUM into the
;; address component, instead it uses the single register as the address.
(define_insn_and_split "toc_nosplit<mode>"
  [(set (match_operand:P 0 "base_reg_operand" "=b")
