	  ...
	  switch (type)
	    ...
	    case CPP_NAME:
	      *value = id = HT_IDENT_TO_GCC_IDENT (HT_NODE (tok->val.node.node));
	      ...
	      /* If the identifier is the name of a lazy builtin function,
		 create the builtin now.  */
	      if (IDENTIFIER_LAZY_BUILTIN_P (id))
		built_in_lazy_create (id, true);
