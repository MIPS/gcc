...
c_finish_omp_barrier (location_t loc)
{
  ...
  x = builtin_decl (BUILT_IN_GOMP_BARRIER, BU_EXPLICIT);
...
c_finish_omp_taskwait (location_t loc)
{
  ...
  x = builtin_decl (BUILT_IN_GOMP_TASKWAIT, BU_EXPLICIT);
  x = build_call_expr_loc (loc, x, 0);
  add_stmt (x);
}
...
c_finish_omp_taskyield (location_t loc)
{
  ...
  x = builtin_decl (BUILT_IN_GOMP_TASKYIELD, BU_EXPLICIT);
  x = build_call_expr_loc (loc, x, 0);
  add_stmt (x);
}
...
c_finish_omp_flush (location_t loc)
{
  ...
  x = builtin_decl (BUILT_IN_SYNC_SYNCHRONIZE, BU_EXPLICIT);
  x = build_call_expr_loc (loc, x, 0);
  add_stmt (x);
}
