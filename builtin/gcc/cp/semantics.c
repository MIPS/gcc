...
{
  tree fn = builtin_decl (BUILT_IN_GOMP_BARRIER, BU_EXPLICIT);
...
{
  tree fn = builtin_decl (BUILT_IN_SYNC_SYNCHRONIZE, BU_EXPLICIT);
  VEC(tree,gc) *vec = make_tree_vector ();
  tree stmt = finish_call_expr (fn, &vec, false, false, tf_warning_or_error);
  release_tree_vector (vec);
  finish_expr_stmt (stmt);
...
{
  tree fn = builtin_decl (BUILT_IN_GOMP_TASKWAIT, BU_EXPLICIT);
  VEC(tree,gc) *vec = make_tree_vector ();
  tree stmt = finish_call_expr (fn, &vec, false, false, tf_warning_or_error);
  release_tree_vector (vec);
  finish_expr_stmt (stmt);
...
{
  tree fn = builtin_decl (BUILT_IN_GOMP_TASKYIELD, BU_EXPLICIT);
  VEC(tree,gc) *vec = make_tree_vector ();
  tree stmt = finish_call_expr (fn, &vec, false, false, tf_warning_or_error);
  release_tree_vector (vec);
  finish_expr_stmt (stmt);
