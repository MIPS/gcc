  ...
  switch (expr->value.op.op2->ts.type)
	...
	case BT_REAL:
	  ...
	  if (ikind == 0)
		...
		{
		case 0:
		  fndecl = builtin_decl (BUILT_IN_POWIF, BU_EXPLICIT);
		  break;
		
		case 1:
		  fndecl = builtin_decl (BUILT_IN_POWI, BU_EXPLICIT);
		  break;

		case 2:
		  fndecl = builtin_decl (BUILT_IN_POWIL, BU_EXPLICIT);
		  break;
...
fill_with_spaces (tree start, tree type, tree size)
{
  ...
  if (compare_tree_int (TYPE_SIZE_UNIT (type), 1) == 0)
    return build_call_expr_loc (input_location,
				builtin_decl (BUILT_IN_MEMSET, BU_EXPLICIT),
				3, start,
		       ...
		       int dkind, tree slength, tree src, int skind)
{
  ...
  tmp2 = build_call_expr_loc (input_location,
			      builtin_decl (BUILT_IN_MEMMOVE, BU_EXPLICIT),
			      3, dest, src, dlen);
...

{
  ...
  tmp = build_call_expr_loc (input_location,
			     builtin_decl (BUILT_IN_MALLOC, BU_EXPLICIT),
			     1, size_in_bytes);
  ...
  if (expr1->ts.type == BT_CHARACTER && expr1->ts.deferred)
    {
      ...
      gfc_add_expr_to_block (block, tmp);
      tmp = build_call_expr_loc (input_location,
				 builtin_decl (BUILT_IN_REALLOC, BU_EXPLICIT),
