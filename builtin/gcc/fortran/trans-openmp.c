...
gfc_omp_clause_copy_ctor (tree clause, tree dest, tree src)
{
  ...
  call = build_call_expr_loc (input_location,
			  builtin_decl (BUILT_IN_MEMCPY, BU_EXPLICIT),
			  3, ptr,
			  fold_convert (pvoid_type_node,
...
gfc_omp_clause_assign_op (tree clause ATTRIBUTE_UNUSED, tree dest, tree src)
{
  ...
  call = build_call_expr_loc (input_location,
			  builtin_decl (BUILT_IN_MEMCPY, BU_EXPLICIT), 3,
			  fold_convert (pvoid_type_node,
...
{
  tree decl = builtin_decl ([BUILT_IN_GOMP_BARRIER, BU_EXPLICIT);
...
{
  tree decl = builtin_decl ([BUILT_IN_SYNC_SYNCHRONIZE, BU_EXPLICIT);
  return build_call_expr_loc (input_location, decl, 0);
}
...
{
  tree decl = builtin_decl ([BUILT_IN_GOMP_TASKWAIT, BU_EXPLICIT);
  return build_call_expr_loc (input_location, decl, 0);
}
...
{
  tree decl = builtin_decl ([BUILT_IN_GOMP_TASKYIELD, BU_EXPLICIT);
  return build_call_expr_loc (input_location, decl, 0);
}
