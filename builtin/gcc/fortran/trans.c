  ...
  gfc_add_modify (&block2, res,
		  fold_convert (prvoid_type_node,
				build_call_expr_loc (input_location,
				   builtin_decl [BUILT_IN_MALLOC], 1, size)));
			   ...
			   tree size, tree status)
{
  ...
  gfc_add_modify (block, pointer,
	  fold_convert (TREE_TYPE (pointer),
		build_call_expr_loc (input_location,
			     builtin_decl (BUILT_IN_MALLOC, BU_EXPLICIT), 1,
...
gfc_call_free (tree var)
{
  ...
  call = build_call_expr_loc (input_location,
			      builtin_decl (BUILT_IN_FREE, BU_EXPLICIT),
			      1, var);
			    ...
			    gfc_expr* expr)
{
  ...
  tmp = build_call_expr_loc (input_location,
			     builtin_decl (BUILT_IN_FREE, BU_EXPLICIT), 1,
