...
build_stack_save_restore (gimple *save, gimple *restore)
{
  ...
  *save = gimple_build_call (builtin_decl (BUILT_IN_STACK_SAVE, BU_IMPLICIT),
			     0);
  tmp_var = create_tmp_var (ptr_type_node, "saved_stack");
  gimple_call_set_lhs (*save, tmp_var);

  *restore
    = gimple_build_call (builtin_decl (BUILT_IN_STACK_RESTORE, BU_IMPLICIT),
			 1, tmp_var);
}
    				...
    				gimple_seq *seq_p)
{
  ...
  t = builtin_decl (BUILT_IN_MEMCPY, BU_IMPLICIT);
    				...
    				gimple_seq *seq_p)
{
  ...
  to_ptr = build_fold_addr_expr_loc (loc, to);
  gimplify_arg (&to_ptr, seq_p, loc);
  t = builtin_decl (BUILT_IN_MEMSET, BU_IMPLICIT);
...
gimplify_variable_sized_compare (tree *expr_p)
{
  ...
  t = builtin_decl (BUILT_IN_MEMCMP, BU_IMPLICIT);
...
gimplify_function_tree (tree fndecl)
{
  ...
  if (flag_instrument_function_entry_exit
      ...
      && !flag_instrument_functions_exclude_p (fndecl))
    {
      ...
      x = builtin_decl (BUILT_IN_RETURN_ADDRESS, BU_IMPLICIT);
      call = gimple_build_call (x, 1, integer_zero_node);
      tmp_var = create_tmp_var (ptr_type_node, "return_addr");
      gimple_call_set_lhs (call, tmp_var);
      gimplify_seq_add_stmt (&cleanup, call);
      x = builtin_decl (BUILT_IN_PROFILE_FUNC_EXIT, BU_IMPLICIT);
      ...
      x = builtin_decl (BUILT_IN_PROFILE_FUNC_ENTER, BU_IMPLICIT);
      call = gimple_build_call (x, 2,
				build_fold_addr_expr (current_function_decl),
				tmp_var);
      gimplify_seq_add_stmt (&body, call);
