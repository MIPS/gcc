	       ...
	       tree fntype, bool const_p)
{
  ...
  set_builtin_decl (bcode, decl, decl);
...
Gogo::make_trampoline(tree fnaddr, tree closure, source_location location)
{
  ...
  tree ini = build_call_expr(builtin_decl (BUILT_IN_INIT_TRAMPOLINE,
					   BU_IMPLICIT),
			     3, x, fnaddr, closure);
  ...
  x = build_call_expr(builtin_decl (BUILT_IN_ADJUST_TRAMPOLINE, BU_IMPLICIT),
		      1, x);
