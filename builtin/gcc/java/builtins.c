			   ...
			   tree orig_call)
{
  ...
  if (direct_optab_handler (sync_compare_and_swap_optab, mode)
      ...
      || flag_use_atomic_builtins)
    {
      ...
      stmt = build_call_expr (builtin_decl (fncode, BU_EXPLICIT),
			      ...
			      tree orig_call)
{
      ...
      || flag_use_atomic_builtins)
  {
    ...
    addr = build_addr_sum (value_type, obj_arg, offset_arg);
    stmt = build_call_expr (builtin_decl (builtin, BU_EXPLICIT),
			    3, addr, expected_arg, value_arg);
		     ...
		     tree orig_call)
{
  ...
  stmt = build_call_expr (builtin_decl (BUILT_IN_SYNC_SYNCHRONIZE,
					BU_EXPLICIT), 0);
		...
		int flags)
{
  ...
  set_builtin_decl (val, decl, decl);
}
	  ...
	  if (method_class == java_builtins[i].class_name.t
	      ...
	      if (java_builtins[i].creator != NULL)
		{
	      ...
	      if (flag_indirect_dispatch)
	        return call;
	      fn = builtin_decl (java_builtins[i].builtin_code, BU_EXPLICIT);
	      if (fn == NULL_TREE)
		return call;
