  ...
  if (src_r == NULL)
    {
	 ...
	 Resolve this by expanding the resx node to an abort.  */
      ...
      fn = builtin_decl (BUILT_IN_TRAP, BU_IMPLICIT);
      ...
      else
	{
	  ...
	  fn = builtin_decl (BUILT_IN_EH_COPY_VALUES, BU_IMPLICIT);
	...
	{
	  fn = builtin_decl (BUILT_IN_CXA_END_CLEANUP, BU_IMPLICIT);
	  x = gimple_build_call (fn, 0);
	  gsi_insert_before (&gsi, x, GSI_SAME_STMT);
	}
      else
	{
	  fn = builtin_decl (BUILT_IN_EH_POINTER, BU_IMPLICIT);
	  src_nr = build_int_cst (integer_type_node, src_r->index);
	  ...
	  fn = builtin_decl (BUILT_IN_UNWIND_RESUME, BU_IMPLICIT);
...
lower_eh_dispatch (basic_block src, gimple stmt)
{
  ...
  switch (r->type)
	...
	if (labels == NULL)
	  ...
	  {
	    fn = builtin_decl (BUILT_IN_EH_FILTER, BU_IMPLICIT);
