			   ...
			   struct walk_stmt_info *wi)
{
  ...
  call = gimple_build_call (builtin_decl (BUILT_IN_NONLOCAL_GOTO,
					  BU_IMPLICIT),
  ...
  switch (TREE_CODE (t))
	...
	continue;
	 ...
	 computing extra alignment, but it can really be anything.  */
      builtin = builtin_decl (BUILT_IN_ADJUST_TRAMPOLINE, BU_IMPLICIT);
...
finalize_nesting_tree_1 (struct nesting_info *root)
{
  ...
  if (root->any_tramp_created)
      ...
      for (i = root->inner; i ; i = i->next)
	{
	  ...
	  x = builtin_decl (BUILT_IN_INIT_TRAMPOLINE, BU_IMPLICIT);
