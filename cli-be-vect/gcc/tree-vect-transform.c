  ...
  if (nested_in_vect_loop)
      ...
      if (stmts)
	{
      ...
      if (targetm.vectorize.builtin_build_affine_vec && 
           (builtin_decl 
              = targetm.vectorize.builtin_build_affine_vec (vectype))) 
        {
...
get_initial_def_for_reduction (gimple stmt, tree init_val, tree *adjustment_def)
{
  ...
  int i;
  bool nested_in_vect_loop = false; 
  tree builtin_decl1, builtin_decl2;
  tree init_value;
   ...
   if (adjustment_def)
    ...
    if (targetm.vectorize.builtin_build_uniform_vec
                ...
                = targetm.vectorize.builtin_build_init_vec (vector_type)))
      {
        ...
        if (adjustment_def)
          new_stmt = gimple_build_call (builtin_decl1, 1, def_for_init);
        else
          new_stmt = gimple_build_call (builtin_decl2, 2, init_value, 
                                        def_for_init);
  ...
  case MAX_EXPR:
    ...
    if (targetm.vectorize.builtin_build_uniform_vec
           ...
           = targetm.vectorize.builtin_build_uniform_vec (vector_type)))
      {
        ...
        new_stmt = gimple_build_call (builtin_decl1, 1, init_val);
        add_referenced_var (var);
        init_def = make_ssa_name (var, new_stmt);
        gimple_call_set_lhs (new_stmt, init_def);
        new_bb = gsi_insert_on_edge_immediate (loop_preheader_edge (loop),
			...
			gimple *vec_stmt)
{
  ...
  gcc_assert (dt == vect_reduction_def
              || dt == vect_nested_cycle
              || ((dt == vect_loop_def || dt == vect_invariant_def 
  ...
  */
  ...
  for (j = 0; j < ncopies; j++)
      ...
      if (j == 0 || !single_defuse_cycle)
	{
      ...
      if (j == 0)
        {
	  loop_vec_def0 = vect_get_vec_def_for_operand (ops[!reduc_index], stmt, NULL);
          if (op_type == ternary_op)
            {
              if (reduc_index == 0)
      ...
      if (op_type == binary_op)
      ...
      else
        {
          ...
          if (targetm.vectorize.builtin_pattern
            ...
            {
              new_stmt = gimple_build_call (builtin_decl, 3, arg0, arg1, arg2);
              new_temp = make_ssa_name (vec_dest, new_stmt);
              gimple_call_set_lhs (new_stmt, new_temp);
            }
          else
            {
              expr = build3 (code, vectype, arg0, arg1, arg2);
              new_temp = make_ssa_name (vec_dest, new_stmt);
              gimple_assign_set_lhs (new_stmt, new_temp);
            }
         }
