...
{
    return LIBSTD_LNAME(fputc)(c, p->where_.f);
}

LIBSTD_SPROTO_IMPL(int, __proxy_os, __out_proxy *p, char *s)
{
    return LIBSTD_LNAME(fputs)(s, p->where_.f);
}

LIBSTD_LPROTO_IMPL(int, vfprintf, FILE *restrict stream, const char *restrict format, va_list arg)
{
