...
/* Return true if X is a legitimate constant address.  STRICT_P says
   whether we need strict checking.  */

static bool
m68k_legitimate_constant_address_p (rtx x, bool strict_p)
{
...
static bool
m68k_decompose_address (enum machine_mode mode, rtx x, bool strict_p,
			struct m68k_address *address)
{
  ...
  if (GET_CODE (x) == PLUS
      && GET_CODE (XEXP (x, 1)) == CONST_INT
      && IN_RANGE (INTVAL (XEXP (x, 1)), -0x8000, 0x7fff)
      && m68k_legitimate_base_reg_p (XEXP (x, 0), strict_p))
    {
      address->base = XEXP (x, 0);
  ...
  if (m68k_legitimate_constant_address_p (x, strict_p))
    {
      address->offset = x;
      return true;
    }
  ...
  if (TARGET_68020)
    {
      ...
      if (GET_CODE (x) == PLUS
	  && m68k_legitimate_constant_address_p (XEXP (x, 1), strict_p))
	{
	  address->offset = XEXP (x, 1);
	  x = XEXP (x, 0);
	}
  ...
  else
    {
      ...
      if (GET_CODE (x) == PLUS
	  && GET_CODE (XEXP (x, 1)) == CONST_INT
	  && IN_RANGE (INTVAL (XEXP (x, 1)), -0x80, 0x7f))
	{
	  address->offset = XEXP (x, 1);
	  x = XEXP (x, 0);
	}
...
print_operand_address (FILE *file, rtx addr)
{
  ...
  if (!m68k_decompose_address (VOIDmode, addr, true, &address))
    gcc_unreachable ();
