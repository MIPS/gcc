...
let rec signed_ctype = function
  ...
  (* Cast to types defined by mode in arm.c, not random types pulled in from
     ...
     compiling with C++.  *)
  | T_uint8 | T_poly8 | T_int8 -> T_intQI
  | T_uint16 | T_poly16 | T_int16 -> T_intHI
...
let deftypes () =
  let typeinfo = [
    ...
    "__builtin_neon_uqi", "poly", 8, 8;
    "__builtin_neon_uhi", "poly", 16, 4;
    ...
    "__builtin_neon_uqi", "poly", 8, 16;
    "__builtin_neon_uhi", "poly", 16, 8;
  ...
  Format.printf "typedef uint8_t poly8_t;\n";
  Format.printf "typedef uint16_t poly16_t;\n"
