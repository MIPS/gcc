...
(define_peephole2
  [(set (match_operand:P 0 "base_reg_operand" "")
	(match_operand:P 1 "fusion_gpr_addis" ""))
   (set (match_operand:SFDF 2 "fusion_extra_operand" "")
	(match_operand:SFDF 3 "fusion_extra_operand" ""))]
  "TARGET_FUSION_EXTRA
   && fusion_extra_p (operands[0], operands[1], operands[2], operands[3])"
  [(parallel [(set (match_dup 0)
		   (unspec:P [(match_dup 1)] UNSPEC_FUSION_EXTRA))
	      (set (match_dup 2)
		   (match_dup 3))])])

;; Merge addis with floating load of constants that can be expressed in
;; single precision, normal mode.
(define_peephole2
  [(set (match_operand:P 0 "base_reg_operand" "")
	(match_operand:P 1 "fusion_gpr_addis" ""))
   (set (match_operand:DF 2 "fpr_reg_operand" "")
	(float_extend:DF (match_operand:SF 3 "offsettable_mem_operand" "")))]
  "TARGET_FUSION_EXTRA
   && fusion_extra_p (operands[0], operands[1], operands[2], operands[3])"
  [(parallel [(set (match_dup 0)
		   (unspec:P [(match_dup 1)] UNSPEC_FUSION_EXTRA))
	      (set (match_dup 2)
		   (float_extend:DF (match_dup 3)))])])

;; Merge addis with floating load of constants that can be expressed in single
;; precision.  Fast math currently generates addis/addi and then the load of
;; the constant from the register, instead of doing the addi as part of the
;; load.
(define_peephole2
  [(set (match_operand:P 0 "base_reg_operand" "")
	(match_operand:P 1 "fusion_gpr_addis" ""))
  ...
  [(parallel [(set (match_dup 0)
		   (unspec:P [(match_dup 1)] UNSPEC_FUSION_EXTRA))
	      (set (match_dup 3)
		   (float_extend:DF (match_dup 5)))])]
{
  rtx addr = gen_rtx_LO_SUM (Pmode, operands[0], operands[2]);
  operands[5] = replace_equiv_address_nv (operands[4], addr, false);
})

;; Fusion insn, created by the define_peephole2 above (and eventually by
;; reload)

(define_insn "*fusion_extra_<P:mode>_<SFDF:mode>"
  [(set (match_operand:P 0 "base_reg_operand" "=b,b")
	(unspec:P [(match_operand:P 1 "fusion_gpr_addis" "")]
 	UNSPEC_FUSION_EXTRA))
   (set (match_operand:SFDF 2 "fusion_extra_operand" "=<Ff>,o")
	(match_operand:SFDF 3 "fusion_extra_operand" "o,<Ff>"))]
  "TARGET_FUSION_EXTRA"
{
  rtx index_reg = operands[0];
  rtx addis_value = operands[1];
  rtx load_store_reg;
  rtx mem;
  rtx addr;
  const char *mode_name = "<SFDF:MODE>mode";
  const char *load_store;
  const char *comment;

  if (MEM_P (operands[2]))
    {
      load_store_reg = operands[3];
      mem = operands[2];
      load_store = (mode == DFmode) ? "stfd" : "stfs";
      comment = "extra store fusion";
    }
  else
    {
      load_store_reg = operands[2];
      mem = operands[3];
      load_store = (mode == DFmode) ? "lfd" : "lfs";
      comment = "extra load fusion";
    }

  gcc_assert (REG_P (load_store_reg) || GET_CODE (load_store_reg) == SUBREG);
  gcc_assert (MEM_P (mem));
  addr = XEXP (mem, 0);

  gcc_assert (GET_CODE (addr) == PLUS || GET_CODE (addr) == LO_SUM);
  gcc_assert (rtx_equal_p (index_reg, XEXP (addr, 0)));

  /* Emit the addis instruction.  */
  emit_fusion_addis (index_reg, addis_value, comment, mode_name);

  /* Emit the D-form load instruction.  */
  emit_fusion_load_store (load_store_reg, index_reg, XEXP (addr, 1),
			  load_store);
...
;; (set (reg) (float_extend (mem))) shows up in loading constants that can
;; be expressed in single precision without loss of precision
(define_insn "*fusion_extra_<P:mode>_extend"
  [(set (match_operand:P 0 "base_reg_operand" "=b")
	(unspec:P [(match_operand:P 1 "fusion_gpr_addis" "")]
 	UNSPEC_FUSION_EXTRA))
   (set (match_operand:DF 2 "fpr_reg_operand" "=d")
	(float_extend:DF (match_operand:SF 3 "offsettable_mem_operand" "o")))]
  "TARGET_FUSION_EXTRA"
{
  rtx index_reg = operands[0];
  rtx addis_value = operands[1];
  rtx load_reg = operands[2];
  rtx mem = operands[3];
  rtx addr;
  const char *mode_name = "SFmode";
  const char *load_store = "lfs";

  gcc_assert (REG_P (load_reg) || GET_CODE (load_reg) == SUBREG);
  gcc_assert (MEM_P (mem));
  addr = XEXP (mem, 0);
  ...
  /* Emit the addis instruction.  */
  emit_fusion_addis (index_reg, addis_value, "extra load fusion", mode_name);

  /* Emit the D-form load instruction.  */
  emit_fusion_load_store (load_reg, index_reg, XEXP (addr, 1), load_store);
