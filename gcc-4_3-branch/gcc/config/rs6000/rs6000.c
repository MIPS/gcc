...
#ifdef ENABLE_BUILTIN_ATTRIBUTES
...
bool (*rs6000_legitimate_address_ptr) (enum machine_mode, rtx, bool)
...
static bool rs6000_debug_rtx_costs (rtx, int, int, int *);
static int rs6000_debug_address_cost (rtx);
...
rs6000_override_options (const char *default_cpu)
{
  ...
  if (rs6000_debug_name)
	...
	{
	  rs6000_legitimate_address_ptr = rs6000_debug_legitimate_address_p;
	  rs6000_legitimize_address_ptr = rs6000_debug_legitimize_address;
...
def_builtin (int mask, const char *name, tree type, int code)
{
  if ((mask & target_flags) || TARGET_PAIRED_FLOAT)
	...
	fatal_error ("internal error: builtin function to %s already processed.",
      ...
      /* Disable setting the builtin attributes for now, until we can figure out
	 an appropriate way to fix gcc.dg/vmx/sn7153.c which fails when the
	 attributes are set.  */
#ifdef ENABLE_BUILTIN_ATTRIBUTES
      {
	tree t = rs6000_builtin_decls[code];
...
{
  bool ret = rs6000_rtx_costs (x, code, outer_code, total);
...
static int
rs6000_debug_address_cost (rtx x)
{
  int ret = TARGET_ADDRESS_COST (x);
			   ...
			   int num_operands ATTRIBUTE_UNUSED)
{
  if (rs6000_warn_cell_microcode)
	...
	warning (OPT_mwarn_cell_microcode,
