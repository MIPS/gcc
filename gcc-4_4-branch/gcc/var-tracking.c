...
mem_dies_at_call (rtx mem)
{
  ...
  while (expr && handled_component_p (expr))
    expr = TREE_OPERAND (expr, 0);

  if (!expr || !SSA_VAR_P (expr) || TREE_CODE (expr) == SSA_NAME)
    return true;

  return (!((TREE_STATIC (expr)
	     || TREE_PUBLIC (expr)
	     || DECL_EXTERNAL (expr))
	    && TREE_READONLY (expr)
	    && !TYPE_NEEDS_CONSTRUCTING (TREE_TYPE (expr)))
	  && (TREE_PUBLIC (expr)
	      || DECL_EXTERNAL (expr)
	      || TREE_ADDRESSABLE (expr)))
	 || (!TREE_READONLY (expr) && is_global_var (expr));
}
	  ...
	  if (cui->store_p)
	    {
		  ...
		  || (find_use_val (loc, GET_MODE (loc), cui)
		      && cselib_lookup (XEXP (loc, 0), Pmode, 0)))
		return MO_VAL_SET;
	    }
	  else
	    {
      ...
      if (type == MO_VAL_LOC)
	{
	  ...
	  if (MEM_P (vloc)
	      ...
	      if (val && !cselib_preserved_value_p (val))
		{
		  ...
		  moa.type = MO_VAL_USE;
		  moa.insn = cui->insn;
		  moa.u.loc = gen_rtx_CONCAT (Pmode, val->val_rtx, mloc);
		  if (dump_file && (dump_flags & TDF_DETAILS))
