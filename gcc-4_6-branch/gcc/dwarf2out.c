...
#define EMIT_ENTRY_VALUE (!dwarf_strict)
#define EMIT_TYPED_DWARF_STACK (!dwarf_strict)
#define EMIT_DEBUG_MACRO (!dwarf_strict)
			 ...
			 enum machine_mode mem_mode)
{
  ...
  if (!EMIT_TYPED_DWARF_STACK && GET_MODE_SIZE (op_mode) > DWARF2_ADDR_SIZE)
    return NULL;
		    ...
		    enum var_init_status initialized)
{
  ...
  switch (GET_CODE (rtl))
      ...
      else if (GET_MODE_SIZE (mode) <= DWARF2_ADDR_SIZE)
	{
      ...
      else if (EMIT_TYPED_DWARF_STACK)
	{
    ...
    case MOD:
      if (GET_MODE_SIZE (mode) > DWARF2_ADDR_SIZE && EMIT_TYPED_DWARF_STACK)
	{
    ...
    case UDIV:
      if (EMIT_TYPED_DWARF_STACK && GET_MODE_CLASS (mode) == MODE_INT)
	{
  ...
  switch (GET_CODE (rtl))
    ...
    default:
      if ((GET_MODE_CLASS (mode) == MODE_INT && GET_MODE (rtl) == mode
	   ...
	   && (dwarf_version >= 4 || !dwarf_strict))
	  || (EMIT_TYPED_DWARF_STACK && mode != VOIDmode && mode != BLKmode))
	{
	  /* Value expression.  */
...
gen_subprogram_die (tree decl, dw_die_ref context_die)
{
  ...
  if (! declaration && TREE_CODE (outer_scope) != ERROR_MARK)
	...
	gen_decl_die (DECL_RESULT (decl), NULL, subr_die);
      ...
      if (call_arg_locations && EMIT_ENTRY_VALUE)
	{
	  struct call_arg_loc_node *ca_loc;
...
dwarf2out_init (const char *filename ATTRIBUTE_UNUSED)
{
  ...
  debug_aranges_section = get_section (DEBUG_ARANGES_SECTION,
				       SECTION_DEBUG, NULL);
  debug_macinfo_section = get_section (!EMIT_DEBUG_MACRO
...
dwarf2out_finish (const char *filename)
{
  ...
  if (debug_info_level >= DINFO_LEVEL_VERBOSE)
    add_AT_macptr (comp_unit_die (),
		   !EMIT_DEBUG_MACRO ? DW_AT_macro_info : DW_AT_GNU_macros,
