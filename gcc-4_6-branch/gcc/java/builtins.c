			   ...
			   tree orig_call)
{
  ...
  if (direct_optab_handler (sync_compare_and_swap_optab, mode)
      ...
      || flag_use_atomic_builtins)
    {
      ...
      stmt = build_call_expr (builtin_decl_explicit (BUILT_IN_BOOL_COMPARE_AND_SWAP_4),
			    ...
			    tree orig_call)
{
  ...
  if (direct_optab_handler (sync_compare_and_swap_optab, mode)
       ...
       but not the multi-word versions.  */
    {
      ...
      stmt = build_call_expr (builtin_decl_explicit (BUILT_IN_BOOL_COMPARE_AND_SWAP_8),
			      3, addr, expected_arg, value_arg);
		     ...
		     tree orig_call)
{
  ...
  stmt = build_call_expr (builtin_decl_explicit (BUILT_IN_SYNCHRONIZE), 0);
