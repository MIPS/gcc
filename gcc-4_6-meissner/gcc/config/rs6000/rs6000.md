...
;; Special isel patterns for optimizing a = (b) ? p[n] : p[n+1].
(define_insn_and_split "isel_adjacent_mem_<mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(unspec:GPR
	 [(if_then_else:GPR
	   (match_operator 1 "scc_comparison_operator"
			   [(match_operand:CC 4 "cc_reg_operand" "y")
			    (const_int 0)])
	   (match_operand:GPR 2 "offsettable_mem_operand" "o")
	   (match_operand:GPR 3 "offsettable_mem_operand" "o"))]
	 UNSPEC_CMOVE))
   (clobber (match_scratch:GPR 5 "=&b"))
   (clobber (match_scratch:GPR 6 "=&r"))]
  "TARGET_ISEL<sel> && TARGET_CMOVE_ADJACENT_MEMORY
   && rs6000_cmove_adjacent_memory_p (operands[2], operands[3])"
  "#"
  "&& reload_completed"
  [(set (match_dup 5) (match_dup 2))
   (set (match_dup 6) (match_dup 3))
   (set (match_dup 0) (if_then_else:GPR
		       (unspec:CC [(match_dup 1)] UNSPEC_CMOVE)
		       (match_dup 5)
		       (match_dup 6)))]
  "
{
  if (GET_CODE (operands[5]) == SCRATCH)
    operands[5] = gen_reg_rtx (GET_MODE (operands[5]));
  if (GET_CODE (operands[6]) == SCRATCH)
    operands[6] = gen_reg_rtx (GET_MODE (operands[6]));
  ...
  /* As a special case if the line size is 1 and pointers are the same size as
     ...
     two addresses.  */
  if (TARGET_CMOVE_ADJACENT_MEMORY == 1 && Pmode == GET_MODE (operands[2]))
    {
      rtx mem = change_address (operands[2], VOIDmode, operands[5]);
      emit_move_insn (operands[5], XEXP (operands[2], 0));
      emit_move_insn (operands[6], XEXP (operands[3], 0));
      emit_insn (gen_isel_adjacent_mem2_<mode> (operands[5], operands[1],
...
(define_insn "isel_adjacent_mem2_<mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r,r")
	(if_then_else:GPR
	 (unspec:CC [(match_operator 1 "scc_comparison_operator"
				     [(match_operand:CC 4 "cc_reg_operand" "y,y")
				      (const_int 0)])] UNSPEC_CMOVE)
	 (match_operand:GPR 2 "reg_or_cint_operand" "O,b")
	 (match_operand:GPR 3 "gpc_reg_operand" "r,r")))]
  "TARGET_ISEL<sel>"
  "*
{
  ...
  strcpy (result, output_isel (operands));
  strcat (result, \"\t\t# isel hack\");
  return result;
}"
  [(set_attr "type" "isel")
   (set_attr "length" "4")])

(define_insn "*isel_adjacent_mem2_rev_<mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(if_then_else:GPR
	 (unspec:CC [(match_operator 1 "scc_rev_comparison_operator"
				     [(match_operand:CC 4 "cc_reg_operand" "y")
				      (const_int 0)])] UNSPEC_CMOVE)
	 (match_operand:GPR 2 "gpc_reg_operand" "b")
	 (match_operand:GPR 3 "gpc_reg_operand" "b")))]
