...
static hashval_t
dead_debug_global_hash (const void *x)
{
  return REGNO (((const struct dead_debug_global_entry *) x)->reg);
}

/* Entries are identical if they refer to the same pseudo.  */
static int
dead_debug_global_eq (const void *x, const void *y)
{
  return ((const struct dead_debug_global_entry *) x)->reg
	 == ((const struct dead_debug_global_entry *) y)->reg;
}

/* Locate the entry for REG in GLOBAL->htab.  */

static struct dead_debug_global_entry *
dead_debug_global_find (struct dead_debug_global *global, rtx reg)
{
  struct dead_debug_global_entry temp_entry, *entry;
  temp_entry.reg = reg;

  entry = (struct dead_debug_global_entry *)
          htab_find_with_hash (global->htab, &temp_entry, REGNO (reg));
...
{
  struct dead_debug_global_entry temp_entry;
  void **slot;
  temp_entry.reg = reg;
  temp_entry.dtemp = dtemp;

  if (global->htab == NULL)
    global->htab = htab_create (31, dead_debug_global_hash,
				dead_debug_global_eq, free);

  slot = htab_find_slot_with_hash (global->htab, &temp_entry, REGNO (reg),
				   INSERT);
  gcc_checking_assert (!*slot);
  *slot = XNEW (struct dead_debug_global_entry);
  *(struct dead_debug_global_entry *) *slot = temp_entry;
  return (struct dead_debug_global_entry *) *slot;
}
...
dead_debug_global_replace_temp (struct dead_debug_global *global,
				...
				bitmap *pto_rescan)
{
  struct dead_debug_global_entry *entry;
  ...
  entry = dead_debug_global_find (global, *DF_REF_REAL_LOC (use));
...
{
  struct dead_debug_use *head, **headp;
  for (head = debug->head, headp = &debug->head; head; head = *headp)
    {
	...
	debug->global->used = BITMAP_ALLOC (NULL);
      ...
      added = bitmap_set_bit (debug->global->used, REGNO (reg));
	...
	if (!dead_debug_insert_temp (debug, REGNO (reg), DF_REF_INSN (ref),
				     DEBUG_TEMP_BEFORE_WITH_VALUE))
	  {
	    ...
	    insn = emit_debug_insn_before (bind, DF_REF_INSN (ref));
	    bitmap_set_bit (debug->to_rescan, INSN_UID (insn));
	  }
...
{
  struct dead_debug_use *newddu;

  if (dead_debug_global_replace_temp (debug->global, use, uregno,
				      &debug->to_rescan))
    return;

  newddu = XNEW (struct dead_debug_use);
			...
			rtx insn, enum debug_temp_where where)
{
  ...
  if (global)
	...
	reg = regno_reg_rtx[uregno];
      entry = dead_debug_global_find (debug->global, reg);
