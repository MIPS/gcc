  ...
  "
{
  ...
  if (TARGET_SWITCH_TABLE_ABSOLUTE_ADDR)
    jmp_reg = op0;
  else
    {
      rtx label_reg = force_reg (Pmode, gen_rtx_LABEL_REF (Pmode, op1));

      if (TARGET_64BIT)
	{
	  rtx tmp = gen_reg_rtx (DImode);
	  emit_insn (gen_extendsidi2 (tmp, op0));
	  op0 = tmp;
	}
      jmp_reg = gen_reg_rtx (Pmode);
      emit_insn (gen_add3_insn (jmp_reg, op0, label_reg));
    }

  if (TARGET_32BIT)
    emit_jump_insn (gen_tablejumpsi (jmp_reg, op1));
  else
    emit_jump_insn (gen_tablejumpdi (jmp_reg, op1));
  DONE;
}")
