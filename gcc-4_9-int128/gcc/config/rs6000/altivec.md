...
;; Like VI, but add ISA 2.07 64-bit integer vector ops
...
(define_mode_attr VI_char [(V16QI "b")
			   (V8HI  "h")
			   (V4SI  "w")
			   (V2DI  "d")])
  ...
  "TARGET_VADDUQM"
{
  if (!TARGET_VSX_TIMODE)
    {
      rs6000_int128_builtin_fixup (operands, 3, V2DImode, UNSPEC_VBPERMQ);
      DONE;
    }
})

(define_insn "altivec_vbpermq_<mode>"
  [(set (match_operand:VINT128 0 "register_operand" "=v")
	(unspec:VINT128 [(match_operand:VINT128 1 "register_operand" "v")
			 (match_operand:VINT128 2 "register_operand" "v")]
			UNSPEC_VBPERMQ))]
