...
;; Register class for floating point conversion, VSX registers allowed
(define_mode_attr rreg [(SF   "ww")
			(DF   "ws")
			(V4SF "wf")
			(V2DF "wd")])

;; Register class for floating point conversion, only traditional FPRs
(define_mode_attr rreg2 [(SF   "f")
			 (DF   "d")])

;; Register class for floating point conversion, only Altivec regs
(define_mode_attr rreg3 [(SF   "wu")
			 (DF   "wv")])
...
;; Reload iterator for the generic secondary reload functions.
(define_mode_iterator RELOAD [QI HI SI DI TI PTI
			      SF DF TF SD DD TD
...
(define_insn_and_split "*extendsfdf2_fpr"
  [(set (match_operand:DF 0 "gpc_reg_operand" "=d,?d,d,wy,?wy,wu")
...
(define_insn_and_split "floatsi<mode>2_lfiwax_mem"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=d,<rreg3>")
	(float:SFDF
	 (sign_extend:DI
...
(define_insn "*mov<mode>_hardfloat32"
  [(set (match_operand:FMOVE64 0 "nonimmediate_operand" "=m,d,d,wv, Z,wa,wa,Y,r,!r,!r")
	(match_operand:FMOVE64 1 "input_operand"         "d,m,d, Z,wv,wa, j,r,Y, r, j"))]
...
; Do not include constraints for loading constants other than 0, as it can
; cause problems during the reload stage.
(define_insn "*mov<mode>_hardfloat64"
  [(set (match_operand:FMOVE64 0 "nonimmediate_operand" "=m,d,d,wv, Z,ws,ws,Y,r,!r,*c*l,!r,*h,r, r,wg, r,wm")
	(match_operand:FMOVE64 1 "input_operand" "        d,m,d, Z,wv,ws, j,r,Y, r,   r, h, 0,j,wg, r,wm, r"))]
  "TARGET_POWERPC64 && TARGET_HARD_FLOAT && TARGET_FPRS && TARGET_DOUBLE_FLOAT
   && (gpc_reg_operand (operands[0], <MODE>mode)
       || gpc_reg_operand (operands[1], <MODE>mode))"
  "@
  ...
  [(set_attr_alternative "type"
	 ...
	 (if_then_else
       ...
       (const_string "mftgpr")
       (const_string "mffgpr")])
   (set_attr "length" "4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4,4")])
...
;; a memory reference that has an address such as pre-modify or offetable
;; address which the VSX/Altivec register set does not provide.

(define_peephole2
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "")
	(match_operand:SFDF 1 "any_operand" ""))
   (set (match_operand:SFDF 2 "gpc_reg_operand" "")
	(match_dup 0))]
  "merge_peephole2_move_p (operands[0], operands[1], operands[2])"
  [(set (match_dup 2) (match_dup 1))])
...

;; Reload patterns.  We may need an additional base register in case the
;; address is invalid for the register class that is being loaded or stored.
(define_expand "reload_<RELOAD:mode>_<P:mptrsize>_store"
  [(parallel [(match_operand:RELOAD 0 "memory_operand" "m")
              (match_operand:RELOAD 1 "register_operand" "r")
...
(define_expand "reload_<RELOAD:mode>_<P:mptrsize>_load"
  [(parallel [(match_operand:RELOAD 0 "register_operand" "=&r")
