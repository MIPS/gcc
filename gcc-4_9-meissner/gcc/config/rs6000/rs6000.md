...
;; Mode iterator for combining all of the boolean operations into a small
;; number of expanders and insns.
(define_mode_iterator BOOL		[SI
					 DI
					 TI
					 PTI
...
;; Mode iterator for boolean operations that are split on GPRs
(define_mode_iterator BOOL_SPLIT	[(DI	"!TARGET_POWERPC64")
					 TI
					 PTI
					 (V16QI	"TARGET_ALTIVEC")
					 (V8HI	"TARGET_ALTIVEC")
...
(define_mode_attr BOOL_REGS_OUTPUT	[(SI	"r")
					 (DI	"&r,r,r,&r")
					 (TI	"&r,r,r,wa,v")
					 (PTI	"&r,r,r")
					 (V16QI	"wa,v,&?r,?r,?r")
...
(define_mode_attr BOOL_REGS_OP1		[(SI	"r")
					 (DI	"r,0,r,r")
					 (TI	"r,0,r,wa,v")
					 (PTI	"r,0,r")
					 (V16QI	"wa,v,r,0,r")
...
(define_mode_attr BOOL_REGS_OP2		[(SI	"r")
					 (DI	"r,r,0,n")
					 (TI	"r,r,0,wa,v")
					 (PTI	"r,r,0")
					 (V16QI	"wa,v,r,r,0")
...
(define_mode_attr BOOL_REGS_UNARY	[(SI	"r")
					 (DI	"r,0,0,r")
					 (TI	"r,0,0,wa,v")
					 (PTI	"r,0,0")
					 (V16QI	"wa,v,r,0,0")
...
;; Mode attribute for the clobber of CC0 for and expanstion
(define_mode_attr BOOL_REGS_AND_CR0	[(SI	"X")
					 (DI	"X,X,X,X")
					 (TI	"X,X,X,X,X")
					 (PTI	"X,X,X")
					 (V16QI	"X,X,X,X,X")
...
;; Mode attribute for the register predicate for boolean operations
(define_mode_attr BOOL_PRED		[(SI	"int_reg_operand")
					 (DI	"int_reg_operand")
					 (TI	"vlogical_operand")
					 (PTI	"int_reg_operand")
					 (V16QI	"vlogical_operand")
					 (V8HI	"vlogical_operand")
					 (V4SI	"vlogical_operand")
					 (V4SF	"vlogical_operand")
					 (V2DI	"vlogical_operand")
					 (V2DF	"vlogical_operand")])

;; Mode attribute for operands[2] of the AND operation
(define_mode_attr BOOL_AND		[(SI	"and_operand")
					 (DI	"logical64_operand")
					 (TI	"vlogical_operand")
					 (PTI	"int_reg_operand")
					 (V16QI	"vlogical_operand")
					 (V8HI	"vlogical_operand")
...
;; Mode attribute for operands[2] of the IOR/XOR operations
(define_mode_attr BOOL_IOR_XOR		[(SI	"logical_operand")
					 (DI	"logical64_operand")
					 (TI	"vlogical_operand")
					 (PTI	"int_reg_operand")
					 (V16QI	"vlogical_operand")
...
;; Mode attribute for the instruction type for boolean operations
(define_mode_attr BOOL_TYPE		[(SI	"integer")
					 (DI	"integer")
					 (TI	"integer,vecsimple,vecsimple")
					 (PTI	"integer")
					 (V16QI "integer,vecsimple,vecsimple")
					 (V8HI  "integer,vecsimple,vecsimple")
					 (V4SI  "integer,vecsimple,vecsimple")
					 (V4SF  "integer,vecsimple,vecsimple")
					 (V2DI  "integer,vecsimple,vecsimple")
					 (V2DF  "integer,vecsimple,vecsimple")])
...
(define_split
  ...
  "")

;; Logical instructions expanders.
;; For doing 64-bit logical instructions on 32-bit, we split and/ior/xor
;; immedate operations during RTL generation time, but we leave
;; register/register operations alone until they are split.

;; All AND insns must have a clobber for all types to allow splitting larger
;; ands to GPRs.
(define_expand "and<mode>3"
  [(parallel
    [(set (match_operand:BOOL 0 "<BOOL_PRED>" "")
	  (and:BOOL (match_operand:BOOL 1 "<BOOL_PRED>" "")
		    (match_operand:BOOL 2 "<BOOL_AND>" "")))
  ...
  ""
{
  if (<MODE>mode == DImode && !TARGET_POWERPC64
      ...
      && can_create_pseudo_p ())
    {
      rs6000_split_logical_di_constant (operands, AND);
      DONE;
    }
})
...
(define_expand "ior<mode>3"
  [(set (match_operand:BOOL 0 "<BOOL_PRED>" "")
	(ior:BOOL (match_operand:BOOL 1 "<BOOL_PRED>" "")
		  (match_operand:BOOL 2 "<BOOL_IOR_XOR>" "")))]
  ""
{
  if ((<MODE>mode == SImode && GET_CODE (operands[2]) == CONST_INT
       && ! logical_operand (operands[2], SImode))
      || (<MODE>mode == DImode && TARGET_POWERPC64
	   && non_logical_cint_operand (operands[2], DImode)))
    {
...
(define_expand "xor<mode>3"
  [(set (match_operand:BOOL 0 "<BOOL_PRED>" "")
	(xor:BOOL (match_operand:BOOL 1 "<BOOL_PRED>" "")
		  (match_operand:BOOL 2 "<BOOL_IOR_XOR>" "")))]
  ""
{
  if ((<MODE>mode == SImode && GET_CODE (operands[2]) == CONST_INT
...
(define_expand "one_cmpl<mode>2"
  [(set (match_operand:BOOL 0 "<BOOL_PRED>" "")
	(not:BOOL (match_operand:BOOL 1 "<BOOL_PRED>" "")))]
  ""
  "")

;; Expanders for creating the vector builtin functions.  ANDC/ORC must reverse
;; argument 1/2, since the canonical format is to have the NOT operand first.
(define_expand "nor<mode>3"
  [(set (match_operand:BOOL_128 0 "<BOOL_PRED>" "")
	(and:BOOL_128
	 (not:BOOL_128 (match_operand:BOOL_128 1 "<BOOL_PRED>" ""))
	 (not:BOOL_128 (match_operand:BOOL_128 2 "<BOOL_PRED>" ""))))]
  ""
  "")

;; Use the canonical format for ANDC, EQV, NAND, and ORC.
(define_expand "andc<mode>3"
  [(set (match_operand:BOOL_128 0 "<BOOL_PRED>" "")
        (and:BOOL_128
	 (not:BOOL_128 (match_operand:BOOL_128 2 "<BOOL_PRED>" ""))
	 (match_operand:BOOL_128 1 "<BOOL_PRED>" "")))]
  ""
{
  if (!TARGET_P8_VECTOR)
    {
	...
	FAIL;
      ...
      emit_insn (gen_one_cmpl<mode>2 (tmp, operands[2]));
      emit_insn (gen_and<mode>3 (operands[0], operands[1], tmp));
      DONE;
    }
})
  ...
  ""
{
  if (!TARGET_P8_VECTOR)
	...
	FAIL;
      ...
      tmp = gen_reg_rtx (<MODE>mode);
      emit_insn (gen_one_cmpl<mode>2 (tmp, operands[2]));
      emit_insn (gen_ior<mode>3 (operands[0], operands[1], tmp));
      DONE;
    }
})
  ...
  if (!TARGET_P8_VECTOR)
    {
      rtx tmp = ((can_create_pseudo_p ())
		 ...
		 : operands[0]);
      emit_insn (gen_and<mode>3 (tmp, operands[1], operands[2]));
      emit_insn (gen_one_cmpl<mode>2 (operands[0], tmp));
    }
})
...
(define_expand "eqv<mode>3"
  [(set (match_operand:BOOL_128 0 "<BOOL_PRED>" "")
	(not:BOOL_128
	 (xor:BOOL_128 (match_operand:BOOL_128 1 "<BOOL_PRED>" "")
		       (match_operand:BOOL_128 2 "<BOOL_PRED>" ""))))]
  ""
{
  if (!TARGET_P8_VECTOR)
    {
...

;; All of the VSX, Altivec, and multi-register logical operations

;; Wide ANDs
(define_insn "*and<mode>3_wide"
  [(set (match_operand:BOOL_SPLIT 0 "<BOOL_PRED>" "=<BOOL_REGS_OUTPUT>")
	(and:BOOL_SPLIT
	 (match_operand:BOOL_SPLIT 1 "<BOOL_PRED>" "%<BOOL_REGS_OP1>")
	 (match_operand:BOOL_SPLIT 2 "<BOOL_PRED>" "<BOOL_REGS_OP2>")))
  ...
  ""
{
  ...
  [(set_attr "type" "integer")
   (set (attr "length")
	(if_then_else
	 (match_test "GET_CODE (operands[2]) == CONST_INT")
	 (const_int 24)
	 (if_then_else
	  (match_test "rs6000_num_insns_logical (operands) == 1")
	  (const_int 4)
	  (if_then_else
	   (match_test "rs6000_num_insns_logical (operands) == 2")
	   (const_int 8)
	   (const_int 16)))))])

(define_split
  [(set (match_operand:BOOL_SPLIT 0 "int_reg_operand" "")
	(and:BOOL_SPLIT
	 (match_operand:BOOL_SPLIT 1 "int_reg_operand" "")
	 (match_operand:BOOL_SPLIT 2 "reg_or_cint_operand" "")))
   (clobber (match_scratch:CC 3 ""))]
  "reload_completed"
  [(const_int 0)]
{
...
;; Wide IOR/XOR operations
(define_insn "*bool<mode>3_wide"
  [(set (match_operand:BOOL_SPLIT 0 "<BOOL_PRED>" "=<BOOL_REGS_OUTPUT>")
	(match_operator:BOOL_SPLIT 3 "boolean_or_operator"
	 [(match_operand:BOOL_SPLIT 1 "<BOOL_PRED>" "%<BOOL_REGS_OP1>")
	  (match_operand:BOOL_SPLIT 2 "<BOOL_PRED>" "<BOOL_REGS_OP2>")]))]
  ""
{
...
;; Wide ANDC operations
(define_insn "*boolc<mode>3_wide"
  [(set (match_operand:BOOL_SPLIT 0 "<BOOL_PRED>" "=<BOOL_REGS_OUTPUT>")
	(match_operator:BOOL_SPLIT 3 "boolean_operator"
	 [(not:BOOL_SPLIT
	   (match_operand:BOOL_SPLIT 1 "<BOOL_PRED>" "<BOOL_REGS_OP1>"))
	  (match_operand:BOOL_SPLIT 2 "<BOOL_PRED>" "<BOOL_REGS_OP2>")]))]
  "GET_MODE_SIZE (<MODE>mode) < 16
   || <MODE>mode == PTImode
   || GET_CODE (operands[3]) == AND	/* ANDC */
   || TARGET_P8_VECTOR"
{
  if (TARGET_VSX && vsx_register_operand (operands[0], <MODE>mode))
    return "xxl%q3 %x0,%x1,%x2";
...
(define_split
  [(set (match_operand:BOOL_SPLIT 0 "int_reg_operand" "")
	(match_operator:BOOL_SPLIT 3 "boolean_operator"
	 [(not:BOOL_SPLIT
	   (match_operand:BOOL_SPLIT 1 "int_reg_operand" ""))
	  (match_operand:BOOL_SPLIT 2 "int_reg_operand" "")]))]
  "reload_completed"
  [(const_int 0)]
{
...
;; Wide NOR/NAND operations
(define_insn "*boolcc<mode>3_wide"
  [(set (match_operand:BOOL_SPLIT 0 "<BOOL_PRED>" "=<BOOL_REGS_OUTPUT>")
	(match_operator:BOOL_SPLIT 3 "boolean_operator"
	 [(not:BOOL_SPLIT
	   (match_operand:BOOL_SPLIT 1 "<BOOL_PRED>" "%<BOOL_REGS_OP1>"))
...
;; Split wide NOT operations
(define_insn "*one_cmpl<mode>3_wide"
  [(set (match_operand:BOOL_SPLIT 0 "<BOOL_PRED>" "=<BOOL_REGS_OUTPUT>")
	(not:BOOL_SPLIT
	 (match_operand:BOOL_SPLIT 1 "<BOOL_PRED>" "<BOOL_REGS_UNARY>")))]
  ""
{
  if (TARGET_VSX && vsx_register_operand (operands[0], <MODE>mode))
