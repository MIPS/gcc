/* Compute machine learning features suitable for predicting the right
   auto vectorisation heuristic.
  
   To add a new feature, put a new COLUMN entry in
   auto-vectorise-features-columns.h.  That will allocate space for it
   in the Features structure and will also ensure it is printed in whichever
   format the user chooses (as well as printing the field in the SQL create
   table statement, if that's what the user is using).
...
/* This typedef describes which type of output we should have.
   The code is able to do both XML and SQL, as well as a
...
enum AutoVectorise_Features_OutputMethod
{
  ...
  /* Print features in SQL format, together with detailed SQL comments
     which make the values easier for a human to read.  */
  AUTOVECTORISE_FEATURES_SQL_VERBOSECOMMENTS,
  /* Print features in XML format.  */
  AUTOVECTORISE_FEATURES_XML
};
typedef enum AutoVectorise_Features_OutputMethod
  AutoVectorise_Features_OutputMethod;

/* This struct holds all the features for each loop.
   Most of the fields are created automatically
   from "auto-vectorise-features-columns.h" .  */
typedef struct features_d Features;
struct features_d
{
# define COLUMN( name, ctype, print, sqltype ) ctype name;
# define ARRAY_COLUMN(name, ctype, print, sqltype, size, sqlnameformat, \
		      sqlnameparam) \
    ctype name[ size ];
 ...
 */
static bool
Features_init (Features * features)
{
  assert (features);
  memset (features, 0, sizeof (Features));
  features->varTable =
    htab_create (32, htab_hash_pointer, htab_eq_pointer, NULL);
...
update_features (Features * features, int code, ...)
{
  ...
  va_start (ap, code);
  target_arch = va_arg (ap, int);
  factor = va_arg (ap, int);
  features->vectorization_factor[target_arch] = factor;
}
 ...
 */
static void
Features_destroy (Features * features)
{
  assert (features);
  if (features->varTable)
    htab_delete (features->varTable);
}
...
/* Print the header if in SQL mode.
   ...
   This prints a create table sql statement which creates all the fields to
   be of the right type.
  
   @param file the file to print to; may not be NULL.  */
static void
Header_printSQL (FILE * file)
{
  int i;
  assert (file);
  fprintf (file,
	   ...
	   " filename varchar not null," " function varchar not null,"
#           define COLUMN(name, ctype, print, sqltype) #name " " sqltype \
	      " not null,"
#           include "auto-vectorise-features-columns.h"
    );
#   define ARRAY_COLUMN(name, ctype, print, sqltype, size, sqlnameformat, \
      ...
      for (i = 0; i < size; ++i) \
	{ \
          fprintf (file, sqlnameformat " %s not null", \
		   (sqlnameparam), (sqltype)); \
        }
#   include "auto-vectorise-features-columns.h"
  fprintf (file,
 ...
 */
static void
Footer_printSQL (FILE * file __attribute__ ((unused)))
{
  /* Nothing to do */
}
 ...
 */
static void
Features_printSQL (Features * features, FILE * file, struct loop *loop,
		   const char *benchmarkId, bool verboseComments)
{
  int i;
  assert (features);
  assert (file);
  ...
  if (verboseComments)
    {
      ...
      fprintf (file, "#loop=%d\n", loop->num);
#     define COLUMN(name, ctype, print, sqltype) \
	fprintf (file, "#" #name "=" print "\n", features->name);
#     define ARRAY_COLUMN(name, ctype, print, sqltype, size, sqlnameformat, \
			  sqlnameparam) \
	for (i = 0; i < size; ++i) \
	  { \
...
#	   include "auto-vectorise-features-columns.h"
	   ,
	   benchmarkId, main_input_filename, current_function_name (),
	   loop->num
 ...
 */
static void
Features_printXML (Features * features, FILE * file, struct loop *loop,
 ...
 */
static void
Features_print (Features * features, FILE * file, struct loop *loop,
		const char *benchmarkId,
		AutoVectorise_Features_OutputMethod outputMethod)
{
  assert (features);
  assert (file);
  assert (benchmarkId);
 ...
 */
static int
Loop_countChildren (struct loop *loop)
{
  int count = 0;
  assert (loop);
 ...
 */
static tree
Features_treeWalker (tree * t, int *walk_subtrees
 ...
 */
static tree
Features_stmtWalker (gimple_stmt_iterator * i, bool * updateNodes
		     __attribute__ ((unused)), struct walk_stmt_info *info)
{
  Features *features = (Features *) info->info;
 ...
 */
static void
Features_walk (Features * features, struct loop *loop)
{
  basic_block *body;
  unsigned i;
  assert (features);
  ...
  for (i = 0; i < loop->num_nodes; i++)
    {
      ...
      info.info = features;
      walk_gimple_seq (bb->il.gimple->seq, Features_stmtWalker,
		       Features_treeWalker, &info);

      memset (&info, 0, sizeof (info));
      info.info = features;
      walk_gimple_seq (bb->il.gimple->phi_nodes, Features_stmtWalker, NULL,
		       &info);
    }
  free (body);
}

/* Get a histogram of the different types of edges that are internal to the
   loop.

   @param features the features; may not be null.
   @param loop the loop; may not be null.  */
static void
Features_cfgEdges (Features * features, struct loop *loop)
{
  basic_block *body;
  unsigned i;
  assert (features);
	...
	if (flow_bb_inside_loop_p (loop, e->dest))
	  {
	    ...
	    if (e->flags & EDGE_FALLTHRU)
	      features->num_edges_fallthru++;
	    if (e->flags & EDGE_ABNORMAL)
	      features->num_edges_abnormal++;
	    if (e->flags & EDGE_ABNORMAL_CALL)
	      features->num_edges_abnormalcall++;
	    if (e->flags & EDGE_EH)
	      features->num_edges_eh++;
	    if (e->flags & EDGE_FAKE)
	      features->num_edges_fake++;
	    if (e->flags & EDGE_DFS_BACK)
	      features->num_edges_dfsback++;
	    if (e->flags & EDGE_CAN_FALLTHRU)
	      features->num_edges_canfallthru++;
	    if (e->flags & EDGE_IRREDUCIBLE_LOOP)
	      features->num_edges_irreducibleloop++;
	    if (e->flags & EDGE_SIBCALL)
	      features->num_edges_sibcall++;
	    if (e->flags & EDGE_LOOP_EXIT)
	      features->num_edges_loopexit++;
	    if (e->flags & EDGE_TRUE_VALUE)
	      features->num_edges_truevalue++;
	    if (e->flags & EDGE_FALSE_VALUE)
	      features->num_edges_falsevalue++;
	    if (e->flags & EDGE_EXECUTABLE)
	      features->num_edges_executable++;
	    if (e->flags & EDGE_CROSSING)
	      features->num_edges_crossing++;
	    if (e->flags & EDGE_COMPLEX)
	      features->num_edges_complex++;
	    if (EDGE_CRITICAL_P (e))
	      features->num_edges_critical++;
	  }
      }
    }
  free (body);
 ...
 */
static int
Features_varCounts_trav (void **slot, void *data)
{
  ...
  if (TREE_CODE (var) == VAR_DECL && (TREE_STATIC (var) || TREE_PUBLIC (var)))
	...
	features->num_global_ptrs++;
    }
  else
    {
      features->num_locals++;
      if (TREE_CODE (TREE_TYPE (var)) == POINTER_TYPE)
	features->num_local_ptrs++;
    }
 ...
 */
static void
Features_varCounts (Features * features)
{
  assert (features);
  htab_traverse_noresize (features->varTable, Features_varCounts_trav,
			  features);
}
 ...
 */
static void
Features_compute (Features * features, struct loop *loop)
{
  assert (features);
  assert (loop);
...
void
AutoVectorise_Features_printHeader (FILE * file,
				    AutoVectorise_Features_OutputMethod
				    outputMethod)
{
  assert (file);
...
void
AutoVectorise_Features_printFooter (FILE * file,
				    AutoVectorise_Features_OutputMethod
				    outputMethod)
{
  assert (file);
...
void
AutoVectorise_Features_setup (void)
{
  ...
  if (filename != NULL)
      ...
      if (file == NULL)
	{
      ...
      AutoVectorise_Features_printHeader
	(file, AUTOVECTORISE_FEATURES_SQL_VERBOSECOMMENTS);
...
void
AutoVectorise_Features_shutdown (void)
{
  const char *filename = getenv (AUTOVECTORISE_FEATURES_ENVNAME);
  if (filename != NULL)
    {
      ...
      if (file == NULL)
	{
      ...
      AutoVectorise_Features_printFooter
	(file, AUTOVECTORISE_FEATURES_SQL_VERBOSECOMMENTS);
