	      ...
	      if (best_edge->dest->rbi->visited == *n_traces)
		{
		  ...
		  if (best_edge->dest != bb)
		      ...
		      else
			{
			  ...
			  edge another_edge;
			  FOR_EACH_EDGE (another_edge, bb->succs, ix)
...
connect_traces (int n_traces, struct trace *traces)
{
  ...
  for (i = 0; i < n_traces || !cold_connected ; i++)
      ...
      for (t2 = t; t2 > 0;)
	{
	  ...
	  best = NULL;
	  best_len = 0;
	  FOR_EACH_EDGE (e, traces[t2].first->preds, ix)
	    {
	      int si = e->src->index;
      ...
      while (1)
	{
	  ...
	  best = NULL;
	  best_len = 0;
	  FOR_EACH_EDGE (e, traces[t].last->succs, ix)
	    {
	      int di = e->dest->index;
	  ...
	  if (best)
		...
		if (e->dest != EXIT_BLOCK_PTR
		    ...
		    && (!best || e->probability > best->probability))
		  {
		    ...
		    FOR_EACH_EDGE (e2, e->dest->succs, ix2)
		      {
 	      ...
 	      if (cond_jump_crosses || !invert_worked)
 		{
 		  ...
 		  if (new_bb)
			 ...
			 partition as bb it's falling through from.  */
		      ...
		      EDGE_SUCC (new_bb, 0)->crossing_edge = true;
 		    }
...
{ 
  ...
  FOR_EACH_EDGE (e, jump_dest->preds, ix)
    if (e->crossing_edge)
      {
