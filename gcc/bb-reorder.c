	      ...
	      if (best_edge->dest->rbi->visited == *n_traces)
		{
		  ...
		  if (best_edge->dest != bb)
		      ...
		      else
			{
			  ...
			  edge another_edge;
			  FOR_EACH_SUCC_EDGE (another_edge, bb, ix)
...
connect_traces (int n_traces, struct trace *traces)
{
  ...
  for (i = 0; i < n_traces || !cold_connected ; i++)
      ...
      for (t2 = t; t2 > 0;)
	{
	  ...
	  best = NULL;
	  best_len = 0;
	  FOR_EACH_PRED_EDGE (e, traces[t2].first, ix)
	    {
	      int si = e->src->index;
      ...
      while (1)
	{
	  ...
	  best = NULL;
	  best_len = 0;
	  FOR_EACH_SUCC_EDGE (e, traces[t].last, ix)
	    {
	      int di = e->dest->index;
	  ...
	  if (best)
		...
		if (e->dest != EXIT_BLOCK_PTR
		    ...
		    && (!best || e->probability > best->probability))
		  {
		    ...
		    FOR_EACH_SUCC_EDGE (e2, e->dest, ix2)
		      {
...
{ 
  ...
  FOR_EACH_PRED_EDGE (e, jump_dest, ix)
    if (e->crossing_edge)
      {
