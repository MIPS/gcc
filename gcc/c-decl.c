...
poplevel (int keep, int dummy ATTRIBUTE_UNUSED, int functionbody)
{
  ...
  for (p = scope->names; p; p = TREE_CHAIN (p))
    {
      const location_t *locus = TREE_LOCUS (p);
		 ...
		 int different_tu)
{
  ...
  /* TLS cannot follow non-TLS declaration.  */
	   ...
	   && !DECL_THREAD_LOCAL (olddecl) && DECL_THREAD_LOCAL (newdecl))
    {
      error ("%Hthread-local declaration of '%D' follows non thread-local "
             "declaration", TREE_LOCUS (newdecl), newdecl);
      error ("%Hprevious declaration of '%D'",
             TREE_LOCUS (olddecl), olddecl);
    }
  /* non-TLS declaration cannot follow TLS declaration.  */
  ...
  else
    {
      ...
      if (errmsg)
	{
          const location_t *locus = TREE_LOCUS (newdecl);
	  switch (errmsg)
	    {
	    case 1:
...
/* Subroutine of pushdecl.
   ...
   code however, later parts of the compiler see something like:
    ...
    on a standard type by checking the current value of input_line.  */
...
define_label (location_t location, tree name)
{
  ...
  if (label
	  ...
	  || (DECL_CONTEXT (label) != current_function_decl
	      && C_DECLARED_LABEL_FLAG (label))))
    {
      location_t *prev_loc = TREE_LOCUS (label);
      ...
      else
	{
	  ...
	  if (asmspec)
		...
		warning ("%Hignoring asm-specifier for non-static local "
                         "variable '%D'", TREE_LOCUS (decl), decl);
	      else
  ...
  if (warn_main > 0 && MAIN_NAME_P (DECL_NAME (decl1)))
    {
      tree args;
      int argct = 0;
      const location_t *locus = TREE_LOCUS (decl1);
  ...
  if (current_scope->parms || current_scope->names || current_scope->tags)
    {
      error ("%Hold-style parameter declarations in prototyped "
	     "function definition", TREE_LOCUS (fndecl));
  ...
  for (last = 0, decl = parms; decl; last = decl, decl = TREE_CHAIN (decl))
    {
      DECL_CONTEXT (decl) = current_function_decl;
      if (DECL_NAME (decl) == 0)
	error ("%Hparameter name omitted", TREE_LOCUS (decl));
      else
	{
...
store_parm_decls_oldstyle (void)
{
  ...
  for (parm = parmids; parm; parm = TREE_CHAIN (parm))
	...
	{
	  const location_t *locus = TREE_LOCUS (decl);
  ...
  for (parm = current_scope->parms; parm; parm = TREE_CHAIN (parm))
    {
      const location_t *locus = TREE_LOCUS (parm);
...
check_for_loop_decls (void)
{
  ...
  for (t = getdecls (); t; t = TREE_CHAIN (t))
    {
      const location_t *locus = TREE_LOCUS (t);
