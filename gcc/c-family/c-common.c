...
const struct c_common_resword c_common_reswords[] =
{
  ...
  { "synchronized",	RID_TRANSACTION_RELAXED, D_CXXONLY | D_TRANSMEM },
  { "atomic_noexcept",	RID_TM_ATOMIC_NOEXCEPT, D_CXXONLY | D_TRANSMEM },
  { "atomic_cancel",	RID_TM_ATOMIC_CANCEL, D_CXXONLY | D_TRANSMEM },
...
/* Return LIST without any TM attributes.  */

tree
remove_tm_attribute (tree list)
{
  ...
  for (; list; list = TREE_CHAIN (list))
    {
      ...
      if (tm_attr_to_mask (name) != 0)
	continue;
      *p = build_tree_list (name, TREE_VALUE (list));
      p = &TREE_CHAIN (*p);
    }
  return newlist;
}
