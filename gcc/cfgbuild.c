  ...
  FOR_BB_BETWEEN (bb, min, max->next_bb, next_bb)
      ...
      if (code == JUMP_INSN)
	{
	  ...
	  /* Otherwise, we have a plain conditional or unconditional jump.  */
	    ...
	    {
              /* When we know the LABEL_REF contained in a REG used, we'll
                 rebuild the indirect jump into simple jump. It is required
                 for sequence abstraction.  */
              if (!JUMP_LABEL (insn)
                  && find_reg_note (insn, REG_LABEL, NULL_RTX))
                rebuild_jump_labels (insn);
