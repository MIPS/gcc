...
const_ok_for_arm (HOST_WIDE_INT i)
{
  ...
  if (TARGET_ARM)
    {
      /* 8 bit constant rotated by a multiple of two.  */
      do
	{
	  if ((i & mask) == 0)
	    return TRUE;
	  mask = (mask << 2)
		 | ((mask & (unsigned HOST_WIDE_INT) 0xffffffff) >> (32 - 2))
		 | ~(unsigned HOST_WIDE_INT) 0xffffffff;
	}
  ...
  else
    {
      ...
      /* 8 bit constant shfted left by any number of bits.  */
      for (n = 0; n <= 24; n++)
	{
	  if ((i & ~(0xffu << n)) == 0)
	    return TRUE;
...
arm_load_pic_register (unsigned int scratch)
{
  ...
  if (TARGET_ARM)
  ...
  else if (TARGET_THUMB2)
    {
      ...
      emit_insn (gen_pic_load_dot_plus_four (pic_tmp, l1));
...
thumb2_legitimate_address_p (enum machine_mode mode, rtx x, int strict_p)
{
  ...
  if (code == POST_INC || code == PRE_DEC
	   ...
	   && rtx_equal_p (XEXP (XEXP (x, 1), 0), XEXP (x, 0)))
    {
      ...
      /* Thumb-2 only has autoincrement by 8-bit constant.  */
      return ((use_ldrd || GET_MODE_SIZE (mode) <= 4)
...
legitimize_tls_address (rtx x, unsigned int model, rtx reg)
{
  ...
  switch (model)
    ...
    case TLS_MODEL_INITIAL_EXEC:
      ...
      /* ??? TLS is totaly hosed for thumb2, and also looks broken for
	 16-bit thumb.  */
      if (TARGET_ARM)
...
/* RTX costs for cores with a slow MUL implimentation.  Thumb-2 is not
...
/* Return TRUE if OP is a low register.  */
int
low_register_operand (rtx op, enum machine_mode mode)
{
  if (GET_MODE (op) != mode && mode != VOIDmode)
    return 0;
...
{
  if (TARGET_THUMB1)
    return thumb1_cmp_operand (op, mode);
...
arm_extendqisi_mem_op (rtx op, enum machine_mode mode)
{
  ...
  /* Thumb2 addressing modes are uniform for modes not larger than a word.
     This predicate shouldn't be used in the first place.  */
  if (TARGET_THUMB2)
    abort ();
...
arm_output_epilogue (rtx sibling)
{
  ...
  if (frame_pointer_needed && TARGET_ARM)
      ...
      if (frame_pointer_needed)
	{
      ...
      else if (offsets->outgoing_args != offsets->saved_regs)
	{
...
/* Generate the prologue instructions for entry into an ARM or 32-bit thumb
   function.  */
void
  ...
  if (flag_pic)
    {
      ...
      if (TARGET_ARM)
	scratch = INVALID_REGNUM;
      else
	{
	  /* ??? thumb2 PIC nected functions.  */
	  if (IS_NESTED (func_type))
	    sorry ("Thumb-2 PIC nested functions");
	  scratch = IP_REGNUM;
	}
      arm_load_pic_register (scratch);
    }
  ...
  switch (code)
    ...
    case '!':
	 ...
	 the current condition code, otherwise print 's'.  */
      if (!(TARGET_THUMB2 && TARGET_UNIFIED_ASM))
	abort ();
...
arm_hard_regno_mode_ok (unsigned int regno, enum machine_mode mode)
{
  ...
  if (TARGET_THUMB1)
    /* For 16-bit Thumb we only allow values bigger than SImode in
...
int
thumb1_cmp_operand (rtx op, enum machine_mode mode)
{
  return ((GET_CODE (op) == CONST_INT
	   && INTVAL (op) < 256
	   && INTVAL (op) >= 0)
...
/* Output a reference from a function exception table to a type_info
   object.  The EABI specified that the symbol should be relocated by
   and R_ARM_TARGET2 relocation.  */
