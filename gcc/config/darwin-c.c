...
/* Remember which frameworks have been seen, so that we can ensure
   that all uses of that framework come from the same framwork.  DIR
...
/* Returns a pointer to a malloced string that contains the real pathname to the file,
   given the base name and the name.  */
static char *
...
/* Add PATH to the system includes. PATH must be malloc-ed and
   NUL-terminated.  System framwork paths are C++ aware.  */
static void
...
/* Add PATH to the bracket includes. PATH must be malloc-ed and
   NUL-terminated.  */
static void
add_framework_path_real (char *path)
{
  cpp_dir *p;
...
extern void (*darwin_register_frameworks_cb)(int);
extern void (*add_framework_path_cb) (char *path);

void darwin_init (void)
{
  darwin_register_frameworks_cb = darwin_register_frameworks_real;
  add_framework_path_cb = add_framework_path_real;
}
