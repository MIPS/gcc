...
;;; when optimizing for size, emit the `bound' instruction
;;; to perform bounds checks whenever we see consecutive conditional traps
;;; that compare a register against low and high bounds.

;;; This approach appears to work, but suffers from some flaws: the
;;; stored bounds are [base..extent), but the bound instruction
;;; expects [base..extent].  The bound instruction expects the base
;;; and extent to be in consecutive memory words, but gcc likes to
;;; stuff them in registers, unless we pass -fno-force-mem.  Even so,
;;; it sometimes screws up and reverses the order of the args.

;;; (define_insn "check_bounds"
;;;   [(check_bounds_rtx (match_operand 0 "register_operand" "")
;;; 		     (match_operand 1 "memory_operand" "")
;;; 		     (match_operand 2 "memory_operand" ""))]
;;;   "optimize_size"
;;;   "bound{l}\\t%0, %1")
...
;;; (define_peephole2
;;;   [(set (reg 17)
;;;         (compare (match_operand 0 "register_operand" "")
;;; 		 (match_operand 1 "memory_operand" "")))
;;;    (trap_if (ltu (reg 17) (const_int 0))
;;; 		 (match_operand 2 "const_int_operand" ""))
;;;    (set (reg 17)
;;;         (compare (match_dup 0)
;;; 		 (match_operand 3 "memory_operand" "")))
;;;    (trap_if (geu (reg 17) (const_int 0))
;;; 		 (match_dup 2))]
;;;   "optimize_size"
;;;   [(check_bounds_rtx (match_dup 0) (match_dup 1) (match_dup 3))]
;;;   "")
