...
(define_attr "type"
  "other,multi,
   ...
   mmx,mmxmov,mmxadd,mmxmul,mmxcmp,mmxcvt,mmxshft, up, low"
  (const_string "other"))
...
;; PointLookout instructions 

(define_insn "<mode>_mk"
  [(set (match_operand:BND 0 "register_operand" "=v")
    (unspec:BND [(match_operand:<bnd_ptr> 1 "register_operand" "r")
		 (match_operand:<bnd_ptr> 2 "address_operand" "p")] UNSPEC_BNDMK))]
  "TARGET_PL && ix86_decomposed_address_p (operands[2], operands[1], false)"
{
  return "bndmk\t{%i1, %0|%0, %i1}";

  /* This could be used for new syntax (R.I.P.).  */
  /* return "bndmk\t{%2, %1, %0|%0, %1, %2}";  */
})

(define_expand "mov<mode>"
  [(set (match_operand:BND 0 "general_operand" "=v,m,v")
        (match_operand:BND 1 "general_operand" "m,v,v"))]
  "TARGET_PL"
{
  ix86_expand_move (<MODE>mode, operands);DONE;
})

(define_insn "*mov<mode>_internal_pl"
  [(set (match_operand:BND 0 "nonimmediate_operand" "=v,m,v")
        (match_operand:BND 1 "general_operand" "m,v,v"))]
  "TARGET_PL"
{
...
;; Following two instructions use clobber to prevent memory
;; instruction to be scheduled before the check.
(define_insn "<mode>_cl"
  [(parallel [(unspec [(match_operand:BND 0 "register_operand" "v")
	               (match_operand:<bnd_ptr> 1 "general_operand" "p")] UNSPEC_BNDCL)
              (use (mem:QI (match_dup 1)))
	      (clobber (mem:QI (match_dup 1)))])]
  "TARGET_PL"
{
  return "bndcl\t{%a1, %0|%0, %a1}";
})

(define_insn "<mode>_cu"
  [(parallel [(unspec [(match_operand:BND 0 "register_operand" "v")
	               (match_operand:<bnd_ptr> 1 "general_operand" "p")] UNSPEC_BNDCU)
              (use (mem:QI (match_dup 1)))
	      (clobber (mem:QI (match_dup 1)))])]
  "TARGET_PL"
{ 
  return "bndcu\t{%a1, %0|%0, %a1}";
})

(define_insn "<mode>_ldx"
  [(parallel [(set:BND (match_operand:BND 0 "register_operand" "=v")
                   (unspec:BND [(match_operand:<bnd_ptr> 1 "address_operand" "p")
	                        (match_operand:<bnd_ptr> 2 "register_operand" "l")] UNSPEC_BNDLDX))
              (use (mem:QI (match_dup 1)))])]
  "TARGET_PL && ix86_decomposed_address_p (operands[1], operands[2], true)"
{
  return "bndldx\t{%I1, %0|%0, %I1}";

  /* This could be used for new syntax (R.I.P.).  */
  /* return "bndldx\t{%2, %E1, %0|%0, %E1, %2}";  */
})

(define_insn "<mode>_stx"
  [(parallel [(unspec [(match_operand:<bnd_ptr> 0 "address_operand" "p")
	               (match_operand:<bnd_ptr> 1 "register_operand" "l")
	               (match_operand:BND 2 "register_operand" "v")] UNSPEC_BNDSTX)
              (use (mem:QI (match_dup 1)))
	      (clobber (mem:QI (match_dup 1)))])]
  "TARGET_PL && ix86_decomposed_address_p (operands[0], operands[1], true)"
{
  return "bndstx\t{%2, %I0|%I0, %2}";

  /* This could be used for new syntax (R.I.P.).  */
  /* return "bndstx\t{%2, %1, %E0|%E0, %1, %2}";  */
}
  [(set_attr "type" "other")]
)
