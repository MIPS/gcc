...
;;??? status registers
(define_insn "bnd_cl<mode>"
  [(unspec [(match_operand:BND 0 "register_operand" "v,v")
	    (match_operand:<bnd_ptr> 1 "general_operand" "r,p")] UNSPEC_BNDCL)]
  "TARGET_PL"
{
  return "bndcl\t{%1, %0|%0, %1}";
})
;;[(set_attr "type" "low")])

(define_insn "bnd_cu<mode>" 
  [(unspec [(match_operand:BND 0 "register_operand" "v,v")
	    (match_operand:<bnd_ptr> 1 "general_operand" "r,p")] UNSPEC_BNDCU)]
  "TARGET_PL" 
{ 
  return "bndcu\t{%1, %0|%0, %1}"; 
}) 

(define_insn "bnd_ldx<mode>"
  [(set (match_operand:BND 0 "register_operand" "=v") 
    (unspec [(match_operand:<bnd_ptr> 1 "address_no_index_operand" "p")
	     (match_operand:<bnd_ptr> 2 "register_operand" "r")] UNSPEC_BNDLDX))]
  "TARGET_PL"
{
  return "bndldx\t{%2, %E1, %0|%0, %E1, %2}";
})

(define_insn "bnd_stx<mode>"
