  ...
  if (TARGET_64BIT)
    {
	 ...
	 This confuse dwarf2out code so do this behind scene.  */
      if (!x86_64_sign_extended_value (operands[2]))
	{
	  rtx reg = gen_rtx_REG (DImode, FIRST_REX_INT_REG + 3); /* R10 */
	  emit_move_insn (reg, operands[2]);
	  emit_insn (gen_pro_epilogue_adjust_stack_rex64_2
		     (operands[0], operands[1], operands[2], reg));
	}
      else
...
(define_insn "pro_epilogue_adjust_stack_rex64_2"
  [(set (match_operand:DI 0 "register_operand" "=r,r")
   ...
   (use (match_operand:DI 3 "register_operand" "r,r"))
   (clobber (reg:CC 17))
   (clobber (mem:BLK (scratch)))]
  "TARGET_64BIT"
{
  switch (get_attr_type (insn))
    ...
    case TYPE_ALU:
      return "add{q}\t{%3, %0|%0, %3}";

    case TYPE_LEA:
      operands[2] = gen_rtx_PLUS (DImode, operands[1], operands[3]);
      return "lea{q}\t{%a2, %0|%0, %a2}";
