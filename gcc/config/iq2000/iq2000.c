...
static int
iq2000_arg_partial_bytes (cumulative_args_t cum, enum machine_mode mode,
			  tree type ATTRIBUTE_UNUSED,
			  bool named ATTRIBUTE_UNUSED)
{
  if (mode == DImode
      && (get_cumulative_args (cum))->arg_words == MAX_ARGS_IN_REGISTERS - 1)
    {
      if (TARGET_DEBUG_D_MODE)
...
iq2000_expand_prologue (void)
{
  ...
  for (cur_arg = fnargs; cur_arg != 0; cur_arg = next_arg)
      ...
      if (TREE_ADDRESSABLE (passed_type))
	{
      ...
      entry_parm = iq2000_function_arg (pack_cumulative_args (&args_so_far),
					passed_mode,
					passed_type, true);

      iq2000_function_arg_advance (pack_cumulative_args (&args_so_far),
  ...
  next_arg_reg = iq2000_function_arg (pack_cumulative_args (&args_so_far),
  ...
  if (cum && targetm.calls.must_pass_in_stack (mode, type))
     {
	  ...
	  passed in registers.  */
       ...
       temp = *cum;
       if (iq2000_function_arg (pack_cumulative_args (&temp), mode, type, named)	   != 0)
	 return 1;
     }
