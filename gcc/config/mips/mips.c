...
#define MIPS_MAX_FIRST_STACK_STEP					\
  (!TARGET_COMPRESSION && !TARGET_USE_SAVE_RESTORE ? 0x7ff0		\
   : (ISA_HAS_SAVE_RESTORE && TARGET_MICROMIPS_R7) ? 0xff8		\
  ...
  if (dest_code == REG && GP_REG_P (REGNO (dest)))
      ...
      if (src_code == CONST_INT)
	{
	  ...
	  if (TARGET_MICROMIPS_R7 && TARGET_LI48)
		...
		return "nop16; sdbbp32 8; # li48\t%0,%1";
	    }
...
mips_save_restore_pattern_p (rtx pattern, HOST_WIDE_INT adjust,
			     struct mips_save_restore_info *info, bool fp_p,
			     bool jrc_p)
{
  ...
  if (!ISA_HAS_SAVE_RESTORE || (fp_p && !ISA_HAS_SAVEF_RESTOREF))
    return false;
      ...
      else if (offset == save_offset)
	{
	  if (GENERATE_MIPS16E_SAVE_RESTORE)
	  ...
	  else
	    {
	      if (fp_p)
		{
		  while (umipsr7_savef_restoref_regs[i++]
			 != FP_REG_P (REGNO (reg)))
...
const char *
mips_output_save_restore (rtx pattern, HOST_WIDE_INT adjust, bool fp_p,
			  bool jrc_p)
{
  ...
  if (GENERATE_MIPS16E_SAVE_RESTORE)
    ...
    {
      int insn_mode = insn16_p && !v0v1_p ? 16 : 32;
...
/* Returns true if it's safe to use SAVE/RESTORE. Returns false otherwise.  */
...
static void
mips_for_each_saved_gpr (HOST_WIDE_INT sp_offset, HOST_WIDE_INT step1,
			 mips_save_restore_fn fn, bool restore_jrc_p,
			 rtx *restore)
{
  int regno;
  HOST_WIDE_INT offset;
  ...
  if (((frame->mask | frame->fmask | frame->acc_mask) != 0)
	...
	step1 = MIN (size, MIPS_MAX_FIRST_STACK_STEP);
	  ...
	  && cfun->machine->safe_to_use_save_restore)
 	{
	  ...
	  mask = frame->mask;
	  insn = mips_build_save_restore (false, &mask, &offset,
...
mips_expand_return (void)
{
  ...
  sp_offset = INTVAL (XEXP(SET_SRC(XVECEXP (restore, 0, 1)), 1));
...
{
  const struct mips_frame_info *frame;
  HOST_WIDE_INT step1, step2, step3, step4;
  HOST_WIDE_INT cop0_acc_size;
  HOST_WIDE_INT fpr_size;
  ...
  if (GENERATE_MIPS16E_SAVE_RESTORE && frame->mask != 0
  ...
  else
    {
      ...
      mips_for_each_saved_gpr (frame->total_size - step2, step2,
  ...
  if (!use_jraddiusp_p && !use_restore_p && !use_restoref_p)
  ...
  if (!sibcall_p)
      ...
      else
	{
	  ...
	  emit_jump_insn (pat);
	  if (use_jraddiusp_p || use_restore_jrc_p)
...
static void
mark_mem (rtx insn, rtx mem, basic_block bb,
  ...
  if (REG_P (base))
      ...
      if (dump_file)
	{
	  fprintf (dump_file, "Marking r%d in insn %d\n", REGNO (base),
		   INSN_UID (insn));
	  dump_rtl_slim (dump_file, insn, NULL_RTX, 1, 0);
	}
		 ...
		 hash_table <offset_entry> * offset_table)
{
  ...
  if (REG_P (base)
	  ...
	  && info->best_cost <= info->orig_cost)
	{
	  ...
	  new_set = gen_rtx_SET (VOIDmode, new_reg,
...
static void
adjust_base_offset (rtx insn, basic_block bb,
		    hash_table <offset_entry> * offset_table)
{
...
typedef struct reference_entry_hasher
{
  typedef reference_entry value_type;
  typedef rtx_def compare_type;

  static inline hashval_t hash (const value_type *v)
    { return mips_hash_base (v->symbol); };
  static bool equal (const value_type *v, const compare_type *c)
    { return rtx_equal_p (v->symbol, c); };
  static void remove (value_type *)
    {};
...
static unsigned int
rest_of_handle_optimize_multi_refs (void)
{
  basic_block bb;
  rtx insn, set;
  int i;
      ...
      if (set)
	{
	  ...
	  if (REG_P (*dest)
		   ...
		   i++)
		{
		  ...
		  /* Look up the symbol in the hash table.  */
		  slot = ref_table.find_slot_with_hash (
			   *src, mips_hash_base (*src), NO_INSERT);
		  ...
		  if (entry)
		      ...
		      if (entry->first_load_p)
			{
			  ...
			  new_move = gen_rtx_SET (VOIDmode, *dest, new_reg);
...
class pass_optimize_multi_refs : public rtl_opt_pass
{
  public:
    ...
    /* opt_pass methods: */
    bool gate () { return gate_optimize_multi_refs (); }
    unsigned int execute () { return rest_of_handle_optimize_multi_refs (); }
