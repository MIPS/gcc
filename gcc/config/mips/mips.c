				 ...
				 mips_save_restore_fn fn)
{
  ...
  if (TARGET_MICROMIPS && !TARGET_MICROMIPS_R7)
...
mips_expand_epilogue (bool sibcall_p)
{
  ...
  if (ISA_HAS_SAVE_RESTORE && frame->mask != 0
      ...
      && cfun->machine->safe_to_use_save_restore)
    {
	 ...
	 of the frame.  */
      mips_frame_barrier ();
      if (!sibcall_p
	  && ISA_HAS_SAVE_RESTORE && TARGET_MICROMIPS_R7)
	{
	  rtx pat;
  ...
  if (!sibcall_p && !TARGET_MICROMIPS_R7)
    {
      mips_expand_before_return ();
      if (cfun->machine->interrupt_handler_p)
	{
...
mips_output_equal_conditional_branch (rtx insn, rtx *operands, bool inverted_p)
{
  ...
  if (TARGET_MICROMIPS
	...
	{
	  /* FIXME */
	  branch[!inverted_p] = MIPS_BRANCH_C ("sdbbp32 10 # b%C1i", "%2,%3,%0");
	  branch[inverted_p] = MIPS_BRANCH_C ("sdbbp32 10 # b%N1i", "%2,%3,%0");
	}
...
mips_output_order_conditional_branch (rtx insn, rtx *operands, bool inverted_p)
{
  ...
  if (operands[3] != const0_rtx)
	...
	{
	  /* FIXME */
	  branch[!inverted_p] = MIPS_BRANCH_C ("sdbbp32 11 # b%C1i", "%2,%3,%0");
	  branch[inverted_p] = MIPS_BRANCH_C ("sdbbp32 11 # b%N1i", "%2,%3,%0");
	}
      else
	{
	  branch[!inverted_p] = MIPS_BRANCH_C ("b%C1", "%2,%3,%0");
