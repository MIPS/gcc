...
static struct list *mips_optimize_speed;
...
mips_read_list (const char * filename)
{
  ...
  if (fd == NULL)
    {
      error ("Bad filename for -m[no-]compress, -moptimize-size-list or "
	     "-moptimize-speed-list list: %s\n", filename);
      return NULL;
    }
			  ...
			  mips_optimize_size))
	{
	  ...
	  *attributes = tree_cons (get_identifier ("optimize"),
      ...
      if (mips_find_list (IDENTIFIER_POINTER (DECL_NAME (decl)),
			  mips_optimize_speed))
	{
	  tree attr_args = build_tree_list
	    (NULL_TREE,
	     mips_optimize_speed_level
	     ? build_string (strlen (mips_optimize_speed_level),
			     mips_optimize_speed_level)
	     : build_string (3, "-O2"));
	  *attributes = tree_cons (get_identifier ("optimize"),
				   attr_args,
...
mips_option_override (void)
{
  ...
  mips_optimize_speed = mips_read_list (mips_optimize_speed_list);
