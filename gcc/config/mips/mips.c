...
static int mips_register_move_cost (enum machine_mode, reg_class_t,
				    reg_class_t);
static int mips_memory_move_cost (enum machine_mode, reg_class_t, bool);
...
mips_restore_reg (rtx reg, rtx mem)
{
  ...
  else if (GET_MODE (reg) == DFmode && !TARGET_FLOAT64 && !TARGET_FLOATXX)
    {
		       ...
		       reg_class_t from)
{
  switch (from)
    ...
    case FP_REGS:
      /* MFC1, etc.  */
      return TARGET_FLOATXX ? mips_memory_move_cost (mode, from, 0) : 4;
...
mips_move_from_gpr_cost (enum machine_mode mode, reg_class_t to)
{
  switch (to)
    ...
    case FP_REGS:
      /* MTC1, etc.  */
      return TARGET_FLOATXX ? mips_memory_move_cost (mode, to, 1) : 4;
...
enum machine_mode
mips_hard_regno_caller_save_mode (unsigned int regno, int nregs,
				  enum machine_mode mode)
{
...
#define ISA_REQUIRES_FLOAT64	(ISA_MIPS32R6)
#define ISA_REQUIRES_FLOAT32	(ISA_MIPS1)
  if ((target_flags_explicit & MASK_FLOAT64) != 0)
      ...
      else if (!TARGET_64BIT && TARGET_FLOAT64)
	{
      ...
      else if (ISA_REQUIRES_FLOAT64 && !TARGET_FLOAT64)
	error ("%<-march=%s%> requires %<-mfp64%>", mips_arch_info->name);
    }
  else
    {
  ...
  else if (ISA_REQUIRES_FLOAT64
	   && (TARGET_FLOATXX
	       || (!TARGET_FLOAT64 && TARGET_DOUBLE_FLOAT)))
    error ("%<-march=%s%> requires %<-mfp64%>",
           mips_arch_info->name);
  else if (ISA_REQUIRES_FLOAT32
           && (TARGET_FLOATXX
               || TARGET_FLOAT64))
