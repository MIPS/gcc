...
{
  const struct mips_frame_info *frame;
  HOST_WIDE_INT step1, step2, step3;
  HOST_WIDE_INT cop0_acc_size;
  HOST_WIDE_INT fpr_size;
  rtx base, adjust;
  rtx_insn *insn;
...
static rtx
can_shrink_mem (rtx_insn *insn, basic_block bb, rtx mem,
		hash_table <offset_entry> * offset_table)
{
  ...
  if (REG_P (base) && (offset > 0)
    ...
    {
      modified_offset = get_modified_offset (bb, REGNO (base), offset,
					     offset_table);
      if ((modified_offset > 0) && (offset >= modified_offset))
	{
	  ...
	  new_insn = gen_rtx_SET (new_reg,
				  gen_rtx_PLUS (mode, base,
						GEN_INT (modified_offset)));
...
static void
split_extended_insn (rtx_insn *insn, basic_block bb,
		     hash_table <offset_entry> * offset_table)
{
  ...
  if (set && MEM_P (SET_DEST (set)))
    {
      ...
      if (new_dest)
	{
	  new_rtx = simplify_replace_rtx (SET_DEST (set),
					  XEXP (SET_DEST (set), 0),
					  new_dest);
  ...
  if (set && MEM_P (SET_SRC (set)))
    {
      ...
      if (new_src)
	{
	  new_rtx = simplify_replace_rtx (SET_SRC (set),
					  XEXP (SET_SRC (set), 0),
					  new_src);
