...
(define_predicate "sw4x4_lw4x4_operand"
  (and (match_code "mem")
       (match_test "m16_based_address_p2 (XEXP (op, 0), mode, uw2_operand)")))
...
(define_predicate "movep_src_register"
  (and (match_code "reg")
       (ior (and (not (and (match_test "TARGET_MICROMIPS_R7")
			   (match_test "TARGET_ADD_NEW_MOVEP")))
  ...
  (match_code "const_int")
{
  ...
  /* Otherwise check whether the constant can be loaded in a single
     instruction.  */
  return !LUI_INT (op) && !SMALL_INT (op) && !SMALL_INT_UNSIGNED (op) && !(TARGET_MICROMIPS_R7 && TARGET_LI48 && ISA_HAS_XLP);
})
