...
(define_expand "movsd"
  [(match_operand:SD 0 "nonimmediate_operand" "")
...
(define_split
  ...
  "ENABLE_DECIMAL_FLOAT && reload_completed
...
(define_expand "movdd"
  [(match_operand:DD 0 "nonimmediate_operand" "")
...
(define_split
  ...
  "ENABLE_DECIMAL_FLOAT && ! TARGET_POWERPC64 && reload_completed
   && ((GET_CODE (operands[0]) == REG && REGNO (operands[0]) <= 31)
       || (GET_CODE (operands[0]) == SUBREG
	   && GET_CODE (SUBREG_REG (operands[0])) == REG
	   && REGNO (SUBREG_REG (operands[0])) <= 31))"
...
(define_split
  [(set (match_operand:DD 0 "gpc_reg_operand" "")
	(match_operand:DD 1 "easy_fp_constant" ""))]
  "ENABLE_DECIMAL_FLOAT && TARGET_POWERPC64 && reload_completed
   && ((GET_CODE (operands[0]) == REG && REGNO (operands[0]) <= 31)
       || (GET_CODE (operands[0]) == SUBREG
	   && GET_CODE (SUBREG_REG (operands[0])) == REG
  ...
  "*
{
  switch (which_alternative)
    ...
    case 1:
	  ...
	  && (rs6000_legitimate_offset_address_p (DDmode, XEXP (operands[1], 0),
    ...
    case 2:
      if (GET_CODE (operands[0]) == MEM
          && (rs6000_legitimate_offset_address_p (DDmode, XEXP (operands[0], 0),
		    reload_completed || reload_in_progress)
...
(define_insn "*movdd_fprs64"
  [(set (match_operand:DD 0 "nonimmediate_operand" "=Y,r,!r,f,f,m,!cl,!r,!h,!r,!r,!r")
...
(define_expand "movtd"
  ...
  "ENABLE_DECIMAL_FLOAT && TARGET_HARD_FLOAT
