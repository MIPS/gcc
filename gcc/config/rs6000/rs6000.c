...
static bool rs6000_decimal_float_supported_p (void);
...
rs6000_return_in_memory (tree type, tree fntype ATTRIBUTE_UNUSED)
{
  ...
  if (DEFAULT_ABI == ABI_V4
      && (TYPE_MODE (type) == TFmode || TYPE_MODE (type) == TDmode))
    return true;
...
rs6000_gimplify_va_arg (tree valist, tree type, tree *pre_p, tree *post_p)
{
  ...
  if (TARGET_HARD_FLOAT && TARGET_FPRS
	  ...
	  || TYPE_MODE (type) == SDmode || TYPE_MODE (type) == DDmode || TYPE_MODE (type) == TDmode))
    {
