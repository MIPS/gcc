...
s390_tls_get_offset ()
{
  ...
  if (!symbol)
    {
      symbol = gen_rtx_SYMBOL_REF (Pmode, "__tls_get_offset");
...
get_some_local_dynamic_name ()
{
  ...
  if (cfun->machine)
    return (const char *) cfun->machine;
  ...
  for (insn = get_insns (); insn; insn = NEXT_INSN (insn))
	...
	continue;
      ...
      *temp_used = 1;
      ...
      if (flag_pic)
	{
	  target = gen_rtx_UNSPEC (SImode, gen_rtvec (1, label), 104);
     ...
     rtx x;
{
  ...
  if (GET_CODE (x) == MEM)
    {
      if (s390_decompose_address (XEXP (x, 0), &addr, TRUE)
	  && find_base_register_in_addr (&addr))
	return TRUE;
    }
  ...
  if (GET_CODE (x) == SET && GET_CODE (SET_DEST (x)) == REG)
    {
      if (s390_decompose_address (SET_SRC (x), &addr, TRUE)
	  && find_base_register_in_addr (&addr))
	return TRUE;
    }
     ...
     int last;
{
  ...
  if (first == last)
    {
      if (TARGET_64BIT)
	insn = make_insn_raw (gen_movdi (addr, gen_rtx_REG (Pmode, first)));
      else
	insn = make_insn_raw (gen_movsi (addr, gen_rtx_REG (Pmode, first)));
  ...
  insn = make_insn_raw (gen_store_multiple (addr,
     ...
     int last;
{
  ...
  if (first == last)
    {
      if (TARGET_64BIT)
        insn = make_insn_raw (gen_movdi (gen_rtx_REG (Pmode, first), addr));
      else
        insn = make_insn_raw (gen_movsi (gen_rtx_REG (Pmode, first), addr));

      return insn;
    }

  insn = make_insn_raw (gen_load_multiple (gen_rtx_REG (Pmode, first),
					   addr,
					   GEN_INT (last - first + 1)));
  return insn;
}
...
s390_init_builtins ()
{
  ...
  builtin_function ("__builtin_thread_pointer", ftype,
              S390_BUILTIN_THREAD_POINTER, BUILT_IN_MD, NULL);

  ftype = build_function_type (void_type_node,
			       tree_cons (NULL_TREE, ptr_type_node,
					  void_list_node));
  builtin_function ("__builtin_set_thread_pointer", ftype,
              S390_BUILTIN_SET_THREAD_POINTER, BUILT_IN_MD, NULL);
}
