...
s390_optimize_prologue (bool base_used)
{
  ...
  if (cfun_frame_layout.first_save_gpr <= BASE_REGISTER 
      && cfun_frame_layout.last_save_gpr >= BASE_REGISTER
      && (TARGET_CPU_ZARCH 
...
s390_frame_info (int base_used, int return_addr_used)
{
  ...
  if (!TARGET_PACKED_STACK)
	...
	if (cfun_fpr_bit_p (i))
      ...
      /* If under 31 bit an odd number of gprs has to be saved we have to correct
  ...
  if (cfun_frame_layout.first_restore_gpr != -1)
	   ...
	   i++)
	{
	 ...
	 this will do good for scheduling.  */
	  ...
	  || (cfun_frame_layout.first_restore_gpr < BASE_REGISTER
