...
generate_tls_wrapper (tree fn)
{
  ...
  if (tree init_fn = get_tls_init_fn (var))
      ...
      if (lookup_attribute ("weak", DECL_ATTRIBUTES (init_fn)))
	{
      ...
      finish_expr_stmt (build_cxx_call (init_fn, CILK_CALL_NORMAL, 0, NULL, 
					tf_warning_or_error));
      if (if_stmt)
	{
				...
				location_t *locus)
{
  ...
  FOR_EACH_VEC_SAFE_ELT (ssdf_decls, i, fndecl)
      ...
      if (! (flags_from_decl_or_type (fndecl) & (ECF_CONST | ECF_PURE)))
	{
	  ...
	  call = cp_build_function_call_nary (fndecl, CILK_CALL_NORMAL,
                                              tf_warning_or_error,
				 ...
				 tsubst_flags_t complain)
{
  ...
  if (CLASS_TYPE_P (TREE_TYPE (fn)))
    expr = build_op_call (fn, args, CILK_CALL_NORMAL, complain);
  else
    expr = cp_build_function_call_vec (fn, args, CILK_CALL_NORMAL, complain);
  if (processing_template_decl && expr != error_mark_node)
