...
static tree cp_parser_primary_expression
  PARAMS ((cp_parser *, cp_parser_id_kind *));
...
static tree cp_parser_class_name
  PARAMS ((cp_parser *, bool, bool, bool, bool));
...
static tree
cp_parser_primary_expression (parser, idk)
     cp_parser *parser;
	...
	if (TREE_CODE (decl) == CONST_DECL 
	...
	else
	  {
	    ...
	    *idk = (parser->scope 
		...
		for (fns = get_overloaded_fn (decl); 
		     fns; 
		     fns = OVL_NEXT (fns))
		  {
		    ...
		    /* If this one has a dependent type, then the name
		       is dependent.  */
		    if (cp_parser_dependent_type_p (TREE_TYPE (fn)))
		      {
			dependent_p = true;
			break;
		      }
...
static tree
cp_parser_id_expression (parser, assume_typename_p, template_keyword_p)
     cp_parser *parser;
...
static tree
cp_parser_nested_name_specifier (parser, typename_keyword_p, type_p)
     cp_parser *parser;
     bool typename_keyword_p;
     bool type_p;
{
...
/* Parse a block-declaration.  
   ...
   function-definition as a block-declaration.  This functionality is
   used in order to avoid having to back up and reparse the
   function-declarator; it is merely an optimization.  */

static void
cp_parser_block_declaration (parser, function_definition_allowed_p)
     cp_parser *parser;
     ...
     cp_parser *parser;
{
  ...
  /* If it is `typename' we do not know yet whether it is a type
  ...
  if (token->keyword == RID_TYPENAME)
    {
      /* Peek at the token after `typename'.  */
      token = cp_lexer_peek_nth_token (parser->lexer, 2);
  ...
  if (nested_name_specifier)
    {
	 ...
	 template-name first we would stop after the 
	 template-name.  */
      cp_parser_parse_tentatively (parser);
	 ...
	 typedef-name.  */
      ...
      /* If that didn't work, try a template-name.  This handles cases
	 like:

           namespace N { template <typename T> class C; } 
           template <typename T> class N::C { ... };

         */
      if (!cp_parser_parse_definitely (parser))
	{
	  type = cp_parser_template_name (parser,
					  /*template_keyword_p=*/false);
	  if (DECL_CLASS_TEMPLATE_P (type))
	    type = DECL_TEMPLATE_RESULT (type);
