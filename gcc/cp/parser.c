  ...
  if (token->type != CPP_EOF)
    {
      ...
      /* Peek at the next token.  */
      next_token = cp_lexer_peek_token (lexer);
      /* Update the source position.  */
      cp_lexer_set_source_position_from_token (next_token);
    }
...
/* Routines to parse various constructs.  
   ...
   Those that return `tree' will return the error_mark_node if and
   only if a parse error occurrs, unless otherwise noted.  If the
   construct is optional (indicated either by an `_opt' in the anem of
   the function that does the parsing or via a FLAGS parameter), then
   NULL_TREE is returned if the construct is not present.  */
...
static int cp_parser_translation_unit
  PARAMS ((cp_parser *));
...
static tree cp_parser_expression_list
  PARAMS ((cp_parser *));
static tree cp_parser_pseudo_destructor_name
  PARAMS ((cp_parser *));
...
static tree cp_parser_decl_specifier_seq 
  PARAMS ((cp_parser *, cp_parser_flags, bool *));
static tree cp_parser_type_specifier
  PARAMS ((cp_parser *, cp_parser_flags, bool *, bool *));
...
static enum rid cp_parser_which_keyword
  PARAMS ((cp_token *));
...
/* Returns the code for the keyword represented by TOKEN.  If TOKEN is
   not a keyword, then RID_MAX is returned instead.  */

static enum rid
cp_parser_which_keyword (token)
     cp_token *token;
{
...
cp_parser_error (parser, message)
     ...
     const char *message;
{
  /* If we're parsing tenatively record the fact that an error
     occurred.  */
     ...
     cp_parser *parser;
{
  ...
  switch (token->type)
    ...
    case CPP_KEYWORD:
      switch (cp_parser_which_keyword (token))
	{
     ...
     cp_parser *parser;
{
  ...
  nested_name_specifier_p 
    = (cp_parser_nested_name_specifier_opt (parser) != NULL_TREE);
...
/* Parse an unqualified-id.
   ...
   BIT_NOT_EXPR is a representation of the class-name.

   FIXME: Document representation of other productions.  */
...
/* Parse an (optional) nested-name-specifier.
   ...
   Sets PARSER->SCOPE to the class (TYPE_DECL) or namespace
  ...
  while (true)
    {
	 ...
	 might destroy it.  */
      ...
      new_scope = cp_parser_class_or_namespace_name (parser);
      /* Look for the `::' token.  */
     ...
     cp_parser *parser;
{
  ...
  keyword = cp_parser_which_keyword (token);
  switch (keyword)
    {
    case RID_DYNCAST:
    case RID_STATCAST:
	...
	if (cp_lexer_next_token_is_keyword (parser->lexer, 
					    RID_TEMPLATE))
	  {
	    /* Consume it.  */
	    cp_lexer_consume_token (parser->lexer);
	    /* FIXME: We should do something with the `template'
	       keyword.  */
	  }
	...
	if (!cp_parser_parse_definitely (parser))
	  {
	    postfix_expression 
	      = cp_parser_primary_expression (parser);
	    /* If the primary-expression was an id-expression, the
	       name will not have been looked up at this point.  */
	    if (TREE_CODE (postfix_expression) == IDENTIFIER_NODE)
	      {
		tree decl;

		/* If this turns out to be the postfix-expression for
		   a function call, we will need to do Koenig lookup.
		   On the other hand, in order to determine whether or
		   not this might be a function call, we must look up
		   the name:

		     [basic.lookup.unqual]

		     For purposes of determining (during parsing)
		     whether an expression is a postfix-pression for a
		     function call, the usual name lookup rules apply.  

		   So, we look up the name here.  */
		/* FIXME: Should cp_parser_id_expression be rejecting
		   identifiers that name types, namespaces, and such
		   so that we do not have to do it here?  */
		decl = cp_parser_lookup_name (parser, postfix_expression);

		/* If we didn't find anything, or what we found was a
		   type, then this wasn't really an id-expression
		   after all.  */
		...
		/* Remember that we still have to look up this name
		   for real.  */
	...
	case CPP_DEREF:
	     ...
	     postfix-expression -> pseudo-destructor-name */
	  {
	    ...
	    if (TREE_TYPE (postfix_expression) != error_mark_node)
	      parser->scope = TREE_TYPE (postfix_expression);

	    /* Consume the `.' or `->' operator.  */
	    cp_lexer_consume_token (parser->lexer);
	    /* Try the pseudo-destructor-name production first.  */
	    cp_parser_parse_tenatively (parser);
	    /* Save the scope, since the name lookup for the
	       pseduo-destructor-name will clear it.  */
	    saved_scope = parser->scope;
	    /* Parse the pseudo-destructor-name.  */
	    type = cp_parser_pseudo_destructor_name (parser);
	    /* If it worked, build an appropriate representation.  */
	    if (cp_parser_parse_definitely (parser))
...
/* Parse a pseudo-destructor-name.
   ...
   Returns the TYPE_DECL named by the pseudo-destructor.  For example,
   for `X::~X', this will return the TYPE_DECL for X.  */

static tree
cp_parser_pseudo_destructor_name (parser)
     cp_parser *parser;
{
  cp_token *token;
  bool nested_name_specifier_p;
  ...
  /* Look for the optional nested-name-specifier.  */
  nested_name_specifier_p 
    = (cp_parser_nested_name_specifier (parser) != error_mark_node);
  ...
  /* Peek at the next token.  */
  token = cp_lexer_peek_token (parser->lexer);
  /* If it's the `::' operator, then we are processing the first
     production.  */
  if (token->type == CPP_SCOPE) 
    {
      /* Consume the `::' token.  */
      cp_lexer_consume_token (parser->lexer);
      /* Look for the type-name again.  */
      cp_parser_type_name (parser);
      /* FIXME: Someone should check that the two types are the same.  */
    }
     ...
     cp_parser *parser;
{
  ...
  if (token->type == CPP_KEYWORD)
    {
      enum rid keyword = cp_parser_which_keyword (token);
     ...
     cp_parser *parser;
{
  ...
  /* Otherwise, it must be that we are looking at a
      ...
      else 
	{
	  ...
	  /* Otherwise, the logical-or-expression is all there is.  */
	  else
	    ;
	}
    }
...
cp_parser_constant_expression (parser)
     cp_parser *parser;
{
  /* FIXME: Enforce semantic constraints.  */
  return cp_parser_conditional_expression (parser);
}
	...
	if (keyword == RID_IF)
	  {
	    ...
	    if (token->type == CPP_KEYWORD
		&& cp_parser_which_keyword (token) == RID_ELSE)
	      {
		tree else_stmt;
  ...
  while (cp_lexer_peek_token (parser->lexer)->type != CPP_SEMICOLON)
    {
      ...
      cp_parser_init_declarator (parser, decl_specifiers);
...
static tree
cp_parser_decl_specifier_seq (parser, flags, declares_class_or_enum)
     cp_parser *parser;
  ...
  *declares_class_or_enum = false;
  ...
  while (true)
	  ...
	  && TYPE_BEING_DEFINED (current_class_type))
	{
	  /* Peek at the next token.  */
	  token = cp_lexer_peek_token (parser->lexer);
	  /* If it is an identifier, and has the same name as the
	     class and the next token is a `(', then we are looking at
	     a constructor.  */
	  if (token->type == CPP_NAME
	      && token->value == constructor_name (current_class_type)
	      && (cp_lexer_peek_nth_token (parser->lexer, 2)->type
		  == CPP_OPEN_PAREN))
	    constructor_p = true;
	}
      /* Outside a class-scope we are looking at a constructor
	 declarator, and not a decl-specifier, if we see class-name,
	 followed by a `::', followed by the class-name again.  */
      else if (!decl_spec)
	{
	  tree type;
	  ...
	  cp_parser_require (parser, CPP_SCOPE, "`::'");
	  /* The next token should be an identifier.  */
	  token = cp_parser_require (parser, CPP_NAME, "class-name");
	  /* Check to see that it matches the class-name and that the
	     next token is the opening `(' for the constructor
	     declarator.  */
	  if (!cp_parser_error_occurred (parser)
	      && token 
	      && token->value == constructor_name (type)
	      && cp_lexer_next_token_is (parser->lexer, CPP_OPEN_PAREN))
	    constructor_p = true;
	  /* In either case we want to rollback the tokens we have
	     consumed.  It is easiest to do this by simulating a parse
	     error.  */
	  cp_parser_error (parser, "<dummy message>");
	  /* Rollback the tokens.  */
...
/* Parse a mem-initializer-id.
   ...
   production.  Returns an IDENTIFIER_NODE indicating the field to be
   initialized for the second production.  */
     ...
     cp_parser *parser;
{
  ...
  switch (token->type)
    ...
    case CPP_AND_AND:
      id = ansi_opname (TRUTH_AND_EXPR);
      break;

    case CPP_OR_OR:
      id = ansi_opname (TRUTH_OR_EXPR);
      break;
      
    case CPP_PLUS_PLUS:
      id = ansi_opname (POSTDECREMENT_EXPR);
      break;
     ...
     bool member_p;
{
  ...
  /* If it's the `export' token, issue a warning that we don't support
     it.  */
  if (cp_parser_which_keyword (token) == RID_EXPORT)
    {
  ...
  /* Parse the dependent declaration.  We don't know yet whether it
     will be a function-definition.  */
  cp_parser_parse_tenatively (parser);
  ...
  /* If this is a member template declaration, let the front end
     know.  */
  if (member_p)
...
cp_parser_template_parameter (parser)
     cp_parser *parser;
{
  /* FIXME: Don't assume the parameter is a type-parameter.  */
...
/* Parse a type-parameter.
   ...
   TREE_PURPOSE indicates whether the `class' or `typename' keyword
   was used.  The TREE_VALUE is an IDENTIFIER_NODE indicating the name
   of the class for the productions other than the template template
...
/* Parse a template-id.
   ...
   If the template-name names a function, or set of functions, returns
   a TEMPLATE_ID_EXPR.  If the template-name names a class, returns a
   TYPE_DECL for the specialization.  */
     ...
     cp_parser *parser;
{
  ...
  template = cp_parser_template_name (parser);
  ...
  if (DECL_CLASS_TEMPLATE_P (template))
    {
      ...
      type 
	= lookup_template_class (template, arguments,
				 NULL_TREE, NULL_TREE,
    ...
    {
      /* FIXME: Handle template-templates.  */
      cp_parser_unimplemented ();
      return error_mark_node;
    }
}
...
/* FIXME: Restrict the declaration as for explicit instantiations.  */
...
/* Parse a type-specifier.
   ...
   type-specifier is a keyword (like `int' or `const') then the
   correspoding IDENTIFIER_NODE is returned.  Otherwise, the type
   itself is returned.
...
static tree
cp_parser_type_specifier (parser, flags, declares_class_or_enum,
			  is_cv_qualifier)
     cp_parser *parser;
     cp_parser_flags flags;
     bool *declares_class_or_enum;
     ...
     cp_parser *parser;
{
  ...
  if (ctk != ctk_enum)
					  ...
					  RID_TEMPLATE))
	{
      ...
      /* FIXME: We should do something with the template keyword.  */
	...
	cp_parser_parse_tenatively (parser);
      /* Parse the template-id.  */
      decl = cp_parser_template_id (parser);
  ...
  /* An elaborated-type-specifier sometimes introduces a new type and
     ...
     We must let xref_tag know which one of these situations we are
     in.  We are in the second situation precisely when the next token
     is a `;'.  */
...
/* Parse an init-declarator.
   ...
   init-declarator:
     declarator asm-specifier [opt] attributes [opt] initializer [opt]

   The DECL_SPECIFIERS apply to this declarator.  Returns a
   reprsentation of the entity declared.  */
     ...
     tree decl_specifiers;
{
  ...
  /* If we're allowing GNU extensions, look for asm-specifiers and
     attributes.  */
  if (cp_parser_allow_gnu_extensions_p (parser))
    {
      /* Look for an asm-specifier.  */
  ...
  /* Because start_decl has permanent side-effects, we should only 
     call it if we know we're going ahead.  By this point, we know
     that we cannot possibly be looking at any other construct.  */
  cp_parser_commit_to_tenative_parse (parser);

  /* Enter the newly declared entry in the symbol table.  */
  /* FIXME: We need to pass in the correct `context' tag.  */
  /* FIXME: We need to get some prefix attributes.  */
  ...
  cp_finish_decl (decl, 
		  ...
		  asm_specification,
		  (is_parenthesized_init 
		   ? LOOKUP_ONLYCONVERTING : LOOKUP_NONE));
...
/* Parse a declarator.
   ...
   represented as an ADDR_EXPR.  Otherwise, the reprsentation is as
     ...
     bool *ctor_dtor_or_conv_p;
{
  ...
  /* Everything else is a direct-declarator.  */
    ...
    declarator = cp_parser_direct_declarator (parser, abstract_p);
...
/* Parse a direct-declarator or direct-abstract-declarator.
   ...
   Returns a representation of the declarator.

   FIXME: Document the representations used.  */

static tree
cp_parser_direct_declarator (parser, abstract_p)
     cp_parser *parser;
     bool abstract_p;
{
  cp_token *token;
  ...
  if (token->type == CPP_OPEN_PAREN)
    {
      ...
      declarator 
	= cp_parser_declarator (parser, abstract_p, NULL);
      /* Expect a `)'.  */
      cp_parser_require (parser, CPP_CLOSE_PAREN, "`)'");
    }
  ...
  *cv_qualifier_seq = NULL_TREE;
  ...
  if (token->type == CPP_MULT || token->type == CPP_AND)
	...
	*cv_qualifier_seq = cp_parser_cv_qualifier_seq_opt (parser);
    ...
    {
      /* FIXME: Handle pointer-to-member case.  */
     ...
     cp_parser *parser;
{
  ...
  parser->scope = NULL_TREE;
  /* In this case, it must be an id-expression.  */
  id_expression = cp_parser_id_expression (parser);
     ...
     cp_parser *parser;
{
  ...
  else if (cp_parser_which_keyword (token) == RID_VOID
	   && (cp_lexer_peek_nth_token (parser->lexer, 2)->type 
	       == CPP_CLOSE_PAREN))
    {
     ...
     cp_parser *parser;
{
  ...
  /* Parse the first parameter declaration.  */
  parameters = cp_parser_parameter_declaration (parser);

  /* Look for more parameters.  */
  while (parameters != error_mark_node)
    {
      cp_token *token;
      tree parameter;
	...
	parameters = error_mark_node;
      /* Otherwise, add the new parameter to the list.  */
      else
	{
...
/* Parse a function-definition.  
   ...
   Returns the FUNCTION_DECL for the function.  */
     ...
     cp_parser *parser;
{
  ...
  if (at_class_scope_p () 
      && TYPE_BEING_DEFINED (current_class_type))
    {
      ...
      /* Check for use of inaccessible types in the declaration of
	 FN.  */
      decl_type_access_control (fn);

      /* Create the structure that will contain information about the
	 inline definition.  */
      inline_definition
	= ggc_alloc_cleared (sizeof (cp_parser_inline_definition));
  ...
  begin_function_definition (decl_specifiers, declarator);

  /* Parse the body of the fucntion itself.  */
  return 
     ...
     cp_parser *parser;
{
  ...
  if (cp_parser_which_keyword (token) == RID_TYPENAME)
    {
      /* Remember that we've seen it.  */
      typename_p = true;
      /* Consume the `typename' token.  */
      cp_lexer_consume_token (parser->lexer);
      /* FIXME: The `typename' keyword should matter in some way.  */
    }
     ...
     bool *nested_name_specifier_p;
{
  ...
  *nested_name_specifier_p = false;
  ...
  /* FIXME: Handle the template-id case.  */
  ...
  if (nested_name_specifier)
    /* Although the grammar says `identifier', it really means
       ...
       already been declared with this syntax.  */
    type = cp_parser_class_name (parser);
     ...
     cp_parser *parser;
{
  ...
  /* FIXME: Handle `using' and the old using syntax.  */
  ...
  while (cp_lexer_peek_token (parser->lexer)->type != CPP_SEMICOLON)
	      ...
	      == CPP_COLON))
	{
	  ...
	  decl = grokbitfield (identifier, 
			       /*attributes=*/NULL_TREE,
			       width);
	  /* Check access to the names used to declare the type of
	     DECL.  */
  ...
  /* FIXME: Check restrictions about omitting decl-specifiers or
     declarators.  */
     ...
     cp_parser *parser;
{
  ...
  token = cp_parser_require (parser, CPP_INT, "`0'");
  ...
  while (true)
    {
      ...
      if (cp_parser_which_keyword (token) != RID_ATTRIBUTE)
	break;
...
/* If *IDK is not CP_PARSER_ID_KIND_NONE,look up *ID, which must be
   the representation of an id.  Replace *ID with the DECL, or a
   LOOKUP_EXPR if *ID is a dependent name.  *IDK is always
   CP_PARSER_ID_KIND_NONE when this function routines.  */
  ...
  switch (*idk)
    ...
    case CP_PARSER_ID_KIND_UNQUALIFIED:
	 ...
	 would not have to look up these names at instantiation time.  */
      if (*id && cp_parser_dependent_type_p (TREE_TYPE (*id)))
...
/* Parse an optional `::' token indicating that the following name is
   ...
   Returns the new value of PARSER->SCOPE.  */
...
/* If the DECL_SPECIFIER_SEQ declares a class, and constitute the
   entire declaration, return the TYPE_DECL for the class.  Otherwise,
   return NULL_TREE.  */

static tree
cp_parser_declares_only_class_p (parser, decl_specifier_seq)
     cp_parser *parser;
     tree decl_specifier_seq;
{
  /* If there's only one decl-specifier, and the next token is a `;',
     then there is no declarator.  */
  if (list_length (decl_specifier_seq) == 1
      && cp_lexer_next_token_is (parser->lexer, CPP_SEMICOLON))
    return TREE_VALUE (decl_specifier_seq);
     ...
     const char *token_desc;
{
  ...
  if (token && cp_parser_which_keyword (token) != keyword)
    {
      dyn_string_t error_msg;
