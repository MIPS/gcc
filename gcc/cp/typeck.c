...
/* Return the type of a <=> expression with non-class type OPTYPE.  */

static tree
spaceship_type (tree optype, tsubst_flags_t complain, location_t loc)
{
  ...
  tree d = lookup_qualified_name (std_node, id, /*prefer_type*/1,
				  /*complain*/false, /*find_hidden*/false);
  if (TREE_CODE (d) == TYPE_DECL)
    d = TREE_TYPE (d);
  else if (complain & tf_error)
    qualified_name_lookup_error (std_node, id, d, loc);
  return d;
}
		    ...
		    tsubst_flags_t complain)
{
  ...
  if (code == SPACESHIP_EXPR)
	...
	result_type = NULL_TREE;
      ...
      if (result_type)
	build_type = spaceship_type (result_type, complain, location);
