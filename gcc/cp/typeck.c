...
static void reset_deduced_models (tree *);

/* The type of entry that stores the equivalence classes of types, as
   generated by same-type constraints in where clauses. This structure
   is stored within a hash table, because types may be shared across
   functions and classes where the same-type constraints differ. */
typedef struct type_equiv_entry_s *type_equiv_entry;
struct type_equiv_entry_s GTY(())
{
  /* The type for which we are introducing additional equivalence
     class information. */
  tree type;

  /* The parent of TYPE in the union-find data structure. */
  tree parent;

  /* The archetype of TYPE, which contains the syntactic bits for the
     entire equivalence class. This field is only valid when TYPE ==
     PARENT, i.e., for the representative of the equivalence class. */
  tree archetype;
};

/* Elements in the stack of type equivalence classes. */
typedef struct type_equiv_classes_entry_s *type_equiv_classes_entry;
struct type_equiv_classes_entry_s GTY(())
{
  /* The type equivalence classes within this scope. */
  htab_t GTY((param_is (struct type_equiv_entry_s))) equiv_classes;

  /* The model deductions that have been made based on these type
     equivalences. */
  tree model_deductions;

  /* The previous type equivalence class. */
  type_equiv_classes_entry prev;
};

/* The stack of type equivalence classes. */
static GTY(()) type_equiv_classes_entry type_equiv_classes;

/* Nonzero indicates that we should ignore the parent link from an
   archetype to the representative of its equivalence class. This is
   necessary when we need to treat archetypes separately, for instance
   to instantiate a template from within a constrained template. */
int archetypes_represent_themselves = 0;

/* DPG TBD: Yet another hack, which forces structural checking in
   certain circumstances where we need models using archetypes to
   match models using template parameters. This is horrible, and
   should be removed as soon as it is possible.  */
int force_structural_checking = 1;
...
complete_type (tree type)
{
  ...
  else if (TREE_CODE (type) == RECORD_TYPE && CLASSTYPE_IS_ARCHETYPE (type))
    define_archetype (TYPE_MAIN_VARIANT (type));
  else 
    {
      /* When we're completing a type that has an archetype, define
	 the archetype along with the type itself.  */
      tree archetype = type_archetype (type);
      if (type != archetype && TREE_CODE (archetype) == RECORD_TYPE)
	define_archetype (TYPE_MAIN_VARIANT (archetype));
    }
...
complete_type_or_else (tree type, tree value)
{
  ...
  if (type == error_mark_node)
    ...
    return NULL_TREE;
  else if (type_archetype (type) != type)
    return type;
...
structural_comptypes (tree t1, tree t2, int strict)
{
  ...
  t1 = type_representative (t1);
  t2 = type_representative (t2);
  ...
  if (TREE_CODE (t1) != ARRAY_TYPE && t1 != TYPE_MAIN_VARIANT (t1))
    t1 = type_representative (TYPE_MAIN_VARIANT (t1));
  if (TREE_CODE (t2) != ARRAY_TYPE && t2 != TYPE_MAIN_VARIANT (t2))
    t2 = type_representative (TYPE_MAIN_VARIANT (t2));
  if (t1 == t2)
    return true;
  ...
  switch (TREE_CODE (t1))
    ...
    case ASSOCIATED_TYPE:
      if (ASSOCIATED_TYPE_NAME (t1) != ASSOCIATED_TYPE_NAME (t2))
	return false;
      if (!same_type_p (TYPE_CONTEXT (t1), TYPE_CONTEXT (t2)))
	return false;
      break;
...
{
  timevar_push (TV_TYPE_COMPARE);

  if (strict == COMPARE_STRICT 
      && (template_processing_nondependent_p ()
	  || force_structural_checking))
    /* Right now, we don't properly maintain canonical types in
       constrained templates (because of our poor handling of
       same-type constraints). So, use structural comparisons
       instead.  */
  ...
  if (strict == COMPARE_STRICT)
    {
      ...
      if (t1 == t2)
	POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, true);

      if (t1 == error_mark_node || t2 == error_mark_node)
	POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, false);

      if (TYPE_STRUCTURAL_EQUALITY_P (t1) || TYPE_STRUCTURAL_EQUALITY_P (t2))
	/* At least one of the types requires structural equality, so
	   perform a deep check. */
	POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, structural_comptypes (t1, t2, strict));
      ...
      if (VERIFY_CANONICAL_TYPES)
	{
	  ...
	  POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, result);
	}
      else
	POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, TYPE_CANONICAL (t1) == TYPE_CANONICAL (t2));
    }
  else if (strict == COMPARE_STRUCTURAL)
    POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, structural_comptypes (t1, t2, COMPARE_STRICT));
  else
    POP_TIMEVAR_AND_RETURN (TV_TYPE_COMPARE, structural_comptypes (t1, t2, strict));
}
				...
				tree access_path, bool preserve_reference)
{
  ...
  /* Pull out the archetype, if we need it. */
  if (uses_template_parms (object_type))
    object_type = type_archetype (object_type);
...
finish_class_member_access_expr (tree object, tree name, bool template_p)
{
  ...
  if (processing_template_decl && expr != error_mark_node
      && !(CLASS_TYPE_P (object_type) 
           && CLASSTYPE_IS_ARCHETYPE (object_type)
           && BASELINK_P (member)
...
{
  return cp_build_function_call (function, params, /*extra_flags=*/0);
}

tree
cp_build_function_call (tree function, tree params, int extra_flags)
{
  tree fntype, fndecl;
  tree name = NULL_TREE;
  int is_method;
  ...
  nargs = convert_arguments (nargs, argarray, parm_types,
			     params, fndecl, LOOKUP_NORMAL|extra_flags);
  if (nargs < 0)
    return error_mark_node;
                   ...
                   int extra_flags)
{
  ...
  if (code == DOTSTAR_EXPR)
    ...
    expr = build_new_op (code, extra_flags|LOOKUP_NORMAL, arg1, arg2, NULL_TREE,
			 overloaded_p);
  ...
  if (processing_template_decl && expr != error_mark_node)
	...
	{
	  TREE_VALUE (expr) = build_min_non_dep (code, 
						 TREE_VALUE (expr), 
						 orig_arg1, orig_arg2);
	  return expr;
	}
		 ...
		 int convert_p ATTRIBUTE_UNUSED)
{
  ...
  /* Immediately reduce to archetypes, if we have them. */
  type0 = type_archetype (type0);
  type1 = type_archetype (type1);
...
cp_pointer_int_sum (enum tree_code resultcode, tree ptrop, tree intop)
{
  ...
  if (TREE_CODE (TREE_TYPE (ptrop)) == POINTER_TYPE
      && opaque_type_p (TREE_TYPE (TREE_TYPE (ptrop))))
    {
      return build2 (resultcode, TREE_TYPE (ptrop), ptrop, intop);
    }
...
pointer_diff (tree op0, tree op1, tree ptrtype)
{
  ...
  tree target_type = type_archetype (TREE_TYPE (ptrtype));
  ...
  if (!COMPLETE_TYPE_P (type_archetype (TREE_TYPE (TREE_TYPE (op1)))))
	...
	if (TREE_CODE (argtype) == POINTER_TYPE)
	  {
	    ...
	    if (!COMPLETE_OR_VOID_TYPE_P (type) && !opaque_type_p (type))
                     ...
                     int extra_flags)
{
  ...
  if (modifycode != NOP_EXPR)
    {
      tree rval = build_new_op (MODIFY_EXPR, extra_flags|LOOKUP_NORMAL, 
                                lhs, rhs,
  ...
  if (!expr)
    expr = build_modify_expr (lhs, modifycode, rhs);
  ...
  if (processing_template_decl && expr != error_mark_node)
	...
	{
	  TREE_VALUE (expr)
	    = build_min_non_dep (MODOP_EXPR, TREE_VALUE (expr), 
				 orig_lhs, 
				 build_min_nt (modifycode, 
					       NULL_TREE, 
					       NULL_TREE),
				 orig_rhs);
	  return expr;
      ...
      return build_min_non_dep (MODOP_EXPR, expr, 
				orig_lhs, 
				build_min_nt (modifycode, NULL_TREE, NULL_TREE),
				orig_rhs);
    }
  ...
  *no_warning = false;
  ...
  if (processing_template_decl)
    {
      ...
      /* If the return value or the return type is type-dependent, we
         cannot do type checking now. */
      if (!retval
          || type_dependent_expression_p (retval)
          || dependent_type_p (valtype))
        {
  ...
  if ((DECL_OVERLOADED_OPERATOR_P (current_function_decl) == NEW_EXPR
      ...
      && (!DECL_CONTEXT (current_function_decl)
          || TREE_CODE (DECL_CONTEXT (current_function_decl)) != RECORD_TYPE
          || !CLASSTYPE_CONCEPT_P (DECL_CONTEXT (current_function_decl))))
  ...
  if (!processing_template_decl 
      && fn_returns_value_p && flag_elide_constructors)
    {
      if (retval != NULL_TREE
...
comp_ptr_ttypes_real (tree to, tree from, int constp)
{
  ...
  for (; ; to = TREE_TYPE (to), from = TREE_TYPE (from))
    {
      /* Work on the archetypes. */
      to = type_archetype (to);
      from = type_archetype (from);
...
/* Routines for handling same-type constraints. */

/* Hash an equivalence class. Note that all TYPENAME_TYPEs of the same
   name are hashed based only on the name, so that we can do a deeper
   comparison of the contexts. */
static hashval_t 
hash_equiv_class (const void* data)
{
  tree type = ((type_equiv_entry)data)->type;

  if (TREE_CODE (type) == TYPENAME_TYPE 
      || TREE_CODE (type) == ASSOCIATED_TYPE)
    return htab_hash_pointer (DECL_NAME (TYPE_NAME (type)));
  else if (TREE_CODE (type) == TEMPLATE_TYPE_PARM)
    return (hashval_t)((TEMPLATE_TYPE_IDX (type) << 4)
		       + TEMPLATE_TYPE_LEVEL (type));
  else
    return htab_hash_pointer (type);
}

/* Determine if two equivalence classes are equal. */
static int 
equal_equiv_class (const void* data1, const void* data2)
{
  tree type1 = ((type_equiv_entry)data1)->type;
  tree type2 = ((type_equiv_entry)data2)->type;

  if (TREE_CODE (type1) != TREE_CODE (type2))
    return false;

  if (TREE_CODE (type1) == TYPENAME_TYPE
      || TREE_CODE (type2) == ASSOCIATED_TYPE)
    {
      if (DECL_NAME (TYPE_NAME (type1)) != DECL_NAME (TYPE_NAME (type2)))
        return false;
      if (cp_type_quals (type1) != cp_type_quals (type2))
        return false;
      return same_type_p (TYPE_CONTEXT (type1), TYPE_CONTEXT (type2));
    }
  else if (TREE_CODE (type1) == TEMPLATE_TYPE_PARM)
    return (TEMPLATE_TYPE_IDX (type1) == TEMPLATE_TYPE_IDX (type2)
	    && TEMPLATE_TYPE_LEVEL (type1) == TEMPLATE_TYPE_LEVEL (type2)
	    && dependent_type_p (type1) == dependent_type_p (type2)
	    && cp_type_quals (type1) == cp_type_quals (type2));
  return htab_eq_pointer (type1, type2);
}

/* Determine if we have type equivalence classes. */
bool
have_type_equivalence_classes (void)
{
  return type_equiv_classes != NULL;
}

/* Push a new set of type equivalence classes, in response to the
   introduction of a new where clause. If INHERITED, then we inherit
   the equivalence classes from the outermost set of equivalence
   classes. */
void 
push_type_equivalence_classes (bool inherited)
{
  type_equiv_classes_entry eq;

  timevar_push (TV_TYPE_REP);

  eq = GGC_NEW (struct type_equiv_classes_entry_s);
  eq->equiv_classes = htab_create_ggc (11, 
                                       hash_equiv_class,
                                       equal_equiv_class,
                                       NULL);
  eq->model_deductions = NULL_TREE;
  if (inherited)
    {
      /* DPG TBD: Copy equivalence classes from previous, or should we
	 just create our own? */
    }
  else
    {
      if (type_equiv_classes)
        /* Reset any deduced models in the parent; they are not valid
           here. */
        reset_deduced_models (&type_equiv_classes->model_deductions);
    }
  eq->prev = type_equiv_classes;
  type_equiv_classes = eq;

  timevar_pop (TV_TYPE_REP);
}

/* Pops the current set of type equivalence classes, at the end of a
   class or function that has a where clause. */
void
pop_type_equivalence_classes (void)
{
  type_equiv_classes_entry victim = type_equiv_classes;

  timevar_push (TV_TYPE_REP);
  gcc_assert (type_equiv_classes);

  /* For each deduced model, mark it like its concept is marked. */
  reset_deduced_models (&victim->model_deductions);

  type_equiv_classes = type_equiv_classes->prev;
  ggc_free (victim);
  timevar_pop (TV_TYPE_REP);
}

/* Retrieve the type equivalence entry for the given TYPE. If INSERT
   is true, then we will insert a new entry into the hash table if
   none exists. */
static type_equiv_entry* 
get_type_equiv_entry (tree type, bool insert)
{
  struct type_equiv_entry_s entry;
  entry.type = type;
  return (type_equiv_entry*)htab_find_slot (type_equiv_classes->equiv_classes,
                                            &entry, 
                                            insert? INSERT : NO_INSERT);
}

/* Set the parent of TYPE to PARENT, merging their type equivalence
   classes.  */
static type_equiv_entry
set_type_parent_real (tree type, tree parent)
{
  type_equiv_entry *slot;

  timevar_push (TV_TYPE_REP);

  slot = get_type_equiv_entry (type, /*insert=*/true);

  if (!*slot)
    {
      *slot = GGC_NEW (struct type_equiv_entry_s);
      (*slot)->type = type;
      (*slot)->archetype = NULL_TREE;
    }
  (*slot)->parent = parent;

  /* If the parent does not use any template parameters, then the
     parent should also become the archetype for this
     representative. For instance, if we have a template parameter T
     and a same-type constraint T == int, then int should be both the
     parent of T and the archetype for that equivalence class. */
  if (type != parent && !uses_template_parms (parent))
    {
      type_equiv_entry *pslot = get_type_equiv_entry (parent, /*insert=*/true);
      if (!*pslot) 
        {
          *pslot = GGC_NEW (struct type_equiv_entry_s);
          (*pslot)->type = parent;
          (*pslot)->parent = parent;
        }
      else
        gcc_assert ((*pslot)->parent == parent);

      (*pslot)->archetype = parent;
    }
...
/* Has the same interface as set_type_parent_real. */
void
set_type_parent (tree type, tree parent)
{
  set_type_parent_real (type, parent);
}

/* Set the archetype of TYPE to ARCHETYPE. The operation will actually
   be performed on the representative for the equivalence class. */
void
set_type_archetype (tree type, tree archetype)
{
  type_equiv_entry *slot;

  timevar_push (TV_TYPE_REP);

  gcc_assert (TREE_CODE (archetype) == RECORD_TYPE);

  /* Find the representative, because only they store the
     archetypes. */
  type = type_representative (type);

  /* Set the archetype of TYPE. */
  slot = get_type_equiv_entry (type, /*insert=*/true);
  if (!*slot)
    {
      *slot = GGC_NEW (struct type_equiv_entry_s);
  ...
  /* Put the archetype in the equivalence class of TYPE. */
  slot = get_type_equiv_entry (archetype, /*insert=*/true);
  if (!*slot)
    {
      *slot = GGC_NEW (struct type_equiv_entry_s);
      (*slot)->type = archetype;
...
/* Find the type equivalence class entry in the hash table for a
   particular type, performing full path compression when COMPRESS is
   true. Returns the entry if found, NULL otherwise. */
static type_equiv_entry
find_type_rep_equiv_entry (tree type, bool compress)
{
  tree rep = type;
  type_equiv_entry result = NULL;
  struct type_equiv_entry_s entry;
  ...
  for (;;) 
    {
      type_equiv_entry eq;
      entry.type = rep;
      eq = (type_equiv_entry) htab_find (type_equiv_classes->equiv_classes,
                                         &entry);
      if (eq) 
        {
          if (result == eq)
  ...
  /* If we didn't find anything, we might need to break down a type
     constructor and search for representatives of the types inside
     it.  */ 
  if (!result)
    {
      int quals = cp_type_quals (type);

      /* Handle qualifiers by searching for a representative for any
         variant of TYPE whose qualifiers are a strict subset of the
         qualifiers on TYPE.  */
      if (quals) 
        {
          type_equiv_entry equiv = NULL;
          tree t;
          for (t = TYPE_MAIN_VARIANT (type); t; t = TYPE_NEXT_VARIANT (t))
            {
              int t_quals = cp_type_quals (t);
              if (t_quals != quals && (t_quals & quals) == t_quals)
                {
                  equiv = find_type_rep_equiv_entry (t, /*compress=*/true);
                  if (equiv)
                    break;
                }
            }
          ...
          if (equiv)
            {
              type_equiv_entry *slot = NULL;
              /* Build appropriately qualified versions of the
                 representative and archetype. */
              tree rep = equiv->parent;
              tree archetype = equiv->archetype;

              if (TREE_CODE (rep) != REFERENCE_TYPE)
                rep = cp_build_qualified_type (rep, quals);

              if (archetype && TREE_CODE (archetype) != REFERENCE_TYPE)
                archetype = cp_build_qualified_type (archetype, quals);

              slot = get_type_equiv_entry (rep, /*insert=*/true);
              *slot = GGC_NEW (struct type_equiv_entry_s);
              (*slot)->type = rep;
              (*slot)->parent = rep;
              (*slot)->archetype = archetype;

              result = set_type_parent_real (type, rep);
            }
          POP_TIMEVAR_AND_RETURN (TV_TYPE_REP, result);
        }
    }
...
/* Returns the representative of the equivalence class in which the
   type T belongs, by following the parent pointers. Performs full
   path compression to reduce the lookup effort. 
   If ARCHETYPES_REPRESENT_THEMSELVES is nonzero, the representative of any
   archetype is the archetype itself.
*/
tree
type_representative (tree type)
{
  type_equiv_entry eq;

  eq = find_type_rep_equiv_entry (type, /*compress=*/true);
  if (eq && archetypes_represent_themselves && type == eq->archetype)
    return type;
  else if (eq)
    return eq->parent;
  else
...
/* Retrieve the archetype for the equivalence class containing
   TYPE. If no archetype can be found, then we return TYPE. */
tree 
type_archetype (tree type)
{
  type_equiv_entry eq = find_type_rep_equiv_entry (type, /*compress=*/true);
  if (eq && eq->archetype)
    /* We have an archetype; retrieve it.  */
    return eq->archetype;
  else if (eq && eq->parent != type)
    /* Extract or build the parent's archetype.  */
    return type_archetype (eq->parent);
  else if (type_equiv_classes) 
    {
      /* Build an archetype for this type.  */
      tree archetype;

      timevar_push (TV_TYPE_REP);
      archetype = maybe_build_archetype (eq? eq->parent : type);
      timevar_pop (TV_TYPE_REP);
      return archetype ? archetype : type;
    }
  else 
    return type;
}

/* Perform a union on the equivalence classes of types T1 and
   T2. After this operation, same_type_p (T1, T2) will be
   true. Returns the representative of the resulting equivalence
   class. 

   DPG TBD: We might be able to use the union-by-rank heuristic here
   to improve compilation performance, but we can't always do so
   because, for instance, we don't want the parent of "int" to be a
   template parameter "T".
*/
tree
union_types (tree t1, tree t2)
{
  timevar_push (TV_TYPE_REP);
  t1 = type_representative (t1);
  t2 = type_representative (t2);
  ...
  if (same_type_p (t1, t2))
    POP_TIMEVAR_AND_RETURN (TV_TYPE_REP, t1);

  {
    int quals1 = cp_type_quals (t1);
    int quals2 = cp_type_quals (t2);
    
    if (quals1 || quals2)
      {
        /* The qualifiers of one type must be a subset of the qualifiers
           of the other type. */
        if ((quals1 & quals2) != quals1 && (quals1 & quals2) != quals2)
          {
            error ("incompatible qualifiers in same-type constraint "
                   "%<%T == %T%>", t1, t2);
          }
        else if (quals1 == quals2)
          {
            t1 = TYPE_MAIN_VARIANT (t1);
            t2 = TYPE_MAIN_VARIANT (t2);
          }
        else if ((quals1 & quals2) == quals2)
          {
            tree tmp;
            
            t1 = cp_build_qualified_type (t1, quals1 & ~quals2);
            t2 = TYPE_MAIN_VARIANT (t2);
            
            /* Swap them, so that the more-qualified type is the parent. */
            tmp = t1;
            t1 = t2;
            t2 = tmp;
          }
          ...
          {
            t1 = TYPE_MAIN_VARIANT (t1);
            t2 = cp_build_qualified_type (t2, quals2 & ~quals1);
          }
      }
  }
  ...
  if (TREE_CODE (t1) == TEMPLATE_TYPE_PARM
      ...
      || TREE_CODE (t1) == TYPENAME_TYPE
      || TREE_CODE (t1) == ASSOCIATED_TYPE)
    {
      set_type_parent (t1, t2);
      POP_TIMEVAR_AND_RETURN (TV_TYPE_REP, t2);
    }
  else if (TREE_CODE (t2) == TEMPLATE_TYPE_PARM
           || TREE_CODE (t2) == TEMPLATE_TEMPLATE_PARM
           || TREE_CODE (t2) == TYPENAME_TYPE
           || TREE_CODE (t2) == ASSOCIATED_TYPE)
    {
      set_type_parent (t2, t1);
      POP_TIMEVAR_AND_RETURN (TV_TYPE_REP, t1);
    }
    ...
    {     
      switch (TREE_CODE (t1))
        ...
        case VOID_TYPE:
          if (!same_type_p (t1, t2))
            error("invalid same-type constraint %<%T == %T%>", t1, t2);
          break;
          
        case METHOD_TYPE:
          union_types (TYPE_METHOD_BASETYPE (t1), TYPE_METHOD_BASETYPE (t2));
          /* Fall through */
        case FUNCTION_TYPE:
          {
            tree arg1 = TYPE_ARG_TYPES (t1);
            tree arg2 = TYPE_ARG_TYPES (t2);
            while (arg1 && arg2)
              {
                if (TREE_VALUE (arg1) != void_type_node 
                    && TREE_VALUE (arg2) != void_type_node)
                  union_types (TREE_VALUE (arg1), TREE_VALUE (arg2));
            ...
            if (arg1 || arg2)
              {
                error ("same-type constraint %<%T == %T%> requires function "
                       "types with the same number of arguments", t1, t2);
              }
          }
          /* Fall through */
        case OFFSET_TYPE: 
        ...
        case VECTOR_TYPE:
        case ARRAY_TYPE:
          if (TREE_CODE (t1) == OFFSET_TYPE)
            union_types (TYPE_OFFSET_BASETYPE (t1), TYPE_OFFSET_BASETYPE (t2));
          else if (TREE_CODE (t1) == VECTOR_TYPE)
            {
              if (TYPE_PRECISION (t1) != TYPE_PRECISION (t2))
                error ("same-type constraint %<%T == %T%> requires types to "
                       "have the same precision", t1, t2);
            }
          else if (TREE_CODE (t1) == ARRAY_TYPE)
            {
              if (TYPE_DOMAIN (t1) != TYPE_DOMAIN (t2))
                error ("same-type constraint %<%T == %T%> requires array sizes"
                       " to match", t1, t2);
            }
          union_types (TREE_TYPE (t1), TREE_TYPE (t2));
          break;
          ...
          if (TYPE_PTRMEMFUNC_P (t1))
            {
              if (!TYPE_PTRMEMFUNC_P (t2))
                ...
                {
                  union_types (TYPE_PTRMEMFUNC_FN_TYPE (t1),
                               TYPE_PTRMEMFUNC_FN_TYPE (t2));
                  union_types (TYPE_PTRMEMFUNC_OBJECT_TYPE (t1),
                               TYPE_PTRMEMFUNC_OBJECT_TYPE (t2));
                }
            }
          else if (CLASSTYPE_USE_TEMPLATE (t1))
            {
              tree tmpl1 = most_general_template (CLASSTYPE_TI_TEMPLATE (t1));
              tree tmpl2 = most_general_template (CLASSTYPE_TI_TEMPLATE (t2));
              if (tmpl1 != tmpl2)
                error ("both sides of same-type constraint %<%T == %T%> must "
                       "refer to the same template", t1, t2);
              if (CLASSTYPE_TI_ARGS (t1))
                {
                  ...
                  if (TREE_VEC_LENGTH (args1) != TREE_VEC_LENGTH (args2))
                    error ("both sides of same-type constraint %<%T == %T%> "
                           "must have the same number of template arguments",
                           t1, t2);
                  else
                    {
                      for (i = 0; i < TREE_VEC_LENGTH (args1); i++)
                        {
                          if (TYPE_P (TREE_VEC_ELT (args1, i)))
                            union_types (TREE_VEC_ELT (args1, i), 
                                         TREE_VEC_ELT (args2, i));
                        }
                    }
                }
            }
          else if (!same_type_p (t1, t2))
            error("invalid same-type constraint %<%T == %T%>", t1, t2);
          break;

        case TYPEOF_TYPE:
          error ("typeof cannot be used in a same-type constraint");
          break;

        case BOUND_TEMPLATE_TEMPLATE_PARM:
          sorry ("bound template template parameters");
          break;

        case UNBOUND_CLASS_TEMPLATE:
          sorry ("unbound class templates");
          break;
...
/* Note that we have deduced whether this MODEL_ID is a model or not
   based on the requirements in a template. These conclusions will be
   undone when we finish processing the template. */
void
note_deduced_model (tree model_id)
{
  gcc_assert (type_equiv_classes);

  type_equiv_classes->model_deductions = 
    tree_cons (model_id, NULL_TREE, type_equiv_classes->model_deductions);
}

static void
reset_deduced_models (tree *model_deductions)
{
  tree deduction = *model_deductions;
  while (deduction)
    {
      tree template, concept, model_id;

      model_id = TREE_PURPOSE (deduction);
      template = most_general_template (CLASSTYPE_TI_TEMPLATE (model_id));
      concept = TREE_TYPE (template);
      CLASSTYPE_USE_CONCEPT (model_id) = CLASSTYPE_USE_CONCEPT (concept);

      deduction = TREE_CHAIN (deduction);
    }

  *model_deductions = NULL_TREE;
}
