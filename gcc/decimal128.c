...
/* ------------------------------------------------------------------ */
decimal128 * decimal128FromNumber(decimal128 *d128, decNumber *dn,
                                decContext *set) {
  ...
  if (dn->bits&DECSPECIAL) {                      /* a special value */
    ...
    if (msd>=8) comb=0x18 | (msd & 0x01) | ((exp>>11) & 0x06);
     else comb=(msd & 0x07) | ((exp>>9) & 0x18);
    d128->bytes[0]=(uByte)(comb<<2);
    exp&=0xfff;                                   /* remaining exponent bits */
    decimal128SetExpCon(d128, exp);
    }

  if (isneg) decimal128SetSign(d128, 1);
...
/* ------------------------------------------------------------------ */
decNumber * decimal128ToNumber(decimal128 *d128, decNumber *dn) {
  ...
  if (decimal128Sign(d128)) dn->bits=DECNEG;
  ...
  if (!(dn->bits & DECINF)) {
    ...
    decDenseUnpackCoeff(d128->bytes, sizeof(d128->bytes), dn, bunches, odd);
    }
...
char * decimal128ToEngString(decimal128 *d128, char *string){
  decNumber dn;                         /* work */
  decimal128ToNumber(d128, &dn);
...
/* ------------------------------------------------------------------ */
decimal128 * decimal128FromString(decimal128 *result, char *string,
                                decContext *set) {
  decContext dc;                             /* work */
  decNumber dn;                              /* .. */
...
void decimal128Show(decimal128 *d128) {
  ...
  printf(" D128> %s [S:%d Cb:%02x E:%d]\n", buf,
         decimal128Sign(d128), decimal128Comb(d128), decimal128ExpCon(d128));
  } /* decimal128Show */
#endif
