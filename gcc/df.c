...
static void df_uid_refs_remove (struct df *, unsigned int);
static void df_insn_refs_mark_deleted (struct df *, basic_block, rtx);
...
static struct ref *
df_ref_search_cached (struct df *df, rtx reg, rtx *loc, rtx insn,
	       ...
	       enum df_ref_type ref_type, enum df_ref_flags ref_flags)
{
  ...
  this_ref = df_ref_search_cached (df, reg, loc, insn, ref_type, ref_flags);
  if (this_ref)
    return this_ref;
...
read_modify_subreg_p (rtx x)
{
  ...
  /* Paradoxical subreg writes don't leave a trace of the old content.  */
  return (isize > osize && osize < GET_MODE_SIZE (SImode)
	  && isize > UNITS_PER_WORD);
}
...
df_bb_refs_update (struct df *df, basic_block bb)
{
  ...
  FOR_BB_INSNS (bb, insn)
      ...
      if (bitmap_bit_p (df->insns_modified, uid))
	{
	  ...
	  df_insn_refs_mark_deleted (df, bb, insn);
...
static void
df_uid_refs_remove (struct df *df, unsigned int uid)
{
  struct df_link *link;
...
static void
df_insn_refs_mark_deleted (struct df *df, basic_block bb ATTRIBUTE_UNUSED,
			   rtx insn)
{
  struct df_link *link;
  unsigned int uid;
  ...
  for (link = df->insns[uid].defs; link; link = link->next)
    DF_REF_FLAGS (link->ref) |= DF_REF_DELETED;
...
df_insn_refs_unlink (struct df *df, basic_block bb ATTRIBUTE_UNUSED, rtx insn)
{
  ...
  /* Unlink all refs defined by this insn.  */
  for (link = df->insns[uid].defs; link; link = link->next)
    if (DF_REF_FLAGS (link->ref) & DF_REF_DELETED)
      df_ref_remove (df, link->ref);
