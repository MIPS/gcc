...
{
  dw_loc_descr_ref descr = ggc_alloc_cleared_dw_loc_descr_struct();
...
add_var_loc_to_decl (tree decl, struct var_loc_node *loc)
{
  ...
  if (*slot == NULL)
    {
      temp = ggc_alloc_cleared_var_loc_list_def();
...
add_const_value_attribute (dw_die_ref die, rtx rtl)
{
  switch (GET_CODE (rtl))
	...
	unsigned char *array = ggc_alloc_atomic (length * elt_size);
	unsigned int i;
	unsigned char *p;
