     ...
     char **argv;
{
  ...
  if (opt[1] == 'f')
    {
      if (! strcmp (&opt[2], "version"))
	{
	  ffe_set_is_version (TRUE);
	  ffe_set_is_do_internal_checks (TRUE);
	}
      else if (! strcmp (&opt[2], "null-version"))
	ffe_set_is_null_version (TRUE);
      else if (! strcmp (&opt[2], "f66"))
	{
	  ffe_set_is_onetrip (TRUE);
	  ffe_set_is_ugly_assumed (TRUE);
	}
      else if (! strcmp (&opt[2], "no-f66"))
	{
	  ffe_set_is_onetrip (FALSE);
	  ffe_set_is_ugly_assumed (FALSE);
	}
      else if (! strcmp (&opt[2], "f77"))
	{
	  ffe_set_is_backslash (TRUE);
	  ffe_set_is_typeless_boz (FALSE);
	}
      else if (! strcmp (&opt[2], "no-f77"))
	{
	  ffe_set_is_backslash (FALSE);
	}
      else if (! strcmp (&opt[2], "f90"))
	ffe_set_is_90 (TRUE);
      else if (! strcmp (&opt[2], "no-f90"))
	ffe_set_is_90 (FALSE);
      else if (! strcmp (&opt[2], "automatic"))
	ffe_set_is_automatic (TRUE);
      else if (! strcmp (&opt[2], "no-automatic"))
	ffe_set_is_automatic (FALSE);
      else if (! strcmp (&opt[2], "dollar-ok"))
	ffe_set_is_dollar_ok (TRUE);
      else if (! strcmp (&opt[2], "no-dollar-ok"))
	ffe_set_is_dollar_ok (FALSE);
      else if (! strcmp (&opt[2], "f2c"))
	ffe_set_is_f2c (TRUE);
      else if (! strcmp (&opt[2], "no-f2c"))
	ffe_set_is_f2c (FALSE);
      else if (! strcmp (&opt[2], "f2c-library"))
	ffe_set_is_f2c_library (TRUE);
      else if (! strcmp (&opt[2], "no-f2c-library"))
	ffe_set_is_f2c_library (FALSE);
      else if (! strcmp (&opt[2], "flatten-arrays"))
	ffe_set_is_flatten_arrays (TRUE);
      else if (! strcmp (&opt[2], "no-flatten-arrays"))
	ffe_set_is_flatten_arrays (FALSE);
      else if (! strcmp (&opt[2], "free-form"))
	ffe_set_is_free_form (TRUE);
      else if (! strcmp (&opt[2], "no-free-form"))
	ffe_set_is_free_form (FALSE);
      else if (! strcmp (&opt[2], "fixed-form"))
	ffe_set_is_free_form (FALSE);
      else if (! strcmp (&opt[2], "no-fixed-form"))
	ffe_set_is_free_form (TRUE);
      else if (! strcmp (&opt[2], "pedantic"))
	ffe_set_is_pedantic (TRUE);
      else if (! strcmp (&opt[2], "no-pedantic"))
	ffe_set_is_pedantic (FALSE);
      else if (! strcmp (&opt[2], "vxt"))
	ffe_set_is_vxt (TRUE);
      else if (! strcmp (&opt[2], "not-vxt"))
	ffe_set_is_vxt (FALSE);
      else if (! strcmp (&opt[2], "vxt-not-f90"))
	warning ("%s no longer supported -- try -fvxt", opt);
      else if (! strcmp (&opt[2], "f90-not-vxt"))
	warning ("%s no longer supported -- try -fno-vxt -ff90", opt);
      else if (! strcmp (&opt[2], "no-ugly"))
	{
      ...
      else if (! strcmp (&opt[2], "ugly-args"))
	ffe_set_is_ugly_args (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-args"))
	ffe_set_is_ugly_args (FALSE);
      else if (! strcmp (&opt[2], "ugly-assign"))
	ffe_set_is_ugly_assign (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-assign"))
	ffe_set_is_ugly_assign (FALSE);
      else if (! strcmp (&opt[2], "ugly-assumed"))
	ffe_set_is_ugly_assumed (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-assumed"))
	ffe_set_is_ugly_assumed (FALSE);
      else if (! strcmp (&opt[2], "ugly-comma"))
	ffe_set_is_ugly_comma (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-comma"))
	ffe_set_is_ugly_comma (FALSE);
      else if (! strcmp (&opt[2], "ugly-complex"))
	ffe_set_is_ugly_complex (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-complex"))
	ffe_set_is_ugly_complex (FALSE);
      else if (! strcmp (&opt[2], "ugly-init"))
	ffe_set_is_ugly_init (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-init"))
	ffe_set_is_ugly_init (FALSE);
      else if (! strcmp (&opt[2], "ugly-logint"))
	ffe_set_is_ugly_logint (TRUE);
      else if (! strcmp (&opt[2], "no-ugly-logint"))
	ffe_set_is_ugly_logint (FALSE);
      else if (! strcmp (&opt[2], "xyzzy"))
	ffe_set_is_ffedebug (TRUE);
      else if (! strcmp (&opt[2], "no-xyzzy"))
	ffe_set_is_ffedebug (FALSE);
      else if (! strcmp (&opt[2], "init-local-zero"))
	ffe_set_is_init_local_zero (TRUE);
      else if (! strcmp (&opt[2], "no-init-local-zero"))
	ffe_set_is_init_local_zero (FALSE);
      else if (! strcmp (&opt[2], "emulate-complex"))
	ffe_set_is_emulate_complex (TRUE);
      else if (! strcmp (&opt[2], "no-emulate-complex"))
	ffe_set_is_emulate_complex (FALSE);
      else if (! strcmp (&opt[2], "backslash"))
	ffe_set_is_backslash (TRUE);
      else if (! strcmp (&opt[2], "no-backslash"))
	ffe_set_is_backslash (FALSE);
      else if (! strcmp (&opt[2], "underscoring"))
	ffe_set_is_underscoring (TRUE);
      else if (! strcmp (&opt[2], "no-underscoring"))
	ffe_set_is_underscoring (FALSE);
      else if (! strcmp (&opt[2], "second-underscore"))
	ffe_set_is_second_underscore (TRUE);
      else if (! strcmp (&opt[2], "no-second-underscore"))
	ffe_set_is_second_underscore (FALSE);
      else if (! strcmp (&opt[2], "zeros"))
	ffe_set_is_zeros (TRUE);
      else if (! strcmp (&opt[2], "no-zeros"))
	ffe_set_is_zeros (FALSE);
      else if (! strcmp (&opt[2], "debug-kludge"))
	ffe_set_is_debug_kludge (TRUE);
      else if (! strcmp (&opt[2], "no-debug-kludge"))
	ffe_set_is_debug_kludge (FALSE);
      else if (! strcmp (&opt[2], "onetrip"))
	ffe_set_is_onetrip (TRUE);
      else if (! strcmp (&opt[2], "no-onetrip"))
	ffe_set_is_onetrip (FALSE);
      else if (! strcmp (&opt[2], "silent"))
	ffe_set_is_silent (TRUE);
      else if (! strcmp (&opt[2], "no-silent"))
	...
	ffe_set_is_globals (FALSE);
      else if (! strcmp (&opt[2], "bounds-check"))
	ffe_set_is_subscript_check (TRUE);
      else if (! strcmp (&opt[2], "no-bounds-check"))
	ffe_set_is_subscript_check (FALSE);
      else if (! strcmp (&opt[2], "fortran-bounds-check"))
	ffe_set_is_subscript_check (TRUE);
      else if (! strcmp (&opt[2], "no-fortran-bounds-check"))
	ffe_set_is_subscript_check (FALSE);
      else if (! strcmp (&opt[2], "typeless-boz"))
	ffe_set_is_typeless_boz (TRUE);
      else if (! strcmp (&opt[2], "no-typeless-boz"))
	ffe_set_is_typeless_boz (FALSE);
      else if (! strcmp (&opt[2], "intrin-case-initcap"))
	ffe_set_case_intrin (FFE_caseINITCAP);
      else if (! strcmp (&opt[2], "intrin-case-upper"))
	ffe_set_case_intrin (FFE_caseUPPER);
      else if (! strcmp (&opt[2], "intrin-case-lower"))
	ffe_set_case_intrin (FFE_caseLOWER);
      else if (! strcmp (&opt[2], "intrin-case-any"))
	ffe_set_case_intrin (FFE_caseNONE);
      else if (! strcmp (&opt[2], "match-case-initcap"))
	ffe_set_case_match (FFE_caseINITCAP);
      else if (! strcmp (&opt[2], "match-case-upper"))
	ffe_set_case_match (FFE_caseUPPER);
      else if (! strcmp (&opt[2], "match-case-lower"))
	ffe_set_case_match (FFE_caseLOWER);
      else if (! strcmp (&opt[2], "match-case-any"))
	ffe_set_case_match (FFE_caseNONE);
      else if (! strcmp (&opt[2], "source-case-upper"))
	ffe_set_case_source (FFE_caseUPPER);
      else if (! strcmp (&opt[2], "source-case-lower"))
	ffe_set_case_source (FFE_caseLOWER);
      else if (! strcmp (&opt[2], "source-case-preserve"))
	ffe_set_case_source (FFE_caseNONE);
      else if (! strcmp (&opt[2], "symbol-case-initcap"))
	ffe_set_case_symbol (FFE_caseINITCAP);
      else if (! strcmp (&opt[2], "symbol-case-upper"))
	ffe_set_case_symbol (FFE_caseUPPER);
      else if (! strcmp (&opt[2], "symbol-case-lower"))
	ffe_set_case_symbol (FFE_caseLOWER);
      else if (! strcmp (&opt[2], "symbol-case-any"))
	ffe_set_case_symbol (FFE_caseNONE);
      else if (! strcmp (&opt[2], "case-strict-upper"))
	{
	  ffe_set_case_intrin (FFE_caseUPPER);
	  ffe_set_case_match (FFE_caseUPPER);
	  ffe_set_case_source (FFE_caseNONE);
      ...
      else if (! strcmp (&opt[2], "case-strict-lower"))
	{
	  ffe_set_case_intrin (FFE_caseLOWER);
	  ffe_set_case_match (FFE_caseLOWER);
	  ffe_set_case_source (FFE_caseNONE);
      ...
      else if (! strcmp (&opt[2], "case-initcap"))
	{
	  ffe_set_case_intrin (FFE_caseINITCAP);
	  ffe_set_case_match (FFE_caseINITCAP);
	  ffe_set_case_source (FFE_caseNONE);
      ...
      else if (! strcmp (&opt[2], "case-upper"))
	{
	  ffe_set_case_intrin (FFE_caseNONE);
	  ffe_set_case_match (FFE_caseNONE);
	  ffe_set_case_source (FFE_caseUPPER);
      ...
      else if (! strcmp (&opt[2], "case-lower"))
	{
	  ffe_set_case_intrin (FFE_caseNONE);
	  ffe_set_case_match (FFE_caseNONE);
	  ffe_set_case_source (FFE_caseLOWER);
      ...
      else if (! strcmp (&opt[2], "case-preserve"))
	{
	  ffe_set_case_intrin (FFE_caseNONE);
	  ffe_set_case_match (FFE_caseNONE);
	  ffe_set_case_source (FFE_caseNONE);
...
/* Runs the lexer on a source file (not an INCLUDEd file).  */

void
ffe_file (ffewhereFile wf, FILE *f)
{
  ffewhere_file_set (wf, ffelex_line_number (), TRUE, 0);
  ffelex_run (wf, f, ffe_is_free_form_);
}

/* Initialize this module.  Initializes modules it calls before callers.  */
...
/* ~~~~~ These need to go somewhere on a per-program-unit basis.  */
