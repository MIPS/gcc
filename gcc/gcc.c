...
/* Basename of dump and aux outputs, computed from dumpbase (given or
   ...
   al.  */
static char *outbase;
static int dumpdir_length = 0, outbase_length = 0;
  ...
  */
  ...
  /* If dumpbase was not brought into dumpdir but we're linking, bring
      ...
      else
	{
      ...
      /* We wish to add this basename to the -dumpdir passed through
	 GCC_COLLECT_OPTIONS, say for LTO linking, unless we combine
	 it with the dumpdir, but we do NOT wish to add it to e.g. %b,
	 so we keep outbase_length as zero.  */
      gcc_assert (!outbase);
      outbase_length = 0;
  ...
  if (dumpdir || outbase)
      ...
      if (dumpdir)
	{
	  ...
	  if (dumpdir_trailing_dash_added)
	    {
	      gcc_checking_assert (!outbase);
	      append_dot = true;
	      gcc_checking_assert (e > q);
	      e--;
	      gcc_checking_assert (*e == '-');
	    }
	  while ((p = strchr (q, '\'')) && p < e)
	    {
	      obstack_grow (&collect_obstack, q, p - q);
	      obstack_grow (&collect_obstack, "'\\''", 4);
	      q = ++p;
	    }
	  obstack_grow (&collect_obstack, q, e - q);
	}
	...
	{
	  q = outbase;
	  e = q + (dumpbase && !*dumpbase ? strlen (outbase) : outbase_length);
	  while ((p = strchr (q, '\'')) && p < e)
	    {
	      obstack_grow (&collect_obstack, q, p - q);
	      obstack_grow (&collect_obstack, "'\\''", 4);
	  ...
	  obstack_grow (&collect_obstack, q, e - q);
	  gcc_checking_assert (!append_dot);
	  append_dot = true;
	}
	  ...
	  case 'b':
	    if (!this_is_output_file && dumpdir_length)
	      obstack_grow (&obstack, dumpdir, dumpdir_length
			    + (outbase_length < 0 ? outbase_length : 0));
	    if (this_is_output_file || !outbase_length)
	      obstack_grow (&obstack, input_basename, basename_length);
	    else if (outbase_length > 0)
		...
		if (save_temps_flag)
		  {
		    ...
		    if (!outbase_length)
		      ...
		      {
			int adjust = -outbase_length;
			if (adjust > suffix_length)
			  adjust = suffix_length;
			suffix += adjust;
			suffix_length -= adjust;
		      }
  ...
  if (dumpbase && *dumpbase)
    {
      ...
      gcc_checking_assert (outbase_length > 0);
