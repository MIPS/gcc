...
struct write_types_data
{
  ...
  const int   marker_routine_function_p;
			  ...
			  const struct write_types_data *wtd)
{
  ...
  if (chain_next == NULL)
      ...
      if (chain_prev != NULL)
	{
	  ...
	  oprintf (d.of, "        %s%s (xprev",
		   (wtd->marker_routine_function_p ? "(void) " : ""),
		   wtd->marker_routine);
	  if (wtd->param_prefix)
	    {
...
static const struct write_types_data ggc_wtd =
  {
    "ggc_m", NULL, "ggc_mark", "ggc_test_and_set_mark", " (void *x_p)", 1,
    NULL, write_types_process_struct, prewrite_structure, "void", NULL, "", 0,
...
static const struct write_types_data typed_ggc_wtd =
  {
    ...
    " struct GC_ms_entry * mark_stack_limit)", 0,
...
static const struct write_types_data pch_wtd =
  {
    ...
    " (void *x_p)\n", 0, "gt_pch_note_reorder", write_types_process_struct,
...
/* Write out the 'enum' definition for gt_types_enum.  */
  ...
  write_gt_types_enum (type_p structures, type_p param_structs)
  {
    ...
    oprintf (header_file, " gt_types_enum_atomic,\n");
...
write_typed_markers (type_p structures)
{
  ...
  for (s = param_structs; s; s = s->next)
	...
	if (stru->u.s.line.file == NULL)
	  continue;
	fn = find_source_for_param_struct (param, stru->u.s.line.file);
