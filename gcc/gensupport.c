...
static void process_include PARAMS ((const char *, int, int));
...
static void
process_include (filename, lineno, relative)
     const char *filename;
     int lineno;
     int relative;
{
  ...
  if (! IS_ABSOLUTE_PATHNAME (filename) && relative)
    {
     ...
     int lineno;
{
  switch (GET_CODE (desc))
    ...
    case INCLUDE:
      process_include (XSTR (desc, 0), lineno, 1);
      break;
     ...
     const char *filename, *midfilename;
{
  ...
  process_include (midfilename, -1, 0);
  /* Read the entire file.  */
  while (1)
    {
      rtx desc;
