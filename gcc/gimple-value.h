...
integer_cst_desc::nunits () const
{ return node.base.u.int_length.unextended; }

inline unsigned char 
integer_cst_desc::ext_nunits () const
{ return node.base.u.int_length.extended; }

inline unsigned char 
integer_cst_desc::offset_nunits () const
{ return node.base.u.int_length.offset; }

inline void
integer_cst_desc::check_index (unsigned i) const
{
  if (i >= ext_nunits ())
    tree_int_cst_elt_check_failed (i, ext_nunits (), __FILE__, __LINE__,
				    __FUNCTION__);
}
...
{ 
  /* Is this possible? */
  check_index (0);
  return node.int_cst.val[0];
}

inline unsigned HOST_WIDE_INT 
integer_cst_desc::operator[] (unsigned i) const
{ 
  check_index (i);
  return node.int_cst.val[i];
}

inline const HOST_WIDE_INT *
integer_cst_desc::addr (unsigned i) const
{ 
  check_index (i);
  return &(node.int_cst.val[i]);
}
