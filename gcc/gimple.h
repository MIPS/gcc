...
/* Reset all the memory operand vectors in STMT.  Note that this makes
   no attempt at freeing the existing vectors, it simply clears them.
   It is meant to be used when reading GIMPLE from a file.  */

static inline void
gimple_reset_mem_ops (gimple stmt)
{
  ...
  stmt->gsops.opbase.def_ops = NULL;
  stmt->gsops.opbase.use_ops = NULL;

  if (gimple_has_mem_ops (stmt))
    {
      stmt->gsmem.membase.vdef_ops = NULL;
      stmt->gsmem.membase.vuse_ops = NULL;
      stmt->gsmem.membase.stores = NULL;
      stmt->gsmem.membase.loads = NULL;
    }
}
