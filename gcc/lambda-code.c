...
lambda_loopnest
lambda_loopnest_new (int depth, int invariants, struct obstack *lambda_obstack)
{
  lambda_loopnest ret;
  ret = (lambda_loopnest)obstack_alloc (lambda_obstack, sizeof (*ret));

  LN_LOOPS (ret) = (lambda_loop *)obstack_alloc (lambda_obstack,
                                                 sizeof (struct lambda_loop_s));
  memset (LN_LOOPS (ret), 0, sizeof (struct lambda_loop_s));
  LN_DEPTH (ret) = depth;
...
{
  lambda_lattice ret = (lambda_lattice)obstack_alloc (lambda_obstack,
                                                      sizeof (*ret));
