		 ...
		 enum constraint_num constraint)
{
  ...
  /* Allow memory OP if it matches CONSTRAINT, even if CONSTRAINT is more
     forgiving than "m".  */
  if (MEM_P (op)
		...
		case '5':  case '6':  case '7':	 case '8':  case '9':
		  {
			...
			if ((REG_P (*curr_id->operand_loc[nop])
				...
				|| SUBREG_P (*curr_id->operand_loc[m])))
			  {
				...
				&& REG_USERVAR_P (m_reg)
				/* Branch-specific, should drop in
				   favor of asm tests that rely on the
				   parameter mapping.  */
      ...
      for (nop = 0; nop < early_clobbered_regs_num; nop++)
	{
	  ...
	  for (j = 0; j < n_operands; j++)
		...
		if (HARD_REGISTER_P (operand_reg[i])
		    ...
		    /* Branch-specific, should drop in favor of asm tests
		       that rely on the parameter mapping.  */
		    && INSN_CODE (curr_insn) < 0)
