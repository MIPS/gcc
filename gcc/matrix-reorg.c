	  ...
	  if (mi->dim_map[k] == min_escape_l - 1)
	      ...
	      else
		{
		  ...
		  if (x != -1 && y != -1)
			...
			{
			  tree log = size_int (x - y);
			  new_offset =
			    fold_build2 (LSHIFT_EXPR, sizetype,
					 fold_convert (sizetype, offset), log);
			}
		    }
		  else
		    {
		      ratio =
			fold_build2 (TRUNC_DIV_EXPR, sizetype, d_type_size,
				     d_type_size_k);
		      new_offset =
			fold_build2 (MULT_EXPR, sizetype, offset, ratio);
		    }
