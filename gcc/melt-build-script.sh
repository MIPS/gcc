...
#@#@#
#@#@# It has been AutoGen-ed  August 27, 2014 at 04:18:07 PM by AutoGen 5.18.3
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
   ...
   #@ melt-build-script.tpl:990/819 runtime stamp
    ...
    echo  ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:1006/824" -DGCCMELT_JUSTCOUNT' > $meltjustcount_argstemp
    ...
    [ -f "$meltjustcount_args" ] || meltbuild_error  melt-build-script.tpl:1020/825 missing check justcount args  "$meltjustcount_args"
   meltbuild_info melt-build-script.tpl:1021/826 $meltjustcount_args  is
   cat $meltjustcount_args < /dev/null >&2
   echo
   $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltjustcount_args \
	|| meltbuild_error melt-build-script.tpl:1025/827 running justcount failed with arguments @$meltjustcount_args
   meltbuild_info melt-build-script.tpl:1026/828 done check justcount with $meltjustcount_args
   echo

   $GCCMELT_MOVE_IF_CHANGE  $meltcheckruntime_stamptemp $meltcheckruntime_stamp
   meltbuild_info melt-build-script.tpl:1030/829 done check runtime  $meltcheckruntime_stamp
    
else
    meltbuild_info melt-build-script.tpl:1033/830 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:1037/831 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi

################################################################
#@ melt-build-script.tpl:1042/832
if [ "$melt_overall_goal" = "applications" ]; then
    meltbuild_info melt-build-script.tpl:1044/833 done applications overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi

################################################################
################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:1050/834
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/libmelt-ana-gimple.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:1054/835 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:1056/836"' > $meltgen_args
...
meltbuild_arg "module-makefile=\"$GCCMELT_MODULE_MK\""  >>  $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:1070/837  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null >&2
   $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:1073/838  "$GCCMELT_CC1PLUS" failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:1075/839 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:1079/840 successfully done with times at `date '+%x %H:%M:%S'`: ; times >&2

################################################################
#@ melt-build-script.tpl:1082/841
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:1084/842 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:1088/843 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:1089/844 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:1090/845 end of generated melt-build-script.sh
