...
#@#@#
#@#@# It has been AutoGen-ed  October  2, 2013 at 11:25:39 AM by AutoGen 5.18
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
    ...
    #@ melt-build-script.tpl:730/653 checkruntime
    if [ -f melt-no-check-runtime -o -n "$MELTGCC_NO_CHECK_RUNTIME" -o ! -f melt-runtime.i ]; then
	meltbuild_info melt-build-script.tpl:732/654 skipping check of MELT runtime
    else
	meltcheckruntime_args=meltbuild-checkruntime.args 
	meltcheckruntime_argstemp=$meltcheckruntime_args-tmp$$
	echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:736/655"' ' -DGCCMELT_CHECKMELTRUNTIME' > $meltcheckruntime_argstemp
	...
	[ -f "$meltcheckruntime_args" ] || meltbuild_error  melt-build-script.tpl:747/656 missing check runtime args  "$meltcheckruntime_args"
	meltbuild_info melt-build-script.tpl:748/657 $meltcheckruntime_args  is
	...
	if [ -n "$MELTGCCBUILTIN_BUILD_WITH_CXX" ]; then
		...
		|| meltbuild_error melt-build-script.tpl:752/658 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	else
	    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltcheckruntime_args \
		|| meltbuild_error melt-build-script.tpl:755/659 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	fi
	meltbuild_info melt-build-script.tpl:757/660 done check runtime with $meltcheckruntime_args
    fi

   #@ melt-build-script.tpl:760/661 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:763/662"' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:777/663 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:778/664 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null >&2
    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:781/665 running helloworld failed with arguments @$meltcheckhelloworld_args
   meltbuild_info melt-build-script.tpl:782/666 done check helloworld with $meltcheckhelloworld_args
   #@ melt-build-script.tpl:783/667 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_C" ] || meltbuild_error melt-build-script.tpl:785/668 missing MELT runtime C file $GCCMELT_RUNTIME_C
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:790/669 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_application_stamp" ] || meltbuild_error melt-build-script.tpl:791/670 missing final application stamp "$melt_final_application_stamp"
    ...
    meltbuild_info melt-build-script.tpl:795/671 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:797/672 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:801/673 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi
################################################################
################################################################
#@ melt-build-script.tpl:806/674
if [ "$melt_overall_goal" = "regenerate" ]; then
    meltbuild_notice regenerating runtime support melt-build-script.tpl:808/675 
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:812/676"' > $meltregen_argstemp
   ...
   meltbuild_info melt-build-script.tpl:822/677 $meltregen_args  is
   cat $meltregen_args < /dev/null >&2
    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltregen_args \
	|| meltbuild_error melt-build-script.tpl:825/678 failed with arguments @$meltregen_args
    meltbuild_info melt-build-script.tpl:826/679 done regenerate overall goal
    exit 0
fi

################################################################
################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:832/680
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/xtramelt-playground.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:836/681 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:838/682"' > $meltgen_args
...
meltbuild_arg "module-makefile=\"$GCCMELT_MODULE_MK\""  >>  $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:852/683  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null >&2
   $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:855/684 failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:857/685 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:861/686 successfully done with times at `date '+%x %H:%M:%S'`: ; times >&2

################################################################
#@ melt-build-script.tpl:864/687
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:866/688 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:870/689 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:871/690 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:872/691 end of generated melt-build-script.sh
