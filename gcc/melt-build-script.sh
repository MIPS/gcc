...
#@#@#
#@#@# It has been AutoGen-ed  August 27, 2014 at 07:36:42 PM by AutoGen 5.18.3
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
	...
	[ -f "$meltcheckruntime_args" ] || meltbuild_error  melt-build-script.tpl:954/808 missing check runtime args  "$meltcheckruntime_args"
	meltbuild_info melt-build-script.tpl:955/809 $meltcheckruntime_args  is
	...
	if [ -n "$MELTGCCBUILTIN_BUILD_WITH_CXX" ]; then
		...
		|| meltbuild_error melt-build-script.tpl:959/810 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	else
	    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltcheckruntime_args \
		|| meltbuild_error melt-build-script.tpl:962/811 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	fi
	meltbuild_info melt-build-script.tpl:964/812 done check runtime with $meltcheckruntime_args
    fi

   #@ melt-build-script.tpl:967/813 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:970/814" -DGCCMELT_CHECKHELLO' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:984/815 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:985/816 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null >&2
    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:988/817 running helloworld failed with arguments @$meltcheckhelloworld_args
   meltbuild_info melt-build-script.tpl:989/818 done check helloworld with $meltcheckhelloworld_args
   #@ melt-build-script.tpl:990/819 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_CC" ] || meltbuild_error melt-build-script.tpl:992/820 missing MELT runtime C++ file $GCCMELT_RUNTIME_CC
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:997/821 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_library_stamp" ] || meltbuild_error melt-build-script.tpl:998/822 missing final library stamp "$melt_final_library_stamp"
    ...
    echo
    #@ melt-build-script.tpl:1003/823 justcount
    meltjustcount_args=meltbuild-justcount.args
    meltjustcount_argstemp=$meltjustcount_args-tmp$$
    echo  ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:1006/824" -DGCCMELT_JUSTCOUNT -fexceptions melt-runtime.i ' > $meltjustcount_argstemp
    ...
    [ -f "$meltjustcount_args" ] || meltbuild_error  melt-build-script.tpl:1017/825 missing check justcount args  "$meltjustcount_args"
   meltbuild_info melt-build-script.tpl:1018/826 $meltjustcount_args  is
   cat $meltjustcount_args < /dev/null >&2
   echo
   $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltjustcount_args \
	|| meltbuild_error melt-build-script.tpl:1022/827 running justcount failed with arguments @$meltjustcount_args
   meltbuild_info melt-build-script.tpl:1023/828 done check justcount with $meltjustcount_args
   echo

   $GCCMELT_MOVE_IF_CHANGE  $meltcheckruntime_stamptemp $meltcheckruntime_stamp
   meltbuild_info melt-build-script.tpl:1027/829 done check runtime  $meltcheckruntime_stamp
    
else
    meltbuild_info melt-build-script.tpl:1030/830 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:1034/831 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi

################################################################
#@ melt-build-script.tpl:1039/832
if [ "$melt_overall_goal" = "applications" ]; then
    meltbuild_info melt-build-script.tpl:1041/833 done applications overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi

################################################################
################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:1047/834
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/libmelt-ana-gimple.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:1051/835 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:1053/836"' > $meltgen_args
...
meltbuild_arg "module-makefile=\"$GCCMELT_MODULE_MK\""  >>  $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:1067/837  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null >&2
   $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:1070/838  "$GCCMELT_CC1PLUS" failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:1072/839 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:1076/840 successfully done with times at `date '+%x %H:%M:%S'`: ; times >&2

################################################################
#@ melt-build-script.tpl:1079/841
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:1081/842 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:1085/843 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:1086/844 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:1087/845 end of generated melt-build-script.sh
