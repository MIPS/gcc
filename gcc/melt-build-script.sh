...
#@#@# 
#@#@# It has been AutoGen-ed  June  4, 2013 at 05:55:12 PM by AutoGen 5.12
...
meltbuild_info melt-build-script.tpl:703/644 $meltcommon_args is
...
################################################################
#@ melt-build-script.tpl:710/645 runtime self check

##  melt-build-script.tpl:712/646 FIXME: should skip that when cross-compiler MELT plugin..
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
    ...
    #@ melt-build-script.tpl:718/647 checkruntime
    if [ -f melt-no-check-runtime -o -n "$MELTGCC_NO_CHECK_RUNTIME" -o ! -f melt-runtime.i ]; then
	meltbuild_info melt-build-script.tpl:720/648 skipping check of MELT runtime
    else
	meltcheckruntime_args=meltbuild-checkruntime.args 
	meltcheckruntime_argstemp=$meltcheckruntime_args-tmp$$
	echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:724/649"' > $meltcheckruntime_argstemp
	...
	[ -f "$meltcheckruntime_args" ] || meltbuild_error  melt-build-script.tpl:735/650 missing check runtime args  "$meltcheckruntime_args"
	meltbuild_info melt-build-script.tpl:736/651 $meltcheckruntime_args  is
	...
	if [ -n "$MELTGCCBUILTIN_BUILD_WITH_CXX" ]; then
		...
		|| meltbuild_error melt-build-script.tpl:740/652 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	else
	    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckruntime_args \
		|| meltbuild_error melt-build-script.tpl:743/653 failed  $GCCMELT_CC1 with arguments @$meltcheckruntime_args
	fi
	meltbuild_info melt-build-script.tpl:745/654 done check runtime with $meltcheckruntime_args
    fi

   #@ melt-build-script.tpl:748/655 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:751/656"' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:765/657 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:766/658 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null >&2
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:769/659 running helloworld failed with arguments @$meltcheckhelloworld_args
   meltbuild_info melt-build-script.tpl:770/660 done check helloworld with $meltcheckhelloworld_args
   #@ melt-build-script.tpl:771/661 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_C" ] || meltbuild_error melt-build-script.tpl:773/662 missing MELT runtime C file $GCCMELT_RUNTIME_C
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:778/663 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_application_stamp" ] || meltbuild_error melt-build-script.tpl:779/664 missing final application stamp "$melt_final_application_stamp"
    ...
    meltbuild_info melt-build-script.tpl:783/665 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:785/666 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:789/667 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi
################################################################
################################################################
#@ melt-build-script.tpl:794/668
if [ "$melt_overall_goal" = "regenerate" ]; then
    meltbuild_notice regenerating runtime support melt-build-script.tpl:796/669 
    ...
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:800/670"' > $meltregen_argstemp
   ...
   meltbuild_info melt-build-script.tpl:810/671 $meltregen_args  is
   cat $meltregen_args < /dev/null >&2
    $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @$meltregen_args \
	|| meltbuild_error melt-build-script.tpl:813/672 failed with arguments @$meltregen_args
    meltbuild_info melt-build-script.tpl:814/673 done regenerate overall goal
    exit 0
fi

################################################################
################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:820/674
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/xtramelt-opengpu.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:824/675 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:826/676"' > $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:838/677  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null >&2
   $GCCMELT_CC1_PREFIX $GCCMELT_CC1 @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:841/678 failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:843/679 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:847/680 successfully done with times at `date '+%x %H:%M:%S'`: ; times >&2

################################################################
#@ melt-build-script.tpl:850/681
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:852/682 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:856/683 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:857/684 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:858/685 end of generated melt-build-script.sh
