...
#@#@#
#@#@# It has been AutoGen-ed  April  2, 2014 at 04:17:13 PM by AutoGen 5.18.1
...
if [ ! -f $meltcheckruntime_stamp -o $meltcheckruntime_stamp -ot "$GCCMELT_RUNTIME_ARGS" \
	...
	[ -f "$meltcheckruntime_args" ] || meltbuild_error  melt-build-script.tpl:919/791 missing check runtime args  "$meltcheckruntime_args"
	meltbuild_info melt-build-script.tpl:920/792 $meltcheckruntime_args  is
	...
	if [ -n "$MELTGCCBUILTIN_BUILD_WITH_CXX" ]; then
		...
		|| meltbuild_error melt-build-script.tpl:924/793 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	else
	    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltcheckruntime_args \
		|| meltbuild_error melt-build-script.tpl:927/794 failed  $GCCMELT_CC1PLUS with arguments @$meltcheckruntime_args
	fi
	meltbuild_info melt-build-script.tpl:929/795 done check runtime with $meltcheckruntime_args
    fi

   #@ melt-build-script.tpl:932/796 checkhello
    meltcheckhelloworld_args=meltbuild-checkhelloworld.args 
    meltcheckhelloworld_argstemp=$meltcheckhelloworld_args-tmp$$
    echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:935/797"' > $meltcheckhelloworld_argstemp
    ...
    [ -f "$meltcheckhelloworld_args" ] || meltbuild_error  melt-build-script.tpl:949/798 missing check helloworld args  "$meltcheckhelloworld_args"
   meltbuild_info melt-build-script.tpl:950/799 $meltcheckhelloworld_args  is
   cat $meltcheckhelloworld_args < /dev/null >&2
    $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @$meltcheckhelloworld_args \
	|| meltbuild_error melt-build-script.tpl:953/800 running helloworld failed with arguments @$meltcheckhelloworld_args
   meltbuild_info melt-build-script.tpl:954/801 done check helloworld with $meltcheckhelloworld_args
   #@ melt-build-script.tpl:955/802 runtime stamp
    meltcheckruntime_stamptemp=$meltcheckruntime_stamp-tmp$$
    [ -f "$GCCMELT_RUNTIME_CC" ] || meltbuild_error melt-build-script.tpl:957/803 missing MELT runtime C++ file $GCCMELT_RUNTIME_CC
    ...
    [ -f "$melt_final_translator_stamp" ] || meltbuild_error melt-build-script.tpl:962/804 missing final translator stamp "$melt_final_translator_stamp"
    [ -f "$melt_final_library_stamp" ] || meltbuild_error melt-build-script.tpl:963/805 missing final library stamp "$melt_final_library_stamp"
    ...
    meltbuild_info melt-build-script.tpl:967/806 done check runtime  $meltcheckruntime_stamp
else
    meltbuild_info melt-build-script.tpl:969/807 keeping runtime checks  $meltcheckruntime_stamp
fi

if [ "$melt_overall_goal" = "checkruntime" ]; then
    meltbuild_info melt-build-script.tpl:973/808 done checkruntime overall goal with stamp  $meltcheckruntime_stamp
    exit 0
fi

################################################################
################################################################
### the generated documentation meltgendoc.texi melt-build-script.tpl:979/809
...
if [   ! -f meltgendoc.texi  -o meltbuild-sources/warmelt-first.melt -nt meltgendoc.texi  \
 ...
 -o meltbuild-sources/libmelt-ana-gimple.melt -nt meltgendoc.texi   ]; then
   meltbuild_info melt-build-script.tpl:983/810 generating meltgendoc.texi
   meltgen_args=meltbuild-gendoc.args-tmp$$
   echo ' -DGCCMELT_FROM_ARG="melt-build-script.tpl:985/811"' > $meltgen_args
...
meltbuild_arg "module-makefile=\"$GCCMELT_MODULE_MK\""  >>  $meltgen_args
   ...
   meltbuild_info melt-build-script.tpl:999/812  meltbuild-gendoc.args is
   cat meltbuild-gendoc.args < /dev/null >&2
   $GCCMELT_CC1PLUS_PREFIX $GCCMELT_CC1PLUS @meltbuild-gendoc.args \
     || meltbuild_error melt-build-script.tpl:1002/813  "$GCCMELT_CC1PLUS" failed with arguments @meltbuild-gendoc.args
else
   meltbuild_info melt-build-script.tpl:1004/814 keeping meltgendoc.texi
fi

################
meltbuild_info melt-build-script.tpl:1008/815 successfully done with times at `date '+%x %H:%M:%S'`: ; times >&2

################################################################
#@ melt-build-script.tpl:1011/816
if [ "$melt_overall_goal" = "gendoc" ]; then
    meltbuild_info melt-build-script.tpl:1013/817 done gendoc overall goal with stamp  $melt_final_translator_stamp
    exit 0
fi

## #@ melt-build-script.tpl:1017/818 if we get here something is wrong in this script
meltbuild_error  melt-build-script.tpl:1018/819 unexpected MELT overall goal "$melt_overall_goal" buggy melt-build-script.tpl
#@ eof melt-build-script.tpl:1019/820 end of generated melt-build-script.sh
