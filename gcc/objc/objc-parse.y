...
/* Reserved words that qualify type:
   "const", "volatile", "restrict", "__bounded" or "__unbounded".
   yylval contains an IDENTIFIER_NODE which indicates which one.  */
%token TYPE_QUAL
...
%type <itype> setspecs
%type <itype> boundedness_qual boundedness_qual_opt
	...
	| ANDAND identifier
		{ tree label = lookup_label ($2);
		  ...
		  if (label == 0)
		    ...
		    {
		      TREE_USED (label) = 1;
		      $$ = build1 (ADDR_EXPR, unbounded_ptr_type_node, label);
		      TREE_CONSTANT ($$) = 1;
		    }
		}
			  ...
			  if (objc_method_context
			    ...
			    if (TREE_CODE ($$) != FUNCTION_DECL)
			      error ("`%s' implicitly declared as function",
				     ...
				     && (TREE_TYPE (TREE_TYPE ($$))
					 != void_type_node)
				     && ! BOUNDED_POINTER_TYPE_P (TREE_TYPE (TREE_TYPE ($$))))
...
old_style_parm_decls:
	/* empty */
	| datadecls
	| datadecls ELLIPSIS boundedness_qual_opt
...
boundedness_qual:
	  TYPE_QUAL
		{
		  if ($1 == ridpointers[(int) RID_BOUNDED])
		    $$ = 1;
		  else if ($1 == ridpointers[(int) RID_UNBOUNDED])
		    $$ = 0;
		  else
		    {
...
parmlist_2:  /* empty */
	...
	| parms
		{ $$ = get_parm_info (1, -1); }
	| parms ',' ELLIPSIS boundedness_qual_opt
		{ $$ = get_parm_info (0, $4); }
	;
