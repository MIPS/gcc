...
#define NEXT_PASS(PASS)  (p = next_pass_1 (p, &PASS))
  ...
  NEXT_PASS (pass_ipa_reorg_structs);
  *p = NULL;
...
execute_todo (unsigned int flags)
{
  ...
  if ((curr_properties & PROP_ssa) && !(flags & TODO_skip_verify_ssa))
...
execute_ipa_pass_list (struct tree_opt_pass *pass)
{
  do
      ...
      if (execute_one_pass (pass, true) && pass->sub)
	{
	  ...
	  int old_curr_properties = curr_properties;
	  for (node = cgraph_nodes; node; node = node->next)
	    if (node->analyzed)
	      {
		push_cfun (DECL_STRUCT_FUNCTION (node->decl));
		curr_properties = old_curr_properties;
