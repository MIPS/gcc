...
get_free_reg (HARD_REG_SET dont_begin_colors, HARD_REG_SET free_colors,
	      enum machine_mode mode, int allow_unaligned)
{
  int c;
...
static int
proper_hard_reg_subset_p (HARD_REG_SET s1, HARD_REG_SET s2)
{
  HARD_REG_SET s;
...
static int
enough_free_p (struct web *web1, struct web *web2)
{
...
int
non_conflicting_for_combine (struct web *w1, struct web *w2)
{
  struct web *ow2;
