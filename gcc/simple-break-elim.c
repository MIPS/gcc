...
static void break_continue_elimination_rec (tree);
static void break_elimination (tree);
static void continue_stmt_solution2 (tree *);
static void break_stmt_solution2 (tree *);
static void outward_motion (tree, tree, tree,
			    void (*) (tree, tree, tree, tree));
static void update_break_paths (tree, tree, tree, tree);
static void switch_to_if (tree);
static void construct_body_for_case_labels (tree);
static void complete_body_of_case_labels (tree);
static void reorder_case_labels (tree);
static tree cases_to_if (tree, tree);
static enum eval_bool eval_bool_condition (tree);
...
void
break_continue_elimination (tree fn)
{
  tree fn_body;
...
static void
break_continue_elimination_rec (tree node)
{
...
int
chain_contains_stmt_p (tree node, enum tree_code code)
{
  while (node)
    {
...
void
insert_andif_in_condition (tree condition, tree parent)
{
...
void
insert_orif_in_condition (tree condition, tree parent)
{
  switch (TREE_CODE (parent))
    {
    case FOR_STMT:
...
static void
continue_stmt_solution2 (tree *stmt)
{
  int one_if = 0;
...
static void
break_stmt_solution2 (tree *stmt)
{
  int one_if = 0;
  PATH_TYPE related_parent;
  tree related_loop;
...
void
flow_out_of (PATH_TYPE outer, PATH_TYPE inner, tree last_to_be_executed,
	     tree lvar, void (*update_paths_fnct) (tree, tree, tree, tree))
{
  do {
    /* Remove the FOR_EXPR, since when we break a loop we don't want to
...
static void
outward_motion (tree parent, tree last_executed, tree lvar,
...
static void
update_break_paths (tree st1, tree st2, tree begin_stmt, tree end_stmt)
{
}
...
static void
construct_body_for_case_labels (tree node)
{
...
static void
complete_body_of_case_labels (tree node)
{
  LAST_CASE (node);
...
static tree
cases_to_if (tree svar, tree node)
{
...
void
eliminate_compound_stmts (tree stmt)
{
