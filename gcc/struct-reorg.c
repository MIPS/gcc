			...
			if (old_type == cur_struct->struct_data->decl)
			  {
				...
				new_malloc_tree = build (MODIFY_EXPR, 
							 TREE_TYPE 
							       (new_var_decl),
							 new_var_decl,
							 build1 (NOP_EXPR, 
				...
				append_to_statement_list (new_malloc_tree, 
							  &new_mallocs_list);
  ...
  */
  ...
  for (cur_struct = data_struct_list; cur_struct;
      ...
      if (struct_data->count > 0)
	{
	  ...
	  type_name = get_type_name (struct_data->decl);
	  
	  if (strcmp (type_name, "f1_neuron") == 0
	      && struct_data->struct_clustering)
	    {
	      struct_data->struct_clustering->direct_access = true;
	      if (split_all_fields)
		struct_data->struct_clustering = build_f1neuron_cluster_info ();
	    }

	  else if (strcmp (type_name, "xyz") == 0
		   && struct_data->struct_clustering)
	    struct_data->struct_clustering->direct_access = true;

	  else if (strcmp (type_name, "FILE") == 0)
	    struct_data->struct_clustering = NULL;
	}
      
      if (struct_data->struct_clustering
  ...
  for (c_node = cgraph_nodes; c_node; c_node = c_node->next)
	  ...
	  || (TREE_PUBLIC (fn_decl)))
	{
	  for (caller = c_node->callers; caller; caller = caller->next_caller)
	    {
	      tree call_expr = caller->call_expr;
