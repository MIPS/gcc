...
void func1(void)
{
  ...
  if (gx > 3) { // { dg-warning "Reading variable 'gx' requires lock 'mu4\\\[1\\\]'" }
    ...
    *p = foo->decrementB(gy); // { dg-warning "Reading variable 'gy' requires lock 'get_lock \\(NON_LVALUE_EXPR <foo>\\)'" }
  }
...
// { dg-warning "Access to memory location pointed to by variable 'p' requires lock 'get_lock \\(NON_LVALUE_EXPR <foo>\\)'" "" { target *-*-* } 50 }
