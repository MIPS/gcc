/* { dg-final { check-function-bodies "**" "" "-DCHECK_ASM" } } */

#include "test_sve_acle.h"

/*
** msb_u32_m_tied1:
**	msb	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_m_tied1, svuint32_t,
		z0 = svmsb_u32_m (p0, z0, z1, z2),
		z0 = svmsb_m (p0, z0, z1, z2))

/*
** msb_u32_m_tied2:
**	mov	(z[0-9]+)\.d, z0\.d
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, \1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_m_tied2, svuint32_t,
		z0 = svmsb_u32_m (p0, z1, z0, z2),
		z0 = svmsb_m (p0, z1, z0, z2))

/*
** msb_u32_m_tied3:
**	mov	(z[0-9]+)\.d, z0\.d
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, z2\.s, \1\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_m_tied3, svuint32_t,
		z0 = svmsb_u32_m (p0, z1, z2, z0),
		z0 = svmsb_m (p0, z1, z2, z0))

/*
** msb_u32_m_untied:
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, z2\.s, z3\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_m_untied, svuint32_t,
		z0 = svmsb_u32_m (p0, z1, z2, z3),
		z0 = svmsb_m (p0, z1, z2, z3))

/*
** msb_w0_u32_m_tied1:
**	mov	(z[0-9]+\.s), w0
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_m_tied1, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_m (p0, z0, z1, x0),
		 z0 = svmsb_m (p0, z0, z1, x0))

/*
** msb_w0_u32_m_tied2:
**	mov	(z[0-9]+\.s), w0
**	mov	(z[0-9]+)\.d, z0\.d
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, \2\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_m_tied2, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_m (p0, z1, z0, x0),
		 z0 = svmsb_m (p0, z1, z0, x0))

/*
** msb_w0_u32_m_untied:
**	mov	(z[0-9]+\.s), w0
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, z2\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_m_untied, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_m (p0, z1, z2, x0),
		 z0 = svmsb_m (p0, z1, z2, x0))

/*
** msb_s4_u32_m_tied1:
**	mov	(z[0-9]+\.s), s4
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_m_tied1, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_m (p0, z0, z1, d4),
		 z0 = svmsb_m (p0, z0, z1, d4))

/*
** msb_s4_u32_m_tied2:
**	mov	(z[0-9]+\.s), s4
**	mov	(z[0-9]+)\.d, z0\.d
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, \2\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_m_tied2, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_m (p0, z1, z0, d4),
		 z0 = svmsb_m (p0, z1, z0, d4))

/*
** msb_s4_u32_m_untied:
**	mov	(z[0-9]+\.s), s4
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, z2\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_m_untied, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_m (p0, z1, z2, d4),
		 z0 = svmsb_m (p0, z1, z2, d4))

/*
** msb_2_u32_m_tied1:
**	mov	(z[0-9]+\.s), #2
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_m_tied1, svuint32_t,
		z0 = svmsb_n_u32_m (p0, z0, z1, 2),
		z0 = svmsb_m (p0, z0, z1, 2))

/*
** msb_2_u32_m_tied2:
**	mov	(z[0-9]+\.s), #2
**	mov	(z[0-9]+)\.d, z0\.d
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, \2\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_m_tied2, svuint32_t,
		z0 = svmsb_n_u32_m (p0, z1, z0, 2),
		z0 = svmsb_m (p0, z1, z0, 2))

/*
** msb_2_u32_m_untied:
**	mov	(z[0-9]+\.s), #2
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, z2\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_m_untied, svuint32_t,
		z0 = svmsb_n_u32_m (p0, z1, z2, 2),
		z0 = svmsb_m (p0, z1, z2, 2))

/*
** msb_u32_z_tied1:
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_z_tied1, svuint32_t,
		z0 = svmsb_u32_z (p0, z0, z1, z2),
		z0 = svmsb_z (p0, z0, z1, z2))

/*
** msb_u32_z_tied2:
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_z_tied2, svuint32_t,
		z0 = svmsb_u32_z (p0, z1, z0, z2),
		z0 = svmsb_z (p0, z1, z0, z2))

/*
** msb_u32_z_tied3:
**	movprfx	z0\.s, p0/z, z0\.s
**	mls	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_z_tied3, svuint32_t,
		z0 = svmsb_u32_z (p0, z1, z2, z0),
		z0 = svmsb_z (p0, z1, z2, z0))

/*
** msb_u32_z_untied:
** (
**	movprfx	z0\.s, p0/z, z1\.s
**	msb	z0\.s, p0/m, z2\.s, z3\.s
** |
**	movprfx	z0\.s, p0/z, z2\.s
**	msb	z0\.s, p0/m, z1\.s, z3\.s
** |
**	movprfx	z0\.s, p0/z, z3\.s
**	mls	z0\.s, p0/m, z1\.s, z2\.s
** )
**	ret
*/
TEST_UNIFORM_Z (msb_u32_z_untied, svuint32_t,
		z0 = svmsb_u32_z (p0, z1, z2, z3),
		z0 = svmsb_z (p0, z1, z2, z3))

/*
** msb_w0_u32_z_tied1:
**	mov	(z[0-9]+\.s), w0
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_z_tied1, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_z (p0, z0, z1, x0),
		 z0 = svmsb_z (p0, z0, z1, x0))

/*
** msb_w0_u32_z_tied2:
**	mov	(z[0-9]+\.s), w0
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_z_tied2, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_z (p0, z1, z0, x0),
		 z0 = svmsb_z (p0, z1, z0, x0))

/*
** msb_w0_u32_z_untied:
**	mov	(z[0-9]+\.s), w0
** (
**	movprfx	z0\.s, p0/z, z1\.s
**	msb	z0\.s, p0/m, z2\.s, \1
** |
**	movprfx	z0\.s, p0/z, z2\.s
**	msb	z0\.s, p0/m, z1\.s, \1
** |
**	movprfx	z0\.s, p0/z, \1
**	mls	z0\.s, p0/m, z1\.s, z2\.s
** )
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_z_untied, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_z (p0, z1, z2, x0),
		 z0 = svmsb_z (p0, z1, z2, x0))

/*
** msb_s4_u32_z_tied1:
**	mov	(z[0-9]+\.s), s4
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_z_tied1, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_z (p0, z0, z1, d4),
		 z0 = svmsb_z (p0, z0, z1, d4))

/*
** msb_s4_u32_z_tied2:
**	mov	(z[0-9]+\.s), s4
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_z_tied2, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_z (p0, z1, z0, d4),
		 z0 = svmsb_z (p0, z1, z0, d4))

/*
** msb_s4_u32_z_untied:
**	mov	(z[0-9]+\.s), s4
** (
**	movprfx	z0\.s, p0/z, z1\.s
**	msb	z0\.s, p0/m, z2\.s, \1
** |
**	movprfx	z0\.s, p0/z, z2\.s
**	msb	z0\.s, p0/m, z1\.s, \1
** |
**	movprfx	z0\.s, p0/z, \1
**	mls	z0\.s, p0/m, z1\.s, z2\.s
** )
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_z_untied, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_z (p0, z1, z2, d4),
		 z0 = svmsb_z (p0, z1, z2, d4))

/*
** msb_2_u32_z_tied1:
**	mov	(z[0-9]+\.s), #2
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_z_tied1, svuint32_t,
		z0 = svmsb_n_u32_z (p0, z0, z1, 2),
		z0 = svmsb_z (p0, z0, z1, 2))

/*
** msb_2_u32_z_tied2:
**	mov	(z[0-9]+\.s), #2
**	movprfx	z0\.s, p0/z, z0\.s
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_z_tied2, svuint32_t,
		z0 = svmsb_n_u32_z (p0, z1, z0, 2),
		z0 = svmsb_z (p0, z1, z0, 2))

/*
** msb_2_u32_z_untied:
**	mov	(z[0-9]+\.s), #2
** (
**	movprfx	z0\.s, p0/z, z1\.s
**	msb	z0\.s, p0/m, z2\.s, \1
** |
**	movprfx	z0\.s, p0/z, z2\.s
**	msb	z0\.s, p0/m, z1\.s, \1
** |
**	movprfx	z0\.s, p0/z, \1
**	mls	z0\.s, p0/m, z1\.s, z2\.s
** )
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_z_untied, svuint32_t,
		z0 = svmsb_n_u32_z (p0, z1, z2, 2),
		z0 = svmsb_z (p0, z1, z2, 2))

/*
** msb_u32_x_tied1:
**	msb	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_x_tied1, svuint32_t,
		z0 = svmsb_u32_x (p0, z0, z1, z2),
		z0 = svmsb_x (p0, z0, z1, z2))

/*
** msb_u32_x_tied2:
**	msb	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_x_tied2, svuint32_t,
		z0 = svmsb_u32_x (p0, z1, z0, z2),
		z0 = svmsb_x (p0, z1, z0, z2))

/*
** msb_u32_x_tied3:
**	mls	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_u32_x_tied3, svuint32_t,
		z0 = svmsb_u32_x (p0, z1, z2, z0),
		z0 = svmsb_x (p0, z1, z2, z0))

/*
** msb_u32_x_untied:
** (
**	movprfx	z0, z1
**	msb	z0\.s, p0/m, z2\.s, z3\.s
** |
**	movprfx	z0, z2
**	msb	z0\.s, p0/m, z1\.s, z3\.s
** |
**	movprfx	z0, z3
**	mls	z0\.s, p0/m, z1\.s, z2\.s
** )
**	ret
*/
TEST_UNIFORM_Z (msb_u32_x_untied, svuint32_t,
		z0 = svmsb_u32_x (p0, z1, z2, z3),
		z0 = svmsb_x (p0, z1, z2, z3))

/*
** msb_w0_u32_x_tied1:
**	mov	(z[0-9]+\.s), w0
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_x_tied1, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_x (p0, z0, z1, x0),
		 z0 = svmsb_x (p0, z0, z1, x0))

/*
** msb_w0_u32_x_tied2:
**	mov	(z[0-9]+\.s), w0
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_x_tied2, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_x (p0, z1, z0, x0),
		 z0 = svmsb_x (p0, z1, z0, x0))

/*
** msb_w0_u32_x_untied:
**	mov	z0\.s, w0
**	mls	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_ZS (msb_w0_u32_x_untied, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_x (p0, z1, z2, x0),
		 z0 = svmsb_x (p0, z1, z2, x0))

/*
** msb_s4_u32_x_tied1:
**	mov	(z[0-9]+\.s), s4
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_x_tied1, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_x (p0, z0, z1, d4),
		 z0 = svmsb_x (p0, z0, z1, d4))

/*
** msb_s4_u32_x_tied2:
**	mov	(z[0-9]+\.s), s4
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_x_tied2, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_x (p0, z1, z0, d4),
		 z0 = svmsb_x (p0, z1, z0, d4))

/*
** msb_s4_u32_x_untied:
**	mov	z0\.s, s4
**	mls	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_ZS (msb_s4_u32_x_untied, svuint32_t, uint32_t,
		 z0 = svmsb_n_u32_x (p0, z1, z2, d4),
		 z0 = svmsb_x (p0, z1, z2, d4))

/*
** msb_2_u32_x_tied1:
**	mov	(z[0-9]+\.s), #2
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_x_tied1, svuint32_t,
		z0 = svmsb_n_u32_x (p0, z0, z1, 2),
		z0 = svmsb_x (p0, z0, z1, 2))

/*
** msb_2_u32_x_tied2:
**	mov	(z[0-9]+\.s), #2
**	msb	z0\.s, p0/m, z1\.s, \1
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_x_tied2, svuint32_t,
		z0 = svmsb_n_u32_x (p0, z1, z0, 2),
		z0 = svmsb_x (p0, z1, z0, 2))

/*
** msb_2_u32_x_untied:
**	mov	z0\.s, #2
**	mls	z0\.s, p0/m, z1\.s, z2\.s
**	ret
*/
TEST_UNIFORM_Z (msb_2_u32_x_untied, svuint32_t,
		z0 = svmsb_n_u32_x (p0, z1, z2, 2),
		z0 = svmsb_x (p0, z1, z2, 2))
