/* Test the ifunc and target attributes.  If the target does not support MSA
   then we expect that the generic version of function will be executed.  */
/* { dg-require-ifunc "" } */
/* { dg-do run } */
/* { dg-require-effective-target mips_ifunc_callback } */
/* { dg-require-effective-target mips_nomsa_runtime } */
/* { dg-skip-if "code quality test" { *-*-* } { "-O0" "-fuse-linker-plugin" } { "" } } */
/* { dg-output "MSA: no(\n|\r\n|\r)+" } */
/* { dg-output "executing add_generic(\n|\r)+" } */
#include <stdio.h>
#include <stdlib.h>
#include <sys/auxv.h>
