...
test_xor_fetch (int op)
{
  ...
  if (__atomic_xor_fetch (&v, op, __ATOMIC_ACQUIRE) != 2)
    abort ();

  if (__atomic_xor_fetch (&v, 255, __ATOMIC_RELEASE) != 253)
    abort ();

  if (__atomic_xor_fetch (&v, op, __ATOMIC_ACQ_REL) != 252)
    abort ();

  if (__atomic_xor_fetch (&v, 0xfff00000, __ATOMIC_SEQ_CST) != 0xfff00003)
    abort ();
