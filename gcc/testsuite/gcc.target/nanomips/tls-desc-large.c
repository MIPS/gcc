/* { dg-do compile } */
/* { dg-options "-m32 -march=32r6 -mcmodel=large -fPIC -mtls-dialect=desc -ftls-model=global-dynamic" } */
...
/* { dg-final { scan-assembler "f_addr:.*\taddiu\\.b32\t\\\$a0,\\\$gp,%tlsdesc_got\\(tls_var\\).*f_addr\n" } } */
...
/* { dg-final { scan-assembler "f_load:.*\taddiu\\.b32\t\\\$a0,\\\$gp,%tlsdesc_got\\(tls_var\\).*f_load\n" } } */
...
/* { dg-final { scan-assembler "f_store:.*\taddiu\\.b32\t\\\$a0,\\\$gp,%tlsdesc_got\\(tls_var\\).*f_store\n" } } */
