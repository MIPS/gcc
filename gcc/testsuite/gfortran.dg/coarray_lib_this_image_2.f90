...
! { dg-final { scan-tree-dump-times "mylbound = parm...dim\\\[0\\\].extent > 0 \? .*kind=4.*parm.*dim.*lower_bound . 1;" 1 "original" } }
! { dg-final { scan-tree-dump-times "mythis_image = _gfortran_caf_this_image \\(0\\);" 1 "original" } }
! { dg-final { scan-tree-dump-times "bar \\(&parm.\[0-9\]+, caf_token.\[0-9\]+, \\(integer\\(kind=\[48\]\\)\\) parm.\[0-9\]+.base_addr - \\(integer\\(kind=\[48\]\\)\\) x\\);" 1 "original" } }
