...
struct pointer_map_t *chkp_rtx_bounds;
...
chkp_get_rtl_bounds (tree node)
{
  ...
  slot = (rtx *)pointer_map_contains (chkp_rtx_bounds, node);
...
chkp_set_rtl_bounds (tree node, rtx val)
{
  ...
  slot = (rtx *)pointer_map_insert (chkp_rtx_bounds, node);
  *slot = val;
}
...
/* Helper function for chkp_copy_bounds_for_stack_parm.
...
static void
chkp_find_bound_slots (tree type, bool *have_bound,
		      HOST_WIDE_INT offs,
		      HOST_WIDE_INT ptr_size)
{
  if (BOUNDED_TYPE_P (type))
	...
	if (TREE_CODE (field) == FIELD_DECL)
	  {
	    ...
	    chkp_find_bound_slots (TREE_TYPE (field), have_bound,
	...
	chkp_find_bound_slots (etype, have_bound, offs + cur * esize, ptr_size);
    }
}
...
{
  HOST_WIDE_INT ptr_size = TREE_INT_CST_LOW (TYPE_SIZE (chkp_uintptr_type));
  ...
  chkp_find_bound_slots (type, have_bound, 0, ptr_size);
  ...
  /* Detect bounds copy calls.  */
  ...
  else
    {
	 ...
	 obtain returned bounds.  */
      stmt = gimple_build_call (chkp_ret_bnd_fndecl, 0);
      chkp_mark_stmt (stmt);
	...
	case RESULT_DECL:
	  {
	    ...
	    bounds = chkp_make_bounds (node, TYPE_SIZE_UNIT (base_type), NULL, false);
	    chkp_register_bounds (node, bounds);
	  }
	  break;
