...
/* Returns tag for PTR (a ssa name).  If SYMBOL_ONLY is true, only SMT may be
   returned.  */

static tree
get_ptr_tag (tree ptr, bool symbol_only)
{
  tree tag = NULL_TREE;

  gcc_assert (TREE_CODE (ptr) == SSA_NAME);
  if (!symbol_only && SSA_NAME_PTR_INFO (ptr))
...
{  
  ...
  tag_a = get_ptr_tag (ptr_a, false);
  if (!tag_a)
    tag_a = DR_MEMTAG (dra);
  if (!tag_a)
    return false;

  tag_b = get_ptr_tag (ptr_b, false);
  if (!tag_b)
    tag_b = DR_MEMTAG (drb);
  if (!tag_b)
    return false;
 ...
 *ptr_info = NULL;
  ...
  /* Part 1:  Case 3. INDIRECT_REFs.  */
	...
	{
	case SSA_NAME:
	  *memtag = get_ptr_tag (base_address, true);
	  if (!*memtag && TREE_CODE (TREE_OPERAND (memref, 0)) == SSA_NAME)
	    *memtag = get_ptr_tag (TREE_OPERAND (memref, 0), true);
	  break;
	case ADDR_EXPR:
