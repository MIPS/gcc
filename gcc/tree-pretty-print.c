...
void dump_escape_function (con_graph cg, FILE* debug_file, FILE* dump_file);
...
void
dump_escape_function (con_graph cg, FILE* debug_file, FILE* dump_file)
{
  tree arg, vars, var;
  ...
  fprintf (debug_file, "%s (", IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (function)));
  fprintf (dump_file, "%s (", IDENTIFIER_POINTER (DECL_ASSEMBLER_NAME (function)));

  arg = DECL_ARGUMENTS (function);
  while (arg)
    {
      print_generic_expr (debug_file, arg, dump_flags);
  ...
  if (flags & TDF_DETAILS)
    {
      dump_eh_tree (debug_file, DECL_STRUCT_FUNCTION (function));
      dump_eh_tree (dump_file, DECL_STRUCT_FUNCTION (function));
    }
  if (flags & TDF_RAW)
    {
      dump_node (function, TDF_SLIM | flags, debug_file);
      dump_node (function, TDF_SLIM | flags, dump_file);
      return;
    }

  /* Switch CFUN to point to FN.  */
  saved_cfun = cfun;
  cfun = DECL_STRUCT_FUNCTION (function);

  /* When GIMPLE is lowered, the variables are no longer available in
     BIND_EXPRs, so display them separately.  */
  if (cfun && cfun->decl == function && cfun->unexpanded_var_list)
    {
  ...
  if (cfun && cfun->decl == function && cfun->cfg && basic_block_info)
    {
      /* Make a CFG based dump.  */
      check_bb_profile (ENTRY_BLOCK_PTR, debug_file);
      check_bb_profile (ENTRY_BLOCK_PTR, dump_file);
      ...
      FOR_EACH_BB (bb)
	{
	  ...
	  debug_buffer.buffer->stream = debug_file;
	  dump_buffer.buffer->stream = dump_file;

	  if (label_indent < 0)
	    label_indent = 0;

	  dump_bb_header (&debug_buffer, bb, indent, flags);
	  dump_bb_header (&dump_buffer, bb, indent, flags);

	  dump_phi_nodes (&debug_buffer, bb, indent, flags);
	  dump_phi_nodes (&dump_buffer, bb, indent, flags);
	  ...
	  for (bsi = bsi_start (bb); !bsi_end_p (bsi); bsi_next (&bsi))
	    {
	      ...
	      for (i = 0; i<curr_indent; i++)
		{
		  pp_space (&debug_buffer);
		  pp_space (&dump_buffer);
		}

	      dump_generic_node (&debug_buffer, stmt, curr_indent, flags, true);
	      dump_generic_node (&dump_buffer, stmt, curr_indent, flags, true);
	      print_stmt_type (cg, debug_file, stmt);
	      print_stmt_type (cg, dump_file, stmt);
	  ...
	  dump_implicit_edges (&debug_buffer, bb, indent, flags);
	  dump_implicit_edges (&dump_buffer, bb, indent, flags);

	  if (flags & TDF_BLOCKS)
	    {
	      dump_bb_end (&debug_buffer, bb, indent, flags);
	      dump_bb_end (&dump_buffer, bb, indent, flags);
	    }
	}
      pp_flush (&debug_buffer);
      pp_flush (&dump_buffer);
      ...
      fprintf (debug_file, "}\n");
      fprintf (dump_file, "}\n");
      check_bb_profile (EXIT_BLOCK_PTR, debug_file);
      check_bb_profile (EXIT_BLOCK_PTR, dump_file);
    }
  else
    {
      int indent;
      ...
      chain = DECL_SAVED_TREE (function);
      ...
      if (any_var)
	{
      ...
      print_generic_stmt_indented (debug_file, chain, flags, indent);
      print_generic_stmt_indented (dump_file, chain, flags, indent);
      if (ignore_topmost_bind)
	{
	  fprintf (debug_file, "}\n");
	  fprintf (dump_file, "}\n");
