...
static void optimize_stmt (struct dom_walk_data *, 
			   basic_block bb,
			   block_stmt_iterator);
static tree simplify_stmt_for_jump_threading (tree);
...
dom_thread_across_edge (struct dom_walk_data *walk_data, edge e)
{
  ...
  if (! walk_data->global_data)
    {
      tree dummy_cond = build (NE, boolean_type_node,
			       integer_zero_node, integer_zero_node);
      dummy_cond = build (COND_EXPR, void_type_node, dummy_cond, NULL, NULL);
      walk_data->global_data = dummy_cond;
    }

  taken_edge = thread_across_edge (walk_data->global_data, e,
				   &const_and_copies_stack,
				   simplify_stmt_for_jump_threading);

  /* If we were successful, then record the jump threading opportunity.  */
  if (taken_edge)
    {
      if (e->aux)
        edge_info = e->aux;
		  ...
		  if (rhs_def_code != rhs_code)
		    {
		      if (rhs_def_code == MINUS_EXPR)
		        t = fold_build2 (MINUS_EXPR, type,
					 outer_const, def_stmt_op1);
		      else
		        t = fold_build2 (MINUS_EXPR, type,
					 def_stmt_op1, outer_const);
		      rhs_code = PLUS_EXPR;
		    }
		    ...
		    t = fold_build2 (rhs_def_code, type,
				     def_stmt_op1, outer_const);
