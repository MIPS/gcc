...
outermost_indep_loop (struct loop *outer, struct loop *loop, mem_ref_p ref)
{
  ...
  for (aloop = outer;
       aloop != loop;
       aloop = superloop_at_depth (loop, aloop->depth + 1))
...
/* Marks all virtual operands of the statement STMT for renaming.  */
  ...
  FOR_EACH_LOOP (li, loop, LI_FROM_INNERMOST)
    {
      ...
      if (loop->outer == current_loops->tree_root)
	continue;
  ...
  EXECUTE_IF_SET_IN_BITMAP (refs, 0, i, bi)
    {
      ...
      for (sloop = loop; sloop != current_loops->tree_root; sloop = sloop->outer)
...
execute_sm (struct loop *loop, VEC (edge, heap) *exits, mem_ref_p ref)
{
  ...
  for (i = 0; VEC_iterate (edge, exits, i, ex); i++)
    {
      store = build2_gimple (GIMPLE_MODIFY_STMT,
			     unshare_expr (ref->mem), tmp_var);
...
tree_ssa_lim_finalize (void)
{
  ...
  if (memory_accesses.ttae_cache)
    htab_delete (memory_accesses.ttae_cache);
}
