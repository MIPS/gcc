...
find_escape_constraints (tree stmt)
{
  ...
  if (TREE_CODE (stmt) == RETURN_EXPR)
    {
      /* Returns are either bare, with an embedded GIMPLE_MODIFY_STMT, or
	 just a plain old expression.  */
      if (!TREE_OPERAND (stmt, 0))
	return;
      if (TREE_CODE (TREE_OPERAND (stmt, 0)) == GIMPLE_MODIFY_STMT)
	rhs = GIMPLE_STMT_OPERAND (TREE_OPERAND (stmt, 0), 1);
      else
