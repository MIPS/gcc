...
/* Return the LHS of any ASSERT_EXPR where OP appears as the first
   ...
   E->src.  If no such ASSERT_EXPR is found, return OP.  */

static tree
lhs_of_dominating_assert (tree op, edge e, tree stmt)
{
  ...
  FOR_EACH_IMM_USE_SAFE (imm_use, imm_iter, op)
    {
      ...
      if (use_stmt != stmt
        ...
        {
	  if (dominated_by_p (CDI_DOMINATORS, e->src, bb_for_stmt (use_stmt)))
      ...
      else
	{
	 ...
	 to simplify this statement.  */
	  ...
	  && (TREE_CODE (cached_lhs) == SSA_NAME
	      || is_gimple_min_invariant (cached_lhs)))
	record_temporary_equivalence (TREE_OPERAND (stmt, 0), cached_lhs,
				      stack);
    }
  return stmt;
}
  ...
  if (COMPARISON_CLASS_P (cond))
      ...
      if (TREE_CODE (op1) == SSA_NAME)
	{
	 ...
	 operand with the LHS of the ASSERT_EXPR.  */
      if (TREE_CODE (op0) == SSA_NAME)
	op0 = lhs_of_dominating_assert (op0, e, stmt);

      if (TREE_CODE (op1) == SSA_NAME)
	op1 = lhs_of_dominating_assert (op1, e, stmt);
  ...
  /* We can have conditionals which just test the state of a variable
	...
	cached_lhs = SSA_NAME_VALUE (cached_lhs);
	 ...
	 update CACHED_LHS appropriately.  */
      if (TREE_CODE (cached_lhs) == SSA_NAME)
	cached_lhs = lhs_of_dominating_assert (cached_lhs, e, stmt);
