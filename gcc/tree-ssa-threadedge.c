...
fsm_find_thread_path (basic_block start_bb, basic_block end_bb,
		      ...
		      bitmap visited_bbs, loop_p loop)
{
...
/* The hash table for remembering what phis we've seen.  */
hash_table <pointer_hash <gimple_statement_base> > visited_phis;

static bool
seen_phi (gimple stmt)
{
  gimple * slot = visited_phis.find_slot (stmt, INSERT);
  if (*slot != NULL)
    return true;
					 ...
					 vec<basic_block, va_gc> *&path)
{
  ...
  if (seen_phi (def_stmt))
    return;

  gimple_statement_phi *phi = as_a <gimple_statement_phi > (def_stmt);
  ...
  if (var_bb != last_bb_in_path)
	...
	{
	   bitmap visited_bbs = BITMAP_ALLOC (NULL);

	  if (fsm_find_thread_path (var_bb, e->src, next_path, visited_bbs,
				    e->src->loop_father))
	    ++e_count;

	  BITMAP_FREE (visited_bbs);
  ...
  for (i = 0; i < gimple_phi_num_args (phi); i++)
      ...
      if (TREE_CODE (arg) == SSA_NAME)
	{
	  ...
	  fsm_find_control_statement_thread_paths (arg, path);
	  path->pop ();
	  continue;
	}
