...
vectorizable_store (tree stmt, block_stmt_iterator *bsi, tree *vec_stmt)
{
  ...
  for (j = 0; j < ncopies; j++)
      ...
      else 
	{
      ...
      /* FIXME mem-ssa.  We should not need to mark so much to rename here.  */

     /* Set the VDEFS for the vector pointer. If this virtual def has a 
