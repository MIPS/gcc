...
/* Utility functions for loop tranformations prior to vectorization.  */
...
#ifdef ENABLE_CHECKING
  if (!is_ptr_ref && !is_array_ref && !is_addr_expr)
    abort ();
#endif
...
#ifdef ENABLE_CHECKING
  if (TREE_CODE (ref) != ARRAY_REF && TREE_CODE (ref) != INDIRECT_REF)
    abort ();
#endif
