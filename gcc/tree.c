...
build_complex_type (ttype_p component_type)
{
  ...
  if (TYPE_CANONICAL (t) == t)
	...
	TYPE_CANONICAL (t) = build_complex_type
		  ...
		  hash_set<tree> *pset, walk_tree_lh lh)
{
  ...
  switch (TREE_CODE (type))
    ...
    case COMPLEX_TYPE:
      WALK_SUBTREE_PTR (&TREE_TYPE (type));
      break;

    case METHOD_TYPE:
      WALK_SUBTREE_PTR (&TYPE_METHOD_BASETYPE (type));
	...
	WALK_SUBTREE_PTR (&TREE_TYPE (type));
      WALK_SUBTREE_PTR (&TYPE_DOMAIN (type));
      break;

    case OFFSET_TYPE:
      WALK_SUBTREE_PTR (&TREE_TYPE(type));
      WALK_SUBTREE_PTR (&TYPE_OFFSET_BASETYPE (type));
      break;
  ...
  switch (code)
      ...
      if (TREE_CODE (DECL_EXPR_DECL (*tp)) == TYPE_DECL)
	{
	  ...
	  /* Same for scalar types.  */
	    ...
	    {
	      WALK_SUBTREE_PTR (&TYPE_MIN_VALUE (*type_p));
	      WALK_SUBTREE_PTR (&TYPE_MAX_VALUE (*type_p));
	    }

	  WALK_SUBTREE_PTR (&TYPE_SIZE (*type_p));
	  WALK_SUBTREE_TAIL_PTR (&TYPE_SIZE_UNIT (*type_p));
	}
      /* FALLTHRU */
