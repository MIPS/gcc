  ...
  switch (code)
    ...
    case VECTOR_CST:
      return (sizeof (struct tree_vector)
	      + (TREE_TYPE (node)->vector_subparts () - 1) * sizeof (tree));
...
ttype *
make_type_node_stat (enum tree_code code MEM_STAT_DECL)
{
  ttype *t;
  ...
  t = ggc_alloc_cleared_ttype_node_stat (length PASS_MEM_STAT);
  t->set_code (code);

  t->set_uid (next_type_uid++);
  t->set_align (BITS_PER_UNIT);
  t->set_user_align_p (false);
  t->set_main_variant (t);
  t->set_canonical (t);

  /* Default to no attributes for type, but let target change that.  */
  t->set_attributes (NULL_TREE);
...
make_node_stat (enum tree_code code MEM_STAT_DECL)
{
  ...
  if (type == tcc_type)
    return make_type_node_stat (code);
...
static unsigned int
get_int_cst_ext_nunits (ttype *type, const wide_int &cst)
{
  gcc_checking_assert (cst.get_precision () == type->precision ());
  /* We need extra HWIs if CST is an unsigned integer with its
     upper bit set.  */
  if (type->unsigned_p () && wi::neg_p (cst))
...
tree
build_int_cst (ttype_p type, HOST_WIDE_INT low)
{
  /* Support legacy code.  */
  if (!type)
    type = integer_type_node;

  return wide_int_to_tree (type, wi::shwi (low, type->precision ()));
}

tree
build_int_cstu (ttype_p type, unsigned HOST_WIDE_INT cst)
{
  return wide_int_to_tree (type, wi::uhwi (cst, type->precision ()));
}

/* Create an INT_CST node with a LOW value sign extended to TYPE.  */

tree
build_int_cst_type (ttype_p type, HOST_WIDE_INT low)
{
  gcc_assert (type);
  return wide_int_to_tree (type, wi::shwi (low, type->precision ()));
}
...
tree
double_int_to_tree (ttype_p type, double_int cst)
{
  return wide_int_to_tree (type, widest_int::from (cst, type->sign ()));
}
...
tree
force_fit_type (ttype_p type, const wide_int_ref &cst,
		int overflowable, bool overflowed)
{
  signop sign = type->sign ();
  ...
  if (overflowed || !wi::fits_to_tree_p (cst, type))
	...
	{
	  wide_int tmp = wide_int::from (cst, type->precision (), sign);
...
int_cst_hasher::hash (tree x)
{
  ...
  hashval_t code = TREE_TYPE (t)->uid ();
  int i;
...
tree
wide_int_to_tree (ttype_p type, const wide_int_ref &pcst)
{
  tree t;
  int ix = -1;
  int limit = 0;
  ...
  gcc_assert (type);
  unsigned int prec = type->precision ();
  signop sgn = type->sign ();
  ...
  if (ext_len == 1)
      ...
      if (ix >= 0)
	{
	  ...
	  if (!type->cached_values_p ())
	    {
	      type->set_cached_values_p (true);
	      type->set_cached_values (make_tree_vec (limit));
	    }

	  t = TREE_VEC_ELT (type->cached_values (), ix);
	  if (t)
	  ...
	  else
	    {
	      ...
	      TREE_VEC_ELT (type->cached_values (), ix) = t;
	    }
	}
      else
	{
	  ...
	  TREE_SET_TYPE (int_cst_node, type);
...
cache_integer_cst (tree t)
{
  ...
  if (ix >= 0)
      ...
      if (!type->cached_values_p ())
	{
      ...
      gcc_assert (TREE_VEC_ELT (type->cached_values (), ix) == NULL_TREE);
      TREE_VEC_ELT (type->cached_values (), ix) = t;
    }
  else
    {
...
tree
build_low_bits_mask (ttype_p type, unsigned bits)
{
  gcc_assert (bits <= type->precision ());
...
cst_and_fits_in_hwi (const_tree x)
{
  ...
  if (TREE_TYPE (x)->precision () > HOST_BITS_PER_WIDE_INT)
    return false;
...
tree
build_vector_stat (ttype_p type, tree *vals MEM_STAT_DECL)
{
  int over = 0;
  unsigned cnt = 0;
  tree v = make_vector (type->vector_subparts ());
  TREE_SET_TYPE (v, type);

  /* Iterate through elements and check for overflow.  */
  for (cnt = 0; cnt < type->vector_subparts (); ++cnt)
    {
      tree value = vals[cnt];
...
tree
build_vector_from_ctor (ttype_p type, vec<constructor_elt, va_gc> *v)
{
  tree *vec = XALLOCAVEC (tree, type->vector_subparts ());
  ...
  for (; idx < type->vector_subparts (); ++idx)
    vec[pos++] = build_zero_cst (type->type ());
...
tree
build_vector_from_val (ttype_p vectype, tree sc) 
{
  int i, nunits = vectype->vector_subparts ();
  ...
  /* Verify that the vector type is suitable for SC.  Note that there
     ...
     vector element-type.  */
  gcc_checking_assert (types_compatible_p (TREE_TYPE (sc)->main_variant (),
					   vectype->type ()));
...
tree
build_constructor (ttype_p type, vec<constructor_elt, va_gc> *vals)
{
...
/* Build a CONSTRUCTOR node made of a single initializer, with the specified
   INDEX and VALUE.  */
tree
build_constructor_single (ttype_p type, tree index, tree value)
{
  vec<constructor_elt, va_gc> *v;
...
/* Return a new CONSTRUCTOR node whose type is TYPE and whose values
   are in a list pointed to by VALS.  */
tree
build_constructor_from_list (ttype_p type, tree vals)
{
  tree t;
...
tree
build_constructor_va (ttype_p type, int nelts, ...)
{
  vec<constructor_elt, va_gc> *v = NULL;
  va_list p;
...
tree
build_fixed (ttype_p type, FIXED_VALUE_TYPE f)
{
  tree v;
  FIXED_VALUE_TYPE *fp;
...
tree
build_real (ttype_p type, REAL_VALUE_TYPE d)
{
  tree v;
  REAL_VALUE_TYPE *dp;
  int overflow = 0;
...
tree
build_real_truncate (ttype_p type, REAL_VALUE_TYPE d)
{
  return build_real (type, real_value_truncate (type->mode (), d));
}
...
REAL_VALUE_TYPE
real_value_from_int_cst (const ttype_p type, const_tree i)
{
  REAL_VALUE_TYPE d;
  ...
  real_from_integer (&d, type ? type->mode () : VOIDmode, i,
		     TREE_TYPE (i)->sign());
  return d;
}
...
tree
build_real_from_int_cst (ttype_p type, const_tree i)
{
  tree v;
...
tree
build_complex (ttype_p type, tree real, tree imag)
{
  tree t = make_node (COMPLEX_CST);
  ttype *tt = type;
  if (!tt)
    tt = build_complex_type (TREE_TYPE (real));
...
tree
build_complex_inf (ttype_p type, bool neg)
{
  REAL_VALUE_TYPE rinf, rzero = dconst0;

  real_inf (&rinf);
  rzero.sign = neg;
  return build_complex (type, build_real (type->type (), rinf),
			build_real (type->type (), rzero));
}
...
build_one_cst (ttype_p type)
{
  switch (type->code ())
    ...
    case FIXED_POINT_TYPE:
      ...
      gcc_assert (ALL_SCALAR_ACCUM_MODE_P (type->mode ()));
      return build_fixed (type, FCONST1 (type->mode ()));

    case VECTOR_TYPE:
      {
	tree scalar = build_one_cst (type->type ());
    ...
    case COMPLEX_TYPE:
      return build_complex (type,
			    build_one_cst (type->type ()),
			    build_zero_cst (type->type ()));
...
tree
build_all_ones_cst (ttype_p type)
{
  if (type->code () == COMPLEX_TYPE)
    {
      tree scalar = build_all_ones_cst (type->type ());
      return build_complex (type, scalar, scalar);
    }
  else
...
tree
build_minus_one_cst (ttype_p type)
{
  switch (type->code ())
    {
    case INTEGER_TYPE: case ENUMERAL_TYPE: case BOOLEAN_TYPE:
    ...
    case VECTOR_TYPE:
      {
	tree scalar = build_minus_one_cst (type->type());
...
build_zero_cst (ttype_p type)
{
  switch (type->code ())
    ...
    case FIXED_POINT_TYPE:
      return build_fixed (type, FCONST0 (type->mode ()));

    case VECTOR_TYPE:
      {
	tree scalar = build_zero_cst (type->type ());
    ...
    case COMPLEX_TYPE:
      {
	tree zero = build_zero_cst (type->type ());
...
build_case_label (tree low_value, tree high_value, tree label_decl)
{
  ...
  TREE_SET_TYPE (t, void_type_node);
...
integer_all_onesp (const_tree expr)
{
  ...
  return wi::max_value (TREE_TYPE (expr)->precision (), UNSIGNED) == expr;
}
...
{
  if (!TREE_TYPE (expr)->integral_p ()
      && !TREE_TYPE (expr)->pointer_p ())
    return 0;

  unsigned int ret1, ret2, prec = TREE_TYPE (expr)->precision ();
  switch (TREE_CODE (expr))
    {
    case INTEGER_CST:
      ret1 = wi::ctz (expr);
    ...
    CASE_CONVERT:
      ret1 = tree_ctz (TREE_OPERAND (expr, 0));
      if (ret1 && ret1 == TREE_TYPE (TREE_OPERAND (expr, 0))->precision ())
	ret1 = prec;
      return MIN (ret1, prec);
    case SAVE_EXPR:
  ...
  switch (TREE_CODE (expr))
    ...
    case REAL_CST:
      return real_equal (&TREE_REAL_CST (expr), &dconst0)
	     && !(DECIMAL_FLOAT_MODE_P (TREE_TYPE (expr)->mode ()));
    case COMPLEX_CST:
...
tree
first_field (const ttype_p type)
{
  tree t = type->fields ();
...
tree
size_in_bytes (const ttype_p tt)
{
  tree t;
  ttype *type= tt;
...
HOST_WIDE_INT
int_size_in_bytes (const ttype_p tt)
{
  tree t;
  ttype *type = tt;
...
HOST_WIDE_INT
max_int_size_in_bytes (const ttype_p type)
{
...
tree
array_type_nelts (const ttype_p type)
{
  tree min, max;
  ttype *index_type;
  ...
  index_type = type->domain ();
  min = index_type->min_value ();
  max = index_type->max_value ();
...
staticp (tree arg)
{
  switch (TREE_CODE (arg))
    ...
    case ARRAY_RANGE_REF:
      if (TREE_CODE (TREE_TYPE (arg)->size ()) == INTEGER_CST
...
static bool
type_contains_placeholder_1 (const ttype *type)
{
  /* If the size contains a placeholder or the parent type (component type in
     the case of arrays) type involves a placeholder, this type does.  */
  if (CONTAINS_PLACEHOLDER_P (type->size ())
      || CONTAINS_PLACEHOLDER_P (type->size_unit ())
      || (!type->pointer_p ()
	  && type->type ()
	  && type_contains_placeholder_p (type->type ())))
    return true;
  ...
  switch (type->code ())
    ...
    case FIXED_POINT_TYPE:
      ...
      return (CONTAINS_PLACEHOLDER_P (type->min_value ())
	      || CONTAINS_PLACEHOLDER_P (type->max_value ()));

    case ARRAY_TYPE:
      /* We have already checked the component type above, so just check
	 the domain type.  Flexible array members have a null domain.  */
      return type->domain () ?
	type_contains_placeholder_p (type->domain ()) : false;
	...
	for (field = type->fields (); field; field = DECL_CHAIN (field))
	  if (TREE_CODE (field) == FIELD_DECL
	      && (CONTAINS_PLACEHOLDER_P (DECL_FIELD_OFFSET (field))
		  || (type->code () == QUAL_UNION_TYPE
...
bool
type_contains_placeholder_p (ttype_p type)
{
  bool result;

  /* If the contains_placeholder_bits field has been initialized,
     then we know the answer.  */
  if (type->contains_placeholder_internal () > 0)
    return type->contains_placeholder_internal () - 1;
  ...
  type->set_contains_placeholder_internal (1);

  /* Compute the real value.  */
  result = type_contains_placeholder_1 (type);

  /* Store the real value.  */
  type->set_contains_placeholder_internal (result + 1);
...
substitute_placeholder_in_expr (tree exp, tree obj)
{
  ...
  if (code == PLACEHOLDER_EXPR)
    {
      ttype *need_type = TREE_TYPE (exp)->main_variant ();
      tree elt;
	...
	if (TREE_TYPE (elt)->main_variant () == need_type)
	  return elt;
      ...
      for (elt = obj; elt != 0;
	   elt = ((TREE_CODE (elt) == COMPOUND_EXPR
		  ...
		  : (REFERENCE_CLASS_P (elt)
		     ...
		     || EXPRESSION_CLASS_P (elt))
		  ? TREE_OPERAND (elt, 0) : 0))
	if (TREE_TYPE (elt)->pointer_p ()
	    && (TREE_TYPE (elt)->type()->main_variant ()
		== need_type))
...
stabilize_reference_1 (tree e)
{
  ...
  TREE_SET_TYPE (result, TREE_TYPE (e));
...
stabilize_reference (tree ref)
{
  ...
  TREE_SET_TYPE (result, TREE_TYPE (ref));
...
tree
build0_stat (enum tree_code code, ttype_p tt MEM_STAT_DECL)
{
  tree t;
...
tree
build1_stat (enum tree_code code, ttype_p type, tree node MEM_STAT_DECL)
{
...
tree
build2_stat (enum tree_code code, ttype_p tt, tree arg0, tree arg1 MEM_STAT_DECL)
{
...
tree
build3_stat (enum tree_code code, ttype_p tt, tree arg0, tree arg1,
...
tree
build4_stat (enum tree_code code, ttype_p tt, tree arg0, tree arg1,
...
tree
build5_stat (enum tree_code code, ttype_p tt, tree arg0, tree arg1,
...
int
comp_type_attributes (const ttype_p type1, const ttype_p type2)
{
  const_tree a1 = type1->attributes ();
  const_tree a2 = type2->attributes ();
  const_tree a;
...
static void
free_lang_data_in_type (ttype *type)
{
  /* Give the FE a chance to remove its own data first.  */
  lang_hooks.free_lang_data (type);

  type->clear_lang_flags();

  if (type->code () == FUNCTION_TYPE)
    {
      ...
      for (p = type->arg_types (); p; p = TREE_CHAIN (p))
	{
      ...
      type->set_minval (NULL_TREE);
    }
  if (type->code () == METHOD_TYPE)
    {
      tree p;
  ...
  if (type->record_or_union_p ())
	...
	type->set_fields (NULL_TREE);

      /* FIXME: C FE uses TYPE_VFIELD to record C_TYPE_INCOMPLETE_VARS
 	 and danagle the pointer from time to time.  */
      if (type->vfield () && TREE_CODE (type->vfield ()) != FIELD_DECL)
        type->set_vfield (NULL_TREE);
	 ...
	 by function.c and pretty printers.  */
      if (type->methods ())
        type->set_methods (error_mark_node);
      if (type->binfo ())
	{
	  free_lang_data_in_binfo (type->binfo ());
	  ...
	  if ((!BINFO_VTABLE (type->binfo ())
	       || !flag_devirtualize)
	      && ((!BINFO_N_BASE_BINFOS (type->binfo ())
		   && !BINFO_VTABLE (type->binfo ()))
		  || debug_info_level != DINFO_LEVEL_NONE))
	    type->set_binfo (NULL_TREE);
	}
    }
  else
    {
	 ...
	 overloads TYPE_BINFO).  */
      type->set_lang_slot_1 (NULL_TREE);

      if (type->integral_p ()
	  || type->scalar_float_p ()
	  || type->fixed_point_p ())
	{
	  free_lang_data_in_one_sizepos (type->min_value_ptr ());
	  free_lang_data_in_one_sizepos (type->max_value_ptr ());
	}
    }

  free_lang_data_in_one_sizepos (type->size_ptr ());
  free_lang_data_in_one_sizepos (type->size_unit_ptr ());

  if (type->context ()
      && TREE_CODE (type->context ()) == BLOCK)
    {
      tree ctx = type->context ();
      do
	{
	  ctx = BLOCK_SUPERCONTEXT (ctx);
	}
      while (ctx && TREE_CODE (ctx) == BLOCK);
      type->set_context (ctx);
    }
}
...
need_assembler_name_p (tree decl)
{
  ...
  if (flag_lto_odr_type_mering
      ...
      && DECL_NAME (decl)
      && decl == TREE_TYPE (decl)->name ()
      && !TREE_TYPE (decl)->artificial_p ()
      && (type_with_linkage_p (TREE_TYPE (decl))
	  || TREE_TYPE (decl)->code () == INTEGER_TYPE)
 ...
 if (TREE_CODE (decl) == FUNCTION_DECL)
      ...
      if (gimple_has_body_p (decl))
	{
	 ...
	 origin will not be output correctly.  */
	  ...
	  && DECL_CONTEXT_TYPE_P (DECL_ABSTRACT_ORIGIN (decl))
	  && DECL_CONTEXT_TYPE (DECL_ABSTRACT_ORIGIN (decl))
							->record_or_union_p ())
  ...
  if (DECL_P (t))
  ...
  else if (is_a<ttype *>(t))
    {
	 ...
	 types, so we have to do our own traversals here.  */
      ...
      if (!tt->record_or_union_p ())
	fld_worklist_push (tt->cached_values (), fld);
      fld_worklist_push (tt->size (), fld);
      fld_worklist_push (tt->size_unit (), fld);
      fld_worklist_push (tt->attributes (), fld);
      fld_worklist_push (tt->pointer_to (), fld);
      fld_worklist_push (tt->reference_to (), fld);
      fld_worklist_push (tt->name (), fld);
	 ...
	 this way.  */
      if (!tt->pointer_p ())
	fld_worklist_push (tt->minval (), fld);
      if (!tt->record_or_union_p ())
	fld_worklist_push (tt->maxval (), fld);
      fld_worklist_push (tt->main_variant (), fld);
      ...
      if (tt->context ())
	{
	 ...
	 and want not to reach unused types this way.  */
      ...
      if (tt->record_or_union_p () && tt->binfo ())
	{
	  unsigned i;
	  tree tem;
	  FOR_EACH_VEC_ELT (*BINFO_BASE_BINFOS (tt->binfo ()), i, tem)
	    fld_worklist_push (TREE_TYPE (tem), fld);
	  tem = BINFO_VIRTUALS (tt->binfo ());
	  if (tem
      ...
      if (tt->record_or_union_p ())
	{
      ...
      fld_worklist_push (tt->stub_decl (), fld);
      *ws = 0;
    }
...
free_lang_data (void)
{
  ...
  for (i = 0; i < itk_none; ++i)
    if (integer_types[i])
      integer_types[i]->set_alias_set (get_alias_set (integer_types[i]));
...
tree
merge_type_attributes (ttype *t1, ttype *t2)
{
  return merge_attributes (t1->attributes (),
			   t2->attributes ());
}
...
merge_type_attributes (tree t1, tree t2)
{
  ...
  return merge_attributes (tt1->attributes (),
			   tt2->attributes ());
}
...
static bool
check_aligned_type (const ttype *cand, const ttype *base, unsigned int align)
{
  return (cand->quals () == base->quals ()
	  && cand->name () == base->name ()
	  /* Apparently this is needed for Objective-C.  */
	  && cand->context () == base->context ()
	  /* Check alignment.  */
...
type_cache_hasher::equal (type_hash *a, type_hash *b)
{
  ...
  if (a->hash != b->hash
      || a->type->code () != b->type->code ()
      || a->type->type () != b->type->type ()
      || !attribute_list_equal (a->type->attributes (),
				 b->type->attributes ())
      || (a->type->code () != COMPLEX_TYPE
  ...
  /* Be careful about comparing arrays before and after the element type
     ...
     complete.  */
  if (a->type->complete_p () && b->type->complete_p ()
      && (a->type->align () != b->type->align ()
	  || a->type->mode () != b->type->mode ()))
    return 0;
  ...
  switch (a->type->code ())
    ...
    case VECTOR_TYPE:
      return a->type->vector_subparts () == b->type->vector_subparts ();

    case ENUMERAL_TYPE:
      if (a->type->values () != b->type->values ()
	  && !(a->type->values ()
	       && TREE_CODE (a->type->values()) == TREE_LIST
	       && b->type->values ()
	       && TREE_CODE (b->type->values ()) == TREE_LIST
	       && type_list_equal (a->type->values (), b->type->values ())))
	return 0;
    ...
    case REAL_TYPE:
    case BOOLEAN_TYPE:
      if (a->type->precision () != b->type->precision ())
	return false;
      return ((a->type->max_value () == b->type->max_value ()
	       || tree_int_cst_equal (a->type->max_value (),
				      b->type->max_value ()))
	      && (a->type->min_value () == b->type->min_value ()
		  || tree_int_cst_equal (a->type->min_value (),
					 b->type->min_value ())));

    case FIXED_POINT_TYPE:
      return a->type->saturating_p () == b->type->saturating_p ();

    case OFFSET_TYPE:
      return a->type->offset_basetype () == b->type->offset_basetype ();

    case METHOD_TYPE:
      if (a->type->method_basetype () == b->type->method_basetype ()
	  && (a->type->arg_types () == b->type->arg_types ()
	      || (a->type->arg_types ()
		  && TREE_CODE (a->type->arg_types ()) == TREE_LIST
		  && b->type->arg_types ()
		  && TREE_CODE (b->type->arg_types ()) == TREE_LIST
		  && type_list_equal (a->type->arg_types (),
				      b->type->arg_types ()))))
        break;
      return 0;
    case ARRAY_TYPE:
      return a->type->domain () == b->type->domain ();

    case RECORD_TYPE:
    case UNION_TYPE:
    case QUAL_UNION_TYPE:
      return (a->type->fields () == b->type->fields ()
	      || (a->type->fields ()
		  && TREE_CODE (a->type->fields ()) == TREE_LIST
		  && b->type->fields ()
		  && TREE_CODE (b->type->fields ()) == TREE_LIST
		  && type_list_equal (a->type->fields (),
				      b->type->fields ())));

    case FUNCTION_TYPE:
      if (a->type->arg_types () == b->type->arg_types ()
	  || (a->type->arg_types ()
	      && TREE_CODE (a->type->arg_types ()) == TREE_LIST
	      && b->type->arg_types ()
	      && TREE_CODE (b->type->arg_types ()) == TREE_LIST
...
int
type_num_arguments (const ttype_p type)
{
  int i = 0;
  tree t;
...
{
  unsigned bitno = TREE_TYPE (t)->precision () - 1;
...
tree_int_cst_sgn (const_tree t)
{
  if (wi::eq_p (t, 0))
    return 0;
  else if (TREE_TYPE (t)->unsigned_p ())
    return 1;
  else if (wi::neg_p (t))
    return -1;
  else
...
unsigned int
element_precision (const ttype *type)
{
  enum tree_code code = type->code ();
...
/* This can be eliminated once all callers are 'type *'.  */
unsigned int
...
bool
operation_no_trapping_overflow (ttype_p type, enum tree_code code)
{
  gcc_checking_assert (type->any_integral_p ());
      ...
      else
	{
	  ...
	  if (CONVERT_EXPR_CODE_P (code)
	      || code == NON_LVALUE_EXPR)
	    {
	      ...
	      hstate.add_int (TREE_TYPE (t)->unsigned_p ());
			     ...
			     bool can_alias_all)
{
  ...
  for (t = to_type->pointer_to (); t; t = t->next_ptr_to ())
    if (t->mode () == mode && t->ref_can_alias_all_p () == can_alias_all)
      return t;
  ...
  t->set_type (to_type);
  t->set_mode (mode);
  t->set_ref_can_alias_all_p (can_alias_all);
...
static ttype *
build_range_type_1 (ttype *type, tree lowval, tree highval, bool shared)
{
...
ttype *
build_range_type (ttype_p type, tree lowval, tree highval)
{
...
ttype *
build_nonshared_range_type (ttype_p type, tree lowval, tree highval)
{
...
bool
subrange_type_for_debug_p (const ttype_p type, tree *lowval, tree *highval)
{
  ttype *base_type = type->type ();
  tree low, high;

  /* Subrange types have a base type which is an integral type.  */
  if (!base_type->integral_p ())
    return false;
  ...
  if ((base_type->code () == INTEGER_TYPE
       || base_type->code () == BOOLEAN_TYPE)
      && int_size_in_bytes (type) == int_size_in_bytes (base_type)
      && tree_int_cst_equal (low, base_type->min_value ())
      && tree_int_cst_equal (high, base_type->max_value ())
      && type->identifier () == base_type->identifier ())
    return false;
...
static ttype *
build_array_type_1 (ttype *elt_type, ttype *index_type, bool shared)
{
  ttype *t;

  if (elt_type->code () == FUNCTION_TYPE)
    {
  ...
  t->set_type (elt_type);
  t->set_domain (index_type);
  t->set_addr_space (elt_type->addr_space ());
  layout_type (t);

  /* If the element type is incomplete at this point we get marked for
     structural equality.  Do not record these types in the canonical
     type hashtable.  */
  if (t->structural_equality_p ())
    return t;

  if (shared)
    {
      inchash::hash hstate;
      hstate.add_object (elt_type->hash ());
      if (index_type)
	hstate.add_object (index_type->hash ());
      t = type_hash_canon (hstate.end (), t);
    }

  if (t->canonical () == t)
    {
      if (elt_type->structural_equality_p ()
	  || (index_type && index_type->structural_equality_p ())
	  || in_lto_p)
	t->set_structural_equality_p ();
      else if (elt_type->canonical () != elt_type
	       || (index_type && index_type->canonical () != index_type))
	t->set_canonical (build_array_type_1 (elt_type->canonical (),
					      index_type
					      ? index_type->canonical ()
						: NULL,
					      shared));
    }
...
ttype *
build_array_type (ttype_p elt_type, ttype_p index_type)
{
...
ttype *
build_nonshared_array_type (ttype_p elt_type, ttype_p index_type)
{
...
ttype *
build_array_type_nelts (ttype_p elt_type, unsigned HOST_WIDE_INT nelts)
{
...
ttype *
build_function_type (ttype_p value_type, tree arg_types)
{
  ttype *t;
  inchash::hash hstate;
  ...
  if (value_type->code () == FUNCTION_TYPE)
    {
  ...
  t->set_type (value_type);
  t->set_arg_types (arg_types);

  /* If we already have such a type, use the old one.  */
  hstate.add_object (value_type->hash ());
  type_hash_list (arg_types, hstate);
  t = type_hash_canon (hstate.end (), t);

  /* Set up the canonical type. */
  any_structural_p   = value_type->structural_equality_p ();
  any_noncanonical_p = value_type->canonical () != value_type;
  ...
  if (any_structural_p)
    ...
    t->set_canonical (build_function_type (value_type->canonical (),
					   canon_argtypes));
...
static ttype *
build_function_type_list_1 (bool vaargs, ttype *return_type, va_list argp)
{
  tree t, args, last;
  ttype *ret;
...
ttype *
build_function_type_list (ttype_p return_type, ...)
{
  ttype *args;
  va_list p;
...
ttype *
build_varargs_function_type_list (ttype_p return_type, ...)
{
  ttype *args;
  va_list p;
...
static ttype *
build_function_type_array_1 (bool vaargs, ttype *return_type, int n,
...
ttype *
build_function_type_array (ttype_p return_type, int n, type_array arg_types)
{
...
ttype *
build_varargs_function_type_array (ttype_p return_type, int n,
				   type_array arg_types)
{
...
ttype *
build_method_type_directly (ttype_p basetype,
			    ttype_p rettype,
			    tree argtypes)
{
  ttype *t;
  ...
  t->set_method_basetype (basetype->main_variant ());
  t->set_type (rettype);
  ...
  t->set_arg_types (argtypes);

  /* If we already have such a type, use the old one.  */
  hstate.add_object (basetype->hash ());
  hstate.add_object (rettype->hash ());
  type_hash_list (argtypes, hstate);
  t = type_hash_canon (hstate.end (), t);

  /* Set up the canonical type. */
  any_structural_p
    = (basetype->structural_equality_p ()
       || rettype->structural_equality_p ());
  any_noncanonical_p
    = (basetype->canonical () != basetype
       || rettype->canonical () != rettype);
  ...
  if (any_structural_p)
    ...
    t->set_canonical (build_method_type_directly (basetype->canonical (),
						  rettype->canonical (),
						  canon_argtypes));
  if (!t->complete_p ())
    layout_type (t);
...
ttype *
build_method_type (ttype_p basetype, ttype_p type)
{
  gcc_assert (type->code () == FUNCTION_TYPE);
...
ttype *
build_offset_type (ttype_p basetype, ttype_p type)
{
  ttype *t;
  inchash::hash hstate;
  ...
  t->set_offset_basetype (basetype->main_variant ());
  t->set_type (type);

  /* If we already have such a type, use the old one.  */
  hstate.add_object (basetype->hash ());
  hstate.add_object (type->hash ());
  t = type_hash_canon (hstate.end (), t);
  ...
  if (t->canonical () == t)
    {
      if (basetype->structural_equality_p ()
	  || type->structural_equality_p ())
	t->set_structural_equality_p ();
      else if (basetype->main_variant()->canonical () != basetype
	       || type->canonical () != type)
	t->set_canonical (build_offset_type (
				    basetype->main_variant()->canonical (),
				    type->canonical ()));
    }
...
ttype *
build_complex_type (ttype_p component_type)
{
  ttype *t;
  inchash::hash hstate;

  gcc_assert (component_type->integral_p ()
	      || component_type->scalar_float_p ()
	      || component_type->fixed_point_p ());

  /* Make a node of the sort we want.  */
  t = make_type_node (COMPLEX_TYPE);

  t->set_type (component_type->main_variant ());

  /* If we already have such a type, use the old one.  */
  hstate.add_object (component_type->hash ());
  t = type_hash_canon (hstate.end (), t);
  ...
  if (t->canonical () == t)
    {
      if (component_type->structural_equality_p ())
	t->set_structural_equality_p ();
      else if (component_type->canonical () != component_type)
	t->set_canonical (build_complex_type (component_type->canonical ()));
    }
  ...
  if (! t->name ())
	...
	name = 0;
      ...
      if (name != 0)
	t->set_name (build_decl (UNKNOWN_LOCATION, TYPE_DECL,
				 get_identifier (name), t));
    }

  return build_qualified_type (t, component_type->quals ());
}
...
ttype *
excess_precision_type (ttype_p type)
{
  if (flag_excess_precision != EXCESS_PRECISION_FAST)
	...
	case REAL_TYPE:
	  switch (flt_eval_method)
	    {
	    case 1:
	      if (type->mode () == float_type_node->mode ())
		return double_type_node;
	      break;
	    case 2:
	      if (type->mode () == float_type_node->mode ()
		  || type->mode () == double_type_node->mode())
	  ...
	  break;
	case COMPLEX_TYPE:
	  if (type->type()->code () != REAL_TYPE)
	    return NULL;
	  switch (flt_eval_method)
	    {
	    case 1:
	      if (type->type()->code () == float_type_node->code())
		return complex_double_type_node;
	      break;
	    case 2:
	      if (type->type()->code () == float_type_node->code()
		  || (type->type()->code () == double_type_node->code()))
...
tree
get_unwidened (tree op, ttype_p for_type)
{
  /* Set UNS initially if converting OP to FOR_TYPE is a zero-extension.  */
  ttype *type = TREE_TYPE (op);
  unsigned final_prec = (for_type != NULL) ? for_type->precision ()
					   : type->precision ();
  int uns
    = (for_type != NULL && for_type != type
       && final_prec > type->precision ()
       && type->unsigned_p ());
  tree win = op;
  ...
  while (CONVERT_EXPR_P (op))
    {
	 ...
	 so avoid them here.  */
      if (TREE_TYPE (TREE_OPERAND (op, 0))->code () == VECTOR_TYPE)
	break;

      bitschange = TREE_TYPE (op)->precision ()
		   - TREE_TYPE (TREE_OPERAND (op, 0))->precision ();

      /* Truncations are many-one so cannot be removed.
	 Unless we are later going to truncate down even farther.  */
      if (bitschange < 0
	  && final_prec > TREE_TYPE (op)->precision ())
	break;
	 ...
	 are all going to be discarded later by truncating to FOR_TYPE.  */
      ...
      if (bitschange > 0)
	{
	  if (! uns || final_prec <= TREE_TYPE (op)->precision ())
	    win = op;
...
get_narrower (tree op, int *unsignedp_ptr)
{
  ...
  int first = 1;
  tree win = op;
  bool integral_p = TREE_TYPE (op)->integral_p ();

  while (TREE_CODE (op) == NOP_EXPR)
    {
      int bitschange
	= (TREE_TYPE (op)->precision ()
	   - TREE_TYPE (TREE_OPERAND (op, 0))->precision ());
      ...
      if (bitschange > 0)
	{
	  ...
	  if (first)
	    uns = TREE_TYPE (op)->unsigned_p ();
	  /* Otherwise, if a sign extension has been stripped,
	     only sign extensions can now be stripped;
	     if a zero extension has been stripped, only zero-extensions.  */
	  else if (uns != TREE_TYPE (op)->unsigned_p ())
	    break;
	  first = 0;
	}
      else /* bitschange == 0 */
	{
	  ...
	  if (TREE_TYPE (op)->integral_p () != integral_p)
	    continue;
	}
  ...
  if (TREE_CODE (op) == COMPONENT_REF
      ...
      && TREE_TYPE (op)->code () != REAL_TYPE
      && TREE_TYPE (op)->code () != FIXED_POINT_TYPE
	...
	= tree_to_uhwi (DECL_SIZE (TREE_OPERAND (op, 1)));
      int unsignedp = (DECL_UNSIGNED (TREE_OPERAND (op, 1))
		       || TREE_TYPE (TREE_OPERAND (op, 1))->unsigned_p ());
	 ...
	 advantage in doing this.  */
      ...
      if (innerprec < TREE_TYPE (op)->precision ()
...
retry:
  ...
  if (type->precision () > TREE_TYPE (c)->precision ())
    return true;

  /* Third, unsigned integers with top bit set never fit signed types.  */
  if (!type->unsigned_p () && sgn_c == UNSIGNED)
    {
      unsigned prec = GET_MODE_PRECISION (TREE_TYPE (c)->mode ()) - 1;
      if (prec < TREE_TYPE (c)->precision ())
	{
...
void
get_type_static_bounds (const ttype_p type, mpz_t min, mpz_t max)
{
  if (!type->pointer_p () && type->min_value ()
      && TREE_CODE (type->min_value ()) == INTEGER_CST)
    wi::to_mpz (type->min_value (), min, type->sign ());
  else
    {
      if (type->unsigned_p ())
	mpz_set_ui (min, 0);
	...
	{
	  wide_int mn = wi::min_value (type->precision (), SIGNED);
	  wi::to_mpz (mn, min, SIGNED);
	}
    }

  if (!type->pointer_p () && type->max_value ()
      && TREE_CODE (type->max_value ()) == INTEGER_CST)
    wi::to_mpz (type->max_value (), max, type->sign ());
  else
    {
      wide_int mn = wi::max_value (type->precision (), type->sign ());
      wi::to_mpz (mn, max, type->sign ());
    }
}
...
bool
variably_modified_type_p (ttype_p type, tree fn)
{
  tree t;
...
#define RETURN_TRUE_IF_VAR(T)						\
  ...
  RETURN_TRUE_IF_VAR (type->size ());
  RETURN_TRUE_IF_VAR (type->size_unit ());
  ...
  switch (type->code ())
    ...
    case REFERENCE_TYPE:
    case VECTOR_TYPE:
      if (variably_modified_type_p (type->type (), fn))
	return true;
      break;
    ...
    case BOOLEAN_TYPE:
	 ...
	 aren't constant.  */
      RETURN_TRUE_IF_VAR (type->min_value ());
      RETURN_TRUE_IF_VAR (type->max_value ());
      break;
    ...
    case QUAL_UNION_TYPE:
	 ...
	 recursion via pointers.  */
      ...
      for (t = type->fields (); t; t = DECL_CHAIN (t))
	if (TREE_CODE (t) == FIELD_DECL)
	  {
    ...
    case ARRAY_TYPE:
	 ...
	 variably modified if the element type is.  */
      RETURN_TRUE_IF_VAR (type->type()->size ());
      RETURN_TRUE_IF_VAR (type->type()->size_unit ());
      break;
...
tree
get_containing_scope (const ttype *t)
{
  return t->context ();
}
...
decl_function_context (const_tree decl)
{
  ...
  /* C++ virtual functions use DECL_CONTEXT for the class of the vtable
    ...
    context = TREE_TYPE (TREE_VALUE (TREE_TYPE (decl)->arg_types ()))
			 ->main_variant ();
  else
...
static ttype *
make_vector_type (ttype *innertype, int nunits, machine_mode mode)
{
  ttype *t;
  inchash::hash hstate;
...
build_common_tree_nodes (bool signed_char, bool short_double)
{
  ...
  TREE_SET_TYPE (error_mark_node, error_type_node);
...
/* Returns a vector tree node given a mode (integer, vector, or BLKmode) and
   the inner type.  */
ttype *
build_vector_type_for_mode (ttype_p innertype, machine_mode mode)
{
  int nunits;
  ...
  switch (GET_MODE_CLASS (mode))
    ...
    case MODE_INT:
      ...
      gcc_assert (GET_MODE_BITSIZE (mode)
		  % TREE_INT_CST_LOW (innertype->size ()) == 0);

      nunits = GET_MODE_BITSIZE (mode)
	       / TREE_INT_CST_LOW (innertype->size ());
      break;
...
ttype *
build_vector_type (ttype_p innertype, int nunits)
{
...
build_truth_vector_type (unsigned nunits, unsigned vector_size)
{
  ...
  ttype *bool_type = build_nonstandard_boolean_type (esize);
...
static tree
build_call_1 (ttype_p return_type, tree fn, int nargs)
{
  tree t;

  t = build_vl_exp (CALL_EXPR, nargs + 3);
  TREE_SET_TYPE (t, return_type);
  CALL_EXPR_FN (t) = fn;
...
bool
tree_nop_conversion_p (const ttype_p outer_type, const ttype_p inner_type)
{
  /* Use precision rather then machine mode when we can, which gives
     the correct answer even for submode (bit-field) types.  */
  if ((outer_type->integral_p ()
       || outer_type->integral_p ()
       || outer_type->code () == OFFSET_TYPE)
      && (inner_type->integral_p ()
	  || inner_type->pointer_p ()
	  || inner_type->code () == OFFSET_TYPE))
    return outer_type->precision () == inner_type->precision ();

  /* Otherwise fall back on comparing machine modes (e.g. for
     aggregate types, floats).  */
  return outer_type->mode () == inner_type->mode ();
}
...
tree_sign_nop_conversion (const_tree exp)
{
  ...
  return (outer_type->unsigned_p () == inner_type->unsigned_p ()
	  && outer_type->pointer_p () == inner_type->pointer_p ());
}
...
strip_float_extensions (tree exp)
{
  ...
  if (TREE_CODE (exp) == REAL_CST && !TREE_TYPE (exp)->decimal_float_p ())
    {
      REAL_VALUE_TYPE orig;
      tree type = NULL;

      orig = TREE_REAL_CST (exp);
      if (TREE_TYPE (exp)->precision () > float_type_node->precision ()
	  && exact_real_truncate (float_type_node->mode (), &orig))
	type = float_type_node;
      else if (TREE_TYPE (exp)->precision ()
	       > double_type_node->precision ()
	       && exact_real_truncate (double_type_node->mode (), &orig))
  ...
  if (expt->decimal_float_p () != subt->decimal_float_p ())
    return exp;

  if (subt->precision () > expt->precision ())
    return exp;
...
virtual_method_call_p (const_tree target)
{
  ...
  /* If we do not have BINFO associated, it means that type was built
     ...
     call.  */
  if (!obj_type_ref_class (target)->binfo ())
    return false;
  return true;
}
...
get_binfo_at_offset (tree binfo, HOST_WIDE_INT offset, tree expected_type)
{
  ...
  while (true)
	...
	return NULL_TREE;
      ...
      for (fld = type->fields (); fld; fld = DECL_CHAIN (fld))
	{
      ...
      if (!fld || TREE_TYPE (fld)->code () != RECORD_TYPE)
	return NULL_TREE;
...
bool
typedef_variant_p (const ttype_p type)
{
  return is_typedef_decl (type->name ());
}

/* Warn about a use of an identifier which was marked deprecated.  */
void
warn_deprecated_use (ttype *node, tree attr)
{
  const char *msg;
  ...
  if (!attr)
    {
      tree decl = node->stub_decl ();
      if (decl)
	attr = lookup_attribute ("deprecated",
				 TREE_TYPE (decl)->attributes ());
    }
  ...
  if (node->name ())
    {
      if (TREE_CODE (node->name ()) == IDENTIFIER_NODE)
	what = node->name ();
      else if (TREE_CODE (node->name ()) == TYPE_DECL
	       && DECL_NAME (node->name ()))
	what = DECL_NAME (node->name ());
    }
...
warn_deprecated_use (tree node, tree attr)
{
  ...
  if (is_a<ttype *>(node))
    {
      warn_deprecated_use (as_a<ttype *>(node), attr);
      return ;
    }
...
array_ref_element_size (tree exp)
{
  ...
  ttype *elmt_type = TREE_TYPE (TREE_OPERAND (exp, 0))->type ();
  ...
  if (aligned_size)
	...
	aligned_size = fold_convert_loc (loc, sizetype, aligned_size);
      return size_binop_loc (loc, MULT_EXPR, aligned_size,
			     size_int (elmt_type->align_unit ()));
    }

  /* Otherwise, take the size from that of the element type.  Substitute
     any PLACEHOLDER_EXPR that we have.  */
  else
    return SUBSTITUTE_PLACEHOLDER_IN_EXPR (elmt_type->size_unit (), exp);
}
...
{
  ttype *domain_type = TREE_TYPE (TREE_OPERAND (exp, 0))->domain ();
  ...
  if (domain_type && domain_type->min_value ())
    return SUBSTITUTE_PLACEHOLDER_IN_EXPR (domain_type->min_value (), exp);
...
array_ref_up_bound (tree exp)
{
  ...
  if (domain_type && domain_type->max_value ())
    return SUBSTITUTE_PLACEHOLDER_IN_EXPR (domain_type->max_value (), exp);
  ...
  while (handled_component_p (ref))
    {
	 ...
	 is not at the end of a structure.  */
      if (TREE_CODE (ref) == COMPONENT_REF
	  && TREE_TYPE (TREE_OPERAND (ref, 0))->code () == RECORD_TYPE)
	{
...
machine_mode
element_mode (const ttype *t)
{
  if (t->vector_p () || t->code () == COMPLEX_TYPE)
    t = t->type ();
  return t->mode ();
}
...
{
  if (is_a<const ttype *>(t))
    return element_mode (as_a<const ttype *>(t));
  else
    return element_mode (TREE_TYPE (t));
}
...
static bool
verify_type_variant (const ttype_p t, ttype_p tv)
{
...
#define verify_variant_match(flag)					    \
  ...
  verify_variant_match (code);
  /* FIXME: Ada builds non-artificial variants of artificial types.  */
  if (tv->artificial_p () && 0)
    verify_variant_match (artificial_p);
  if (tv->pointer_p())
    verify_variant_match (ref_can_alias_all_p);
  /* FIXME: TYPE_SIZES_GIMPLIFIED may differs for Ada build.  */
  verify_variant_match (unsigned_p);
  verify_variant_match (align_ok_p);
  verify_variant_match (packed_p);
  if (t->code () == REFERENCE_TYPE)
    verify_variant_match (ref_is_rvalue_p);
  if (t->aggregate_p ())
    verify_variant_match (reverse_storage_order_p);
  else
    verify_variant_match (saturating_p);
  /* FIXME: This check trigger during libstdc++ build.  */
  if (t->record_or_union_p () && t->complete_p () && 0)
    verify_variant_match (final_p);

  /* tree_type_common checks.  */

  if (t->complete_p ())
    {
      verify_variant_match (size);
      verify_variant_match (mode);
      if (t->size_unit () != tv->size_unit ()
	  ...
	  && !operand_equal_p (t->size_unit (), tv->size_unit (), 0))
	{
	  error ("type variant has different size_unit");
	  debug_tree (tv);
	  error ("type variant's size_unit");
	  debug_tree (tv->size_unit());
	  error ("type's size_unit");
	  debug_tree (t->size_unit());
	  return false;
	}
    }
  verify_variant_match (precision);
  verify_variant_match (needs_constructing_p);
  if (t->record_or_union_p ())
    verify_variant_match (transparent_aggr_p);
  else if (t->code () == ARRAY_TYPE)
    verify_variant_match (nonaliased_component_p);
  ...
  if ((!in_lto_p || !t->file_scope_p ()) && 0)
    verify_variant_match (context);
  verify_variant_match (string_flag_p);
  if (t->alias_set_known_p())
    {
      error ("type variant with alias_set_known_p");
      debug_tree (tv);
      return false;
    }
  ...
  if (t->record_or_union_p () && t->vfield () != tv->vfield ()
      && (in_lto_p || !tv->vfield ()
	  || TREE_CODE (tv->vfield ()) != TREE_LIST))
    {
      error ("type variant has different vfield");
      debug_tree (tv);
      return false;
    }
  if ((t->code () == ENUMERAL_TYPE && t->complete_p ())
       || t->code () == INTEGER_TYPE
       || t->code () == BOOLEAN_TYPE
       || t->code () == REAL_TYPE
       || t->code () == FIXED_POINT_TYPE)
    {
      verify_variant_match (max_value);
      verify_variant_match (min_value);
    }
  if (t->code () == METHOD_TYPE)
    verify_variant_match (method_basetype);
  if (t->record_or_union_p () && t->methods ())
    {
      error ("type variant has methods");
      debug_tree (tv);
      return false;
    }
  if (t->code () == OFFSET_TYPE)
    verify_variant_match (offset_basetype);
  if (t->code () == ARRAY_TYPE)
    verify_variant_match (array_max_size);
  /* FIXME: Be lax and allow TYPE_BINFO to be missing in variant types
     ...
     of the copies.  */
  if (t->record_or_union_p () && t->binfo () && tv->binfo ()
      && t->binfo () != tv->binfo ()
    ...
    {
      error ("type variant has different binfo");
      debug_tree (tv);
      error ("type variant's binfo");
      debug_tree (tv->binfo ());
      error ("type's binfo");
      debug_tree (t->binfo ());
      return false;
  ...
  if (t->code () == ENUMERAL_TYPE)
    verify_variant_match (values);
  else if (t->code () == ARRAY_TYPE)
    verify_variant_match (domain);
  /* Permit incomplete variants of complete type.  While FEs may complete
	   ...
	   && t->fields () != tv->fields ())
    {
	 ...
	 qualified type. Verify that they looks same.  */
      for (f1 = t->fields (), f2 = tv->fields ();
	   f1 && f2;
	   f1 = TREE_CHAIN (f1), f2 = TREE_CHAIN (f2))
	if (TREE_CODE (f1) != FIELD_DECL || TREE_CODE (f2) != FIELD_DECL
	    || (TREE_TYPE (f1)->main_variant ()
		 != TREE_TYPE (f2)->main_variant ()
  ...
  else if ((t->code () == FUNCTION_TYPE || t->code () == METHOD_TYPE))
    verify_variant_match (arg_types);
  ...
  if (t->type () != tv->type ()
      && ((t->code () != ARRAY_TYPE && !t->pointer_p ())
	  || t->type()->main_variant () != tv->type()->main_variant ()))
    {
      error ("type variant has different type");
      debug_tree (tv);
      error ("type variant's type");
      debug_tree (tv->type ());
      error ("type's type");
      debug_tree (t->type ());
      return false;
...
bool
type_with_interoperable_signedness (const ttype_p type)
{
  ...
  return tree_code_for_canonical_type_merging (type->code ())
	   == INTEGER_TYPE
         && (type->precision () == signed_char_type_node->precision ()
	     || type->precision () == size_type_node->precision ());
}
...
bool
gimple_canonical_types_compatible_p (ttype_p t1, ttype_p t2,
				     bool trust_type_canonical)
{
  ...
  enum tree_code code = tree_code_for_canonical_type_merging (t1->code ());
  if (code != tree_code_for_canonical_type_merging (t2->code ()))
    return false;

  /* Qualifiers do not matter for canonical type comparison purposes.  */

  /* Void types and nullptr types are always the same.  */
  if (t1->code () == VOID_TYPE
      || t1->code () == NULLPTR_TYPE)
    return true;
  ...
  if (t1->integral_p ()
	...
	return false;
	 ...
	 inter-operable.  */
      if (t1->unsigned_p () != t2->unsigned_p ()
	...
	return false;
	 ...
	 Still pointers in different address spaces are not compatible.  */
      if (t1->pointer_p ())
	{
	  if (t1->type()->addr_space ()
	      != t2->type()->addr_space ())
	    return false;
	}

      /* Tail-recurse to components.  */
      if (t1->code () == VECTOR_TYPE
	  || t1->code () == COMPLEX_TYPE)
	return gimple_canonical_types_compatible_p (t1->type (),
						    t2->type (),
						    trust_type_canonical);
  ...
  switch (t1->code ())
    ...
    case ARRAY_TYPE:
	 ...
	 the number of elements are the same.  */
      if (!gimple_canonical_types_compatible_p (t1->type (), t2->type (),
						trust_type_canonical)
	  || t1->string_flag_p () != t2->string_flag_p ()
	  || t1->reverse_storage_order_p () != t2->reverse_storage_order_p ()
	  || t1->nonaliased_component_p () != t2->nonaliased_component_p ())
	return false;
      else
	{
	  ttype *i1 = t1->domain ();
	  ...
	  if (i1 == NULL && i2 == NULL)
	    ...
	    {
	      tree min1 = i1->min_value ();
	      tree min2 = i2->min_value ();
	      tree max1 = i1->max_value ();
	      tree max2 = i2->max_value ();
      ...
      else
	{
	  ...
	  for (parms1 = t1->arg_types (), parms2 = t2->arg_types ();
	       parms1 && parms2;
	...
	if (!t1->complete_p () && !t2->complete_p ())
	  return true;


	if (t1->reverse_storage_order_p () != t2->reverse_storage_order_p ())
	  return false;

	/* For aggregate types, all the fields must be the same.  */
	for (f1 = t1->fields (), f2 = t2->fields ();
	     f1 || f2;
	     f1 = TREE_CHAIN (f1), f2 = TREE_CHAIN (f2))
	  {
	    /* Skip non-fields.  */
...
void
verify_type (ttype_p t)
{
  bool error_found = false;
  ttype *mv = t->main_variant ();
  if (!mv)
    {
      error ("Main variant is not defined");
      error_found = true;
    }
  else if (mv != mv->main_variant ())
    {
      error ("Main_variant has different main_variant");
      debug_tree (mv);
      error_found = true;
    }
  ...
  ttype *ct = t->canonical ();
  if (!ct)
    ;
  else if (t->canonical () != ct)
    {
      error ("Canonical has different canonical");
      debug_tree (ct);
      error_found = true;
    }
  /* Method and function types can not be used to address memory and thus
    ...
    {
      error ("Canonical is not compatible");
      debug_tree (ct);
      error_found = true;
    }

  if (t->complete_p () && t->canonical ()
      && t->mode () != t->canonical()->mode ())
    {
      error ("Mode of canonical is not compatible");
      debug_tree (ct);
      error_found = true;
    }
  if (t->main_variant () == t && ct && ct->main_variant () != ct)
   {
      error ("Canonical of main variant is not main variant");
      debug_tree (ct);
      debug_tree (ct->main_variant ());
      error_found = true;
   }
  ...
  if (t->record_or_union_p ())
    {
	 ...
	 and danagle the pointer from time to time.  */
      if (t->vfield ()
	  && TREE_CODE (t->vfield ()) != FIELD_DECL
	  && TREE_CODE (t->vfield ()) != TREE_LIST)
	{
	  error ("Vfield is not FIELD_DECL nor TREE_LIST");
	  debug_tree (t->vfield ());
	  error_found = true;
	}
    }
  else if (t->code () == POINTER_TYPE)
    {
      if (t->next_ptr_to ()
	  && t->next_ptr_to()->code () != POINTER_TYPE)
	{
	  error ("Next_ptr_to is not POINTER_TYPE");
	  debug_tree (t->next_ptr_to ());
	  error_found = true;
	}
    }
  else if (t->code () == REFERENCE_TYPE)
    {
      if (t->next_ref_to ()
	  && t->next_ref_to()->code () != REFERENCE_TYPE)
	{
	  error ("Next_ref_to is not REFERENCE_TYPE");
	  debug_tree (t->next_ref_to ());
	  error_found = true;
	}
    }
  else if (t->integral_p () || t->code () == REAL_TYPE
	   || t->code () == FIXED_POINT_TYPE)
    {
  ...
  /* Java uses TYPE_MINVAL for TYPE_ARGUMENT_SIGNATURE.  */
	   ...
	   && ((t->code () != METHOD_TYPE && t->code () != FUNCTION_TYPE)
	       || in_lto_p))
    {
      error ("Minval non-NULL");
      debug_tree (t->minval ());
      error_found = true;
    }
  ...
  if (t->record_or_union_p ())
    {
      if (t->methods () && TREE_CODE (t->methods ()) != FUNCTION_DECL
	  && TREE_CODE (t->methods ()) != TEMPLATE_DECL
	  && t->methods () != error_mark_node)
	{
	  error ("Methods is not FUNCTION_DECL, TEMPLATE_DECL nor error_mark_node");
	  debug_tree (t->methods ());
	  error_found = true;
	}
    }
  else if (t->code () == FUNCTION_TYPE || t->code () == METHOD_TYPE)
    {
      if (t->method_basetype ()
	  && t->method_basetype()->code () != RECORD_TYPE
	  && t->method_basetype()->code () != UNION_TYPE)
	{
	  error ("Method_basetype is not record nor union");
	  debug_tree (t->method_basetype ());
	  error_found = true;
	}
    }
  else if (t->code () == OFFSET_TYPE)
    {
      if (t->offset_basetype ()
	  && t->offset_basetype()->code () != RECORD_TYPE
	  && t->offset_basetype()->code () != UNION_TYPE)
	{
	  error ("Offset_basetype is not record nor union");
	  debug_tree (t->offset_basetype ());
	  error_found = true;
	}
    }
  else if (t->integral_p () || t->code () == REAL_TYPE
    ...
    {
      if (t->array_max_size ()
	  && TREE_CODE (t->array_max_size ()) != INTEGER_CST)
        {
	  error ("Array_max_size not INTEGER_CST");
	  debug_tree (t->array_max_size ());
	  error_found = true;
        } 
    }
  else if (t->maxval ())
    {
      error ("Maxval non-NULL");
      debug_tree (t->maxval ());
      error_found = true;
    }
  ...
  if (t->record_or_union_p ())
    {
      if (!t->binfo ())
	;
      else if (TREE_CODE (t->binfo ()) != TREE_BINFO)
	{
	  error ("Binfo is not TREE_BINFO");
	  debug_tree (t->binfo ());
	  error_found = true;
	}
      /* FIXME: Java builds invalid empty binfos that do not have
         TREE_TYPE set.  */
      else if (TREE_TYPE (t->binfo ()) != t->main_variant () && 0)
	{
	  error ("Binfo type is not main_variant");
	  debug_tree (TREE_TYPE (t->binfo ()));
	  error_found = true;
	}
    }
  else if (t->lang_slot_1 () && in_lto_p)
    {
      error ("Lang_slot_1 (binfo) field is non-NULL");
      debug_tree (t->lang_slot_1 ());
      error_found = true;
    }

  /* Check various uses of TYPE_VALUES_RAW.  */
  if (t->code () == ENUMERAL_TYPE)
    for (tree l = t->values (); l; l = TREE_CHAIN (l))
      {
	...
	if (TREE_TYPE (value)->code () != INTEGER_TYPE
	    && !useless_type_conversion_p (t, TREE_TYPE (value)))
	  {
    ...
    {
      if (t->domain () && t->domain()->code () != INTEGER_TYPE)
	{
	  error ("Array domain is not integer type");
	  debug_tree (t->domain ());
	  error_found = true;
	}
    }
    ...
    {
      if (t->fields () && !t->complete_p () && in_lto_p)
	{
	  error ("Fields defined in incomplete type");
	  error_found = true;
	}
      for (tree fld = t->fields (); fld; fld = TREE_CHAIN (fld))
	{
	  ...
	  if (TREE_CODE (fld) == FIELD_DECL)
	    ...
	    {
	      error ("Wrong tree in fields list");
	      debug_tree (fld);
	      error_found = true;
	    }
	}
    }
  else if (t->code () == INTEGER_TYPE
	   || t->code () == BOOLEAN_TYPE
	   || t->code () == OFFSET_TYPE
	   || t->code () == REFERENCE_TYPE
	   || t->code () == NULLPTR_TYPE
	   || t->code () == POINTER_TYPE)
    {
      if (t->cached_values_p () != (t->cached_values () != NULL))
	{
	  error ("TYPE_CACHED_VALUES_P is %i while TYPE_CACHED_VALUES is %p",
		 t->cached_values_p (), (void *)(t->cached_values ()));
	  error_found = true;
	}
      else if (t->cached_values_p () 
	       && TREE_CODE (t->cached_values ()) != TREE_VEC)
	{
	  error ("TYPE_CACHED_VALUES is not TREE_VEC");
	  debug_tree (t->cached_values ());
	  error_found = true;
	}
	...
	{
	  int i;
	  for (i = 0; i < TREE_VEC_LENGTH (t->cached_values ()); i++)
	    if (TREE_VEC_ELT (t->cached_values (), i)
		&& TREE_TYPE (TREE_VEC_ELT (t->cached_values (), i)) != t)
	      {
		error ("wrong TYPE_CACHED_VALUES entry");
		debug_tree (TREE_VEC_ELT (t->cached_values (), i));
		error_found = true;
		break;
	      }
	}
    ...
    for (tree l = t->arg_types (); l; l = TREE_CHAIN (l))
      {
	/* C++ FE uses TREE_PURPOSE to store initial values.  */
  ...
  else if (!is_lang_specific (t) && t->values_raw ())
    {
      error ("TYPE_VALUES_RAW field is non-NULL");
      debug_tree (t->values_raw ());
      error_found = true;
    }
  if (t->code () != INTEGER_TYPE
      && t->code () != BOOLEAN_TYPE
      && t->code () != OFFSET_TYPE
      && t->code () != REFERENCE_TYPE
      && t->code () != NULLPTR_TYPE
  ...
  /* ipa-devirt makes an assumption that TYPE_METHOD_BASETYPE is always
     ...
     of a type. */
  if (t->code () == METHOD_TYPE
      && t->method_basetype()->main_variant () != t->method_basetype ())
    {
...
nonnull_arg_p (const_tree arg)
{
  ...
  gcc_assert (TREE_CODE (arg) == PARM_DECL
	      && (TREE_TYPE (arg)->pointer_p ()
		  || TREE_TYPE (arg)->code () == OFFSET_TYPE));
  ...
  if (TREE_TYPE (cfun->decl)->code () == METHOD_TYPE
      && arg == DECL_ARGUMENTS (cfun->decl)
      && flag_delete_null_pointer_checks)
    return true;

  /* Values passed by reference are always non-NULL.  */
  if (TREE_TYPE (arg)->code () == REFERENCE_TYPE
      && flag_delete_null_pointer_checks)
    return true;

  fntype = TREE_TYPE (cfun->decl);
  for (attrs = fntype->attributes (); attrs; attrs = TREE_CHAIN (attrs))
    {
...
/* flag_trap_v is not available in tree.h, so this function must be
   implemented here.  */
