  ...
  if (ext_len == 1)
      ...
      if (ix >= 0)
	{
	  ...
	  if (!TYPE_CACHED_VALUES_P (type))
	    {
	      SET_TYPE_CACHED_VALUES_P (type, true);
	      SET_TYPE_CACHED_VALUES (type, make_tree_vec (limit));
	    }
      ...
      else
	{
	  ...
	  SET_TREE_TYPE (int_cst_node, type);
...
build_case_label (tree low_value, tree high_value, tree label_decl)
{
  ...
  SET_TREE_TYPE (t, void_type_node);
...
size_in_bytes (const ttype_p tt)
{
  ...
  if (tt == error_type_node)
    return integer_zero_node;

  type = TYPE_MAIN_VARIANT (tt);
...
type_contains_placeholder_p (ttype_p type)
{
  ...
  SET_TYPE_CONTAINS_PLACEHOLDER_INTERNAL (type, 1);

  /* Compute the real value.  */
  result = type_contains_placeholder_1 (type);

  /* Store the real value.  */
  SET_TYPE_CONTAINS_PLACEHOLDER_INTERNAL (type, result + 1);
...
stabilize_reference_1 (tree e)
{
  ...
  SET_TREE_TYPE (result, TREE_TYPE (e));
...
stabilize_reference (tree ref)
{
  ...
  SET_TREE_TYPE (result, TREE_TYPE (ref));
...
tree
build_fn_decl (const char *name, ttype_p type)
{
...
build_type_attribute_qual_variant (ttype_p type, tree attribute, int quals)
{
  if (! attribute_list_equal (TYPE_ATTRIBUTES (type), attribute))
	  ...
	  || TREE_CODE (type) == ENUMERAL_TYPE)
	{
      ...
      SET_TYPE_ATTRIBUTES (ntype, attribute);
	...
	SET_TYPE_CANONICAL (ntype, TYPE_CANONICAL (type));
  ...
  if (RECORD_OR_UNION_TYPE_P (type))
	...
	SET_TYPE_FIELDS (type, NULL_TREE);
	 ...
	 by function.c and pretty printers.  */
      if (TYPE_METHODS (type))
        SET_TYPE_METHODS (type, error_mark_node);
      if (TYPE_BINFO (type))
	{
  ...
  else
    {
	 ...
	 overloads TYPE_BINFO).  */
      SET_TYPE_LANG_SLOT_1 (type, NULL_TREE);

      if (INTEGRAL_TYPE_P (type)
	  || SCALAR_FLOAT_TYPE_P (type)
	  || FIXED_POINT_TYPE_P (type))
	{
	  free_lang_data_in_one_sizepos (TYPE_MIN_VALUE_PTR (type));
	  free_lang_data_in_one_sizepos (TYPE_MAX_VALUE_PTR (type));
	}
    }

  free_lang_data_in_one_sizepos (TYPE_SIZE_PTR (type));
  free_lang_data_in_one_sizepos (TYPE_SIZE_UNIT_PTR (type));
...
/* Add type T to one of the list of tree nodes that need their
   language data removed.  The lists are held inside FLD.  */

static void
add_type_to_fld_list (ttype *t, struct free_lang_data_d *fld)
{
...
/* Add decl T to one of the list of tree nodes that need their
   language data removed.  The lists are held inside FLD.  */
static void
  ...
  if (DECL_P (t))
  ...
  else if (TYPE_P (t))
    {
	 ...
	 types, so we have to do our own traversals here.  */
      add_type_to_fld_list (tt, fld);

      if (!RECORD_OR_UNION_TYPE_P (tt))
	fld_worklist_push (TYPE_CACHED_VALUES (tt), fld);
      fld_worklist_push (TYPE_SIZE (tt), fld);
      fld_worklist_push (TYPE_SIZE_UNIT (tt), fld);
      fld_worklist_push (TYPE_ATTRIBUTES (tt), fld);
      fld_worklist_push (TYPE_POINTER_TO (tt), fld);
      fld_worklist_push (TYPE_REFERENCE_TO (tt), fld);
      fld_worklist_push (TYPE_NAME (tt), fld);
	 ...
	 this way.  */
      if (!POINTER_TYPE_P (tt))
	fld_worklist_push (TYPE_MINVAL (tt), fld);
      if (!RECORD_OR_UNION_TYPE_P (tt))
	fld_worklist_push (TYPE_MAXVAL (tt), fld);
      fld_worklist_push (TYPE_MAIN_VARIANT (tt), fld);
      /* Do not walk TYPE_NEXT_VARIANT.  We do not stream it and thus
         do not and want not to reach unused variants this way.  */
      if (TYPE_CONTEXT (tt))
	{
	  tree ctx = TYPE_CONTEXT (tt);
	 ...
	 and want not to reach unused types this way.  */
      ...
      if (RECORD_OR_UNION_TYPE_P (tt) && TYPE_BINFO (tt))
	{
	  unsigned i;
	  tree tem;
	  FOR_EACH_VEC_ELT (*BINFO_BASE_BINFOS (TYPE_BINFO (tt)), i, tem)
	    fld_worklist_push (TREE_TYPE (tem), fld);
	  tem = BINFO_VIRTUALS (TYPE_BINFO (tt));
	  if (tem
      ...
      if (RECORD_OR_UNION_TYPE_P (tt))
	{
      ...
      fld_worklist_push (TYPE_STUB_DECL (tt), fld);
      *ws = 0;
    }
...
free_lang_data_in_cgraph (void)
{
  ...
  FOR_EACH_VEC_ELT (fld.types, i, tt)
...
free_lang_data (void)
{
  ...
  for (i = 0; i < itk_none; ++i)
    if (integer_types[i])
      SET_TYPE_ALIAS_SET (integer_types[i], get_alias_set (integer_types[i]));
...
tree
handle_dll_decl_attribute (tree * pnode, tree name,
			   tree args __attribute__((unused)),
			   ...
			   bool *no_add_attrs)
{
  ...
  return handle_dll_decl_attribute (TYPE_NAME_PTR (node), name, args, flags,
				    no_add_attrs);
}
...
{
  SET_TYPE_READONLY (type, ((type_quals & TYPE_QUAL_CONST) != 0));
  SET_TYPE_VOLATILE (type, ((type_quals & TYPE_QUAL_VOLATILE) != 0));
  SET_TYPE_RESTRICT (type, ((type_quals & TYPE_QUAL_RESTRICT) != 0));
  SET_TYPE_ATOMIC (type, ((type_quals & TYPE_QUAL_ATOMIC) != 0));
  SET_TYPE_ADDR_SPACE (type, DECODE_QUAL_ADDR_SPACE (type_quals));
}
...
static ttype *
find_atomic_core_type (ttype *type)
{
...
build_qualified_type (ttype_p type, int type_quals)
{
  ...
  if (!t)
	...
	/* Build the underlying canonical type, since it is different
	   from TYPE. */
	{
	  ttype *c = build_qualified_type (TYPE_CANONICAL (type), type_quals);
	  SET_TYPE_CANONICAL (t, TYPE_CANONICAL (c));
	}
      else
	/* T is its own canonical type. */
	SET_TYPE_CANONICAL (t, t);
...
build_distinct_type_copy (ttype_p type)
{
  ...
  SET_TYPE_POINTER_TO (t, 0);
  SET_TYPE_REFERENCE_TO (t, 0);
...
{
  ttype *t, *m = TYPE_MAIN_VARIANT (type);

  t = build_distinct_type_copy (type);

  /* Since we're building a variant, assume that it is a non-semantic
     variant. This also propagates TYPE_STRUCTURAL_EQUALITY_P. */
  SET_TYPE_CANONICAL (t, TYPE_CANONICAL (type));
  /* Type variants have no alias set defined.  */
  SET_TYPE_ALIAS_SET (t, -1);

  /* Add the new type to the chain of variants of TYPE.  */
  SET_TYPE_NEXT_VARIANT (t, TYPE_NEXT_VARIANT (m));
  SET_TYPE_NEXT_VARIANT (m, t);
  SET_TYPE_MAIN_VARIANT (t, m);
...
type_hash_list (const_tree list, inchash::hash &hstate)
{
  ...
  for (tail = list; tail; tail = TREE_CHAIN (tail))
    if (TREE_VALUE_TYPE (tail) != error_type_node)
      hstate.add_object (TYPE_HASH (TREE_VALUE_TYPE (tail)));
}
...
ttype *
type_hash_canon (unsigned int hashcode, ttype_p type)
{
  type_hash in;
  type_hash **loc;
...
type_num_arguments (const ttype_p type)
{
  ...
  for (t = TYPE_ARG_TYPES (type); t; t = TREE_CHAIN (t))
    ...
    if (VOID_TYPE_P (TREE_VALUE_TYPE (t)))
      break;
    else
      ++i;
...
/* the check for a type can be removed once the change to ttype * has been
   completed.  */
unsigned int
			     ...
			     bool can_alias_all)
{
  ...
  SET_TREE_TYPE (t, to_type);
  SET_TYPE_MODE (t, mode);
  SET_TYPE_REF_CAN_ALIAS_ALL (t, can_alias_all);
  SET_TYPE_NEXT_PTR_TO (t, TYPE_POINTER_TO (to_type));
  SET_TYPE_POINTER_TO (to_type, t);
			       ...
			       bool can_alias_all)
{
  ...
  SET_TYPE_MODE (t, mode);
  SET_TYPE_REF_CAN_ALIAS_ALL (t, can_alias_all);
  SET_TYPE_NEXT_REF_TO (t, TYPE_REFERENCE_TO (to_type));
  SET_TYPE_REFERENCE_TO (to_type, t);

  /* During LTO we do not set TYPE_CANONICAL of pointers and references.  */
  if (TYPE_STRUCTURAL_EQUALITY_P (to_type) || in_lto_p)
    SET_TYPE_STRUCTURAL_EQUALITY (t);
  else if (TYPE_CANONICAL (to_type) != to_type || could_alias)
    SET_TYPE_CANONICAL (t, build_reference_type_for_mode
				...
				int unsignedp)
{
  ...
  SET_TYPE_PRECISION (itype, precision);
...
build_nonstandard_boolean_type (unsigned HOST_WIDE_INT precision)
{
  ...
  SET_TYPE_PRECISION (type, precision);
...
build_range_type_1 (ttype *type, tree lowval, tree highval, bool shared)
{
  ...
  SET_TREE_TYPE (itype, type);

  SET_TYPE_MIN_VALUE (itype, fold_convert (type, lowval));
  SET_TYPE_MAX_VALUE (itype, highval ? fold_convert (type, highval) : NULL);

  SET_TYPE_PRECISION (itype, TYPE_PRECISION (type));
  SET_TYPE_MODE (itype, TYPE_MODE (type));
  SET_TYPE_SIZE (itype, TYPE_SIZE (type));
  SET_TYPE_SIZE_UNIT (itype, TYPE_SIZE_UNIT (type));
  SET_TYPE_ALIGN (itype, TYPE_ALIGN (type));
  SET_TYPE_USER_ALIGN (itype, TYPE_USER_ALIGN (type));
...
{
  ttype *base_type = TREE_TYPE (type);
  tree low, high;
...
build_array_type_1 (ttype *elt_type, ttype *index_type, bool shared)
{
  ...
  SET_TREE_TYPE (t, elt_type);
  SET_TYPE_DOMAIN (t, index_type);
  SET_TYPE_ADDR_SPACE (t, TYPE_ADDR_SPACE (elt_type));
  layout_type (t);
			     ...
			     bool *any_noncanonical_p)
{
  ...
  for (arg = argtypes; arg && !(*any_structural_p); arg = TREE_CHAIN (arg))
	...
	*any_structural_p = true;
      else if (TYPE_STRUCTURAL_EQUALITY_P (TREE_VALUE_TYPE (arg)))
	*any_structural_p = true;
      else if (TYPE_CANONICAL (TREE_VALUE_TYPE (arg)) != TREE_VALUE_TYPE (arg)
	       || TREE_PURPOSE (arg))
      ...
      for (arg = argtypes; arg; arg = TREE_CHAIN (arg))
        {
          if (arg == void_list_node)
            ...
            canon_argtypes = tree_cons (NULL_TREE,
                                        TYPE_CANONICAL (TREE_VALUE_TYPE (arg)),
                                        canon_argtypes);
        }
...
build_function_type (ttype_p value_type, tree arg_types)
{
  ...
  SET_TREE_TYPE (t, value_type);
  SET_TYPE_ARG_TYPES (t, arg_types);
  ...
  if (any_structural_p)
    ...
    SET_TYPE_CANONICAL (t, build_function_type (TYPE_CANONICAL (value_type),
						canon_argtypes));
...
build_function_type_list_1 (bool vaargs, ttype *return_type, va_list argp)
{
  ...
  tt = build_function_type (return_type, args);
			    ...
			    tree argtypes)
{
  ...
  SET_TYPE_METHOD_BASETYPE (t, TYPE_MAIN_VARIANT (basetype));
  SET_TREE_TYPE (t, rettype);
  ptype = build_pointer_type (basetype);

  /* The actual arglist for this function includes a "hidden" argument
     which is "this".  Put it into the list of argument types.  */
  argtypes = tree_cons (NULL_TREE, ptype, argtypes);
  SET_TYPE_ARG_TYPES (t, argtypes);
...
build_offset_type (ttype_p basetype, ttype_p type)
{
  ...
  SET_TYPE_OFFSET_BASETYPE (t, TYPE_MAIN_VARIANT (basetype));
  SET_TREE_TYPE (t, type);
  ...
  if (TYPE_CANONICAL (t) == t)
	...
	SET_TYPE_CANONICAL (t, build_offset_type
				(TYPE_CANONICAL (TYPE_MAIN_VARIANT (basetype)),
						 TYPE_CANONICAL (type)));
    }
...
build_complex_type (ttype_p component_type)
{
  ...
  SET_TREE_TYPE (t, TYPE_MAIN_VARIANT (component_type));
  ...
  if (TYPE_CANONICAL (t) == t)
	...
	SET_TYPE_CANONICAL (t, build_complex_type
				(TYPE_CANONICAL (component_type)));
    }
  ...
  if (! TYPE_NAME (t))
	...
	name = 0;
      ...
      if (name != 0)
	SET_TYPE_NAME (t, build_decl (UNKNOWN_LOCATION, TYPE_DECL,
...
get_unwidened (tree op, ttype_p for_type)
{
  ...
  unsigned final_prec
    = TYPE_PRECISION (for_type != 0 ? TTYPE (for_type) : type);
  int uns
...
bool
int_fits_type_p (const_tree c, const ttype_p tt)
{
  ttype *type = tt;
...
retry:
  ...
  if (!TYPE_UNSIGNED (type) && sgn_c == UNSIGNED)
    {
      unsigned int prec = GET_MODE_PRECISION (TYPE_MODE (TREE_TYPE (c))) - 1;
...
/* Given TYPE, return the scope in which it was declared, or
   NULL_TREE if there is no containing scope.  */

tree
get_containing_scope (const ttype *t)
{
  return TYPE_CONTEXT (t);
}
...
{
  return (TYPE_P (t) ? get_containing_scope (as_a<const ttype *> (t))
		     : DECL_CONTEXT (t));
}
...
make_vector_type (ttype *innertype, int nunits, machine_mode mode)
{
  ttype *t;
  inchash::hash hstate;
  ttype *mv_innertype = TYPE_MAIN_VARIANT (innertype);

  t = make_type_node (VECTOR_TYPE);
  SET_TREE_TYPE (t, mv_innertype);
  ...
  if (TYPE_STRUCTURAL_EQUALITY_P (mv_innertype) || in_lto_p)
    ...
    SET_TYPE_CANONICAL (t, make_vector_type (TYPE_CANONICAL (mv_innertype),
					     nunits, VOIDmode));
...
static ttype *
build_atomic_base (ttype *type, unsigned int align)
{
  ttype *t;
...
build_common_tree_nodes (bool signed_char, bool short_double)
{
  ...
  SET_TREE_TYPE (error_mark_node, error_type_node);

  initialize_sizetypes ();

  /* Define both `signed char' and `unsigned char'.  */
  signed_char_type_node = make_signed_type (CHAR_TYPE_SIZE);
  SET_TYPE_STRING_FLAG (signed_char_type_node, true);
  unsigned_char_type_node = make_unsigned_type (CHAR_TYPE_SIZE);
  SET_TYPE_STRING_FLAG (unsigned_char_type_node, true);
  ...
  SET_TYPE_STRING_FLAG (char_type_node, true);
  ...
  for (i = 0; i < NUM_INT_N_ENTS; i ++)
    {
      ...
      SET_TYPE_SIZE (int_n_trees[i].signed_type,
		     bitsize_int (int_n_data[i].bitsize));
      SET_TYPE_SIZE (int_n_trees[i].unsigned_type,
		     bitsize_int (int_n_data[i].bitsize));
  ...
  SET_TYPE_PRECISION (boolean_type_node, 1);
  SET_TYPE_MAX_VALUE (boolean_type_node, build_int_cst (boolean_type_node, 1));
  ...
  SET_TYPE_USER_ALIGN (void_type_node, 0);

  void_node = make_node (VOID_CST);
  SET_TREE_TYPE (void_node, void_type_node);
  ...
  SET_TYPE_PRECISION (float_type_node, FLOAT_TYPE_SIZE);
  layout_type (float_type_node);

  double_type_node = make_type_node (REAL_TYPE);
  if (short_double)
    SET_TYPE_PRECISION (double_type_node, FLOAT_TYPE_SIZE);
  else
    SET_TYPE_PRECISION (double_type_node, DOUBLE_TYPE_SIZE);
  layout_type (double_type_node);

  long_double_type_node = make_type_node (REAL_TYPE);
  SET_TYPE_PRECISION (long_double_type_node, LONG_DOUBLE_TYPE_SIZE);
  ...
  SET_TYPE_PRECISION (dfloat32_type_node, DECIMAL32_TYPE_SIZE);
  layout_type (dfloat32_type_node);
  SET_TYPE_MODE (dfloat32_type_node, SDmode);
  dfloat32_ptr_type_node = build_pointer_type (dfloat32_type_node);

  dfloat64_type_node = make_type_node (REAL_TYPE);
  SET_TYPE_PRECISION (dfloat64_type_node, DECIMAL64_TYPE_SIZE);
  layout_type (dfloat64_type_node);
  SET_TYPE_MODE (dfloat64_type_node, DDmode);
  dfloat64_ptr_type_node = build_pointer_type (dfloat64_type_node);

  dfloat128_type_node = make_type_node (REAL_TYPE);
  SET_TYPE_PRECISION (dfloat128_type_node, DECIMAL128_TYPE_SIZE);
...
build_opaque_vector_type (ttype_p innertype, int nunits)
{
  ...
  SET_TYPE_VECTOR_OPAQUE (cand, true);
  SET_TYPE_CANONICAL (cand, TYPE_CANONICAL (t));
  SET_TYPE_NEXT_VARIANT (cand, TYPE_NEXT_VARIANT (t));
  SET_TYPE_NEXT_VARIANT (t, cand);
  SET_TYPE_MAIN_VARIANT (cand, TYPE_MAIN_VARIANT (t));
  return cand;
}
...
tree
build_call_nary (ttype_p return_type, tree fn, int nargs, ...)
{
  tree ret;
  va_list args;
  va_start (args, nargs);
...
tree
build_call_valist (ttype_p return_type, tree fn, int nargs, va_list args)
{
  tree t;
  int i;
...
tree
build_call_array_loc (location_t loc, ttype_p return_type, tree fn,
...
tree
build_call_vec (ttype_p return_type, tree fn, vec<tree, va_gc> *args)
{
  tree ret, t;
  unsigned int ix;
		  ...
		  hash_set<tree> *pset, walk_tree_lh lh)
{
  ...
  switch (TREE_CODE (type))
    ...
    case COMPLEX_TYPE:
      WALK_SUBTREE_PTR (TREE_TYPE_PTR (type));
      break;

    case METHOD_TYPE:
      WALK_SUBTREE_PTR (TYPE_METHOD_BASETYPE_PTR (type));
	...
	WALK_SUBTREE_PTR (TREE_TYPE_PTR (type));
      WALK_SUBTREE_PTR (TYPE_DOMAIN_PTR (type));
      break;

    case OFFSET_TYPE:
      WALK_SUBTREE_PTR (TREE_TYPE_PTR (type));
      WALK_SUBTREE_PTR (TYPE_OFFSET_BASETYPE_PTR (type));
      break;
...
#define WALK_SUBTREE_TAIL_PTR(NODE)				\
  do							\
    {							\
       tp = (NODE);					\
       goto tail_recurse;				\
  ...
  switch (code)
      ...
      if (TREE_CODE (DECL_EXPR_DECL (*tp)) == TYPE_DECL)
	{
	  ...
	  /* Same for scalar types.  */
	    ...
	    {
	      WALK_SUBTREE_PTR (TYPE_MIN_VALUE_PTR (*type_p));
	      WALK_SUBTREE_PTR (TYPE_MAX_VALUE_PTR (*type_p));
	    }

	  WALK_SUBTREE_PTR (TYPE_SIZE_PTR (*type_p));
	  WALK_SUBTREE_TAIL_PTR (TYPE_SIZE_UNIT_PTR (*type_p));
	}
      /* FALLTHRU */
...
obj_type_ref_class (const_tree ref)
{
  ...
  t = TREE_VALUE_TYPE (TYPE_ARG_TYPES (t));
  gcc_checking_assert (TREE_CODE (t) == POINTER_TYPE);
  return TREE_TYPE (t);
}
...
tree
get_binfo_at_offset (tree binfo, HOST_WIDE_INT offset, ttype_p expected_type)
{
...
array_ref_element_size (tree exp)
{
  ...
  ttype *elmt_type = TREE_TYPE (TREE_TYPE (TREE_OPERAND (exp, 0)));
...
{
  ttype *domain_type = TYPE_DOMAIN (TREE_TYPE (TREE_OPERAND (exp, 0)));
...
static bool
verify_type_variant (const ttype *t, ttype *tv)
{
...
bool
gimple_canonical_types_compatible_p (const ttype_p tt1, const ttype_p tt2,
				     bool trust_type_canonical)
{
  ...
  switch (TREE_CODE (t1))
	...
	{
	  ttype *i1 = TYPE_DOMAIN (t1);
	  ttype *i2 = TYPE_DOMAIN (t2);
...
void
verify_type (const ttype_p t)
{
  bool error_found = false;
  ttype *mv = TYPE_MAIN_VARIANT (t);
  if (!mv)
    {
