...
#define TREE_TARGET_GLOBALS(NODE) \
  (TARGET_OPTION_NODE_CHECK (NODE)->u.target_option.globals)
			...
			const char *__f, int __l, const char *__g)
{
  ...
  if (__i < 0 || __i >= __t->u.base.u.int_length.extended)
    tree_int_cst_elt_check_failed (__i, __t->u.base.u.int_length.extended,
				   __f, __l, __g);
  return &CONST_CAST_TREE (__t)->u.int_cst.val[__i];
}
                    ...
                    const char *__f, int __l, const char *__g)
{
  ...
  return &CONST_CAST_TREE (__t)->u.vec.a[__i];
}
                      ...
                      const char *__f, int __l, const char *__g)
{
  ...
  if (__i < 0 || __i >= omp_clause_num_ops [__t->u.omp_clause.code])
    omp_clause_operand_check_failed (__i, __t, __f, __l, __g);
  return CONST_CAST (const_tree *, &__t->u.omp_clause.ops[__i]);
}
                    ...
                    const char *__f, int __l, const char *__g)
{
  ...
  return &CONST_CAST_TREE (__u)->u.exp.operands[__i];
}
                    ...
                    const char *__f, int __l, const char *__g)
{
  ...
  if (__i < 0 || __i >= TREE_OPERAND_LENGTH (__u))
    tree_operand_check_failed (__i, __u, __f, __l, __g);
  return CONST_CAST (const_tree *, &__u->u.exp.operands[__i]);
}
                         ...
                         const char *__f, int __l, const char *__g)
{
  ...
  if (__i < 0 || __i >= TREE_OPERAND_LENGTH (__t))
    tree_operand_check_failed (__i, __t, __f, __l, __g);
  return CONST_CAST (const_tree *, &__t->u.exp.operands[__i]);
}
...
#define FLOATN_TYPE_NODE(IDX)		global_types[TPI_FLOATN_TYPE_FIRST + (IDX)]
#define FLOATN_NX_TYPE_NODE(IDX)	global_types[TPI_FLOATN_NX_TYPE_FIRST + (IDX)]
#define FLOATNX_TYPE_NODE(IDX)		global_types[TPI_FLOATNX_TYPE_FIRST + (IDX)]
...
#define float16_type_node		global_types[TPI_FLOAT16_TYPE]
#define float32_type_node		global_types[TPI_FLOAT32_TYPE]
#define float64_type_node		global_types[TPI_FLOAT64_TYPE]
#define float128_type_node		global_types[TPI_FLOAT128_TYPE]
#define float32x_type_node		global_types[TPI_FLOAT32X_TYPE]
#define float64x_type_node		global_types[TPI_FLOAT64X_TYPE]
#define float128x_type_node		global_types[TPI_FLOAT128X_TYPE]
...
#define COMPLEX_FLOATN_NX_TYPE_NODE(IDX)	global_types[TPI_COMPLEX_FLOATN_NX_TYPE_FIRST + (IDX)]
...
#define const_tm_ptr_type_node		global_types[TPI_CONST_TM_PTR_TYPE]
...
extern tree build_call_array_loc (location_t, ttype_p, tree, int, const tree *);
...
extern ttype *build_complex_type (ttype_p, bool named = false);
...
extern bool check_lang_type (const ttype_p cand, const ttype_p base);
...
extern tree size_in_bytes_loc (location_t, const ttype_p);
inline tree
size_in_bytes (const ttype_p t)
{
...
enum tree_node_structure_enum tree_node_structure (const union tree_node_u *);
...
extern bitmap get_nonnull_args (const ttype_p);
