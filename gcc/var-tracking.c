...
static void vars_clear			PARAMS ((htab_t));
static variable unshare_variable	PARAMS ((dataflow_set *, variable));
static int vars_copy_1			PARAMS ((void **, void *));
static void vars_copy			PARAMS ((htab_t, htab_t));
static void var_reg_delete_and_set	PARAMS ((dataflow_set *, rtx));
static void var_reg_delete		PARAMS ((dataflow_set *, rtx));
static void var_regno_delete		PARAMS ((dataflow_set *, int));
static void var_mem_delete_and_set	PARAMS ((dataflow_set *, rtx));
static void var_mem_delete		PARAMS ((dataflow_set *, rtx));

static void dataflow_set_init		PARAMS ((dataflow_set *, int));
static void dataflow_set_clear		PARAMS ((dataflow_set *));
static void dataflow_set_copy		PARAMS ((dataflow_set *,
						 dataflow_set *));
static int variable_union_info_cmp_pos	PARAMS ((const void *, const void *));
...
static bool variable_part_different_p	PARAMS ((variable_part *,
						 variable_part *));
static bool variable_different_p	PARAMS ((variable, variable, int));
...
static void dump_vars			PARAMS ((htab_t));
...
static void delete_variable_part	PARAMS ((dataflow_set *, rtx, tree,
						 HOST_WIDE_INT));
static int emit_note_insn_var_location	PARAMS ((void **, void *));
static void emit_notes_for_changes	PARAMS ((rtx, enum emit_note_where));
static int emit_notes_for_differences_1	PARAMS ((void **, void *));
static int emit_notes_for_differences_2	PARAMS ((void **, void *));
static void emit_notes_for_differences	PARAMS ((rtx, dataflow_set *,
						 dataflow_set *));
static void emit_notes_in_bb		PARAMS ((basic_block));
static void vt_emit_notes		PARAMS ((void));

static bool vt_get_decl_and_offset	PARAMS ((rtx, tree *,
						 HOST_WIDE_INT *));
static void vt_add_function_parameters	PARAMS ((void));
static void vt_initialize		PARAMS ((void));
static void vt_finalize			PARAMS ((void));
...
static rtx
adjust_stack_reference (mem, adjustment)
     rtx mem;
...
static void
dataflow_set_init (set, vars_size)
     dataflow_set *set;
     int vars_size;
{
...
static int
variable_union_info_cmp_pos (n1, n2)
     const void *n1;
     const void *n2;
{
...
static bool
variable_different_p (var1, var2, compare_current_location)
     variable var1;
     variable var2;
     int compare_current_location;
{
  int i;
...
static int
dataflow_set_different_1 (slot, data)
     void **slot;
     void *data;
{
...
static int
dataflow_set_different_2 (slot, data)
     void **slot;
     void *data;
{
  htab_t htab = (htab_t) data;
...
static void
emit_notes_for_changes (insn, where)
     rtx insn;
     enum emit_note_where where;
{
  emit_note_data data;
...
static int
emit_notes_for_differences_1 (slot, data)
     void **slot;
     void *data;
{
...
static int
emit_notes_for_differences_2 (slot, data)
     void **slot;
     void *data;
{
...
static bool
vt_get_decl_and_offset (rtl, declp, offsetp)
     rtx rtl;
     tree *declp;
