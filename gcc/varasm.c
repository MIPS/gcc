...
/* Subroutine of init_varasm_status; also used to re-initialize the constant
   pool for an inline function.  */
...
void
init_varasm_status (p)
     struct function *p;
{
  p->varasm = (struct varasm_status *) xmalloc (sizeof (struct varasm_status));
  init_const_rtx_hash_table (p);
}
...
void
mark_varasm_state (arg)
     void *arg;
{
  struct varasm_status *p = *(struct varasm_status **)arg;
