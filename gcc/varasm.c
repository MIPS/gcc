...
static void asm_output_bss		PARAMS ((FILE *, tree, const char *, unsigned HOST_WIDE_INT, unsigned HOST_WIDE_INT));
#endif
...
static bool asm_emit_uninitialised	PARAMS ((tree, const char*, unsigned HOST_WIDE_INT, unsigned HOST_WIDE_INT));
void resolve_unique_section	PARAMS ((tree, int, int));
     ...
     const char *asmspec;
{
  ...
  if (name != new_name)
    {
      change_decl_assembler_name (decl, get_identifier (new_name));
