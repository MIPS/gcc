   ...
   && TARGET_LONG_DOUBLE_128"
{
  if (FLOAT128_IEEE_P (<MODE>mode))
  ...
  else
    {
	 ...
	 the proper constant.  */
      if (TARGET_VSX)
	zero = CONST0_RTX (DFmode);
      else
	{
	  zero = gen_reg_rtx (DFmode);
...
;; The extenddf<mode>2 expander only allows registers, but add memory
;; forms to eliminate extra copies.
(define_insn_and_split "extenddf<mode>2_fprs"
  [(set (match_operand:IBM128 0 "nonimmediate_operand" "=m,Y,d,d,&d")
       (float_extend:IBM128 (match_operand:DF 1 "input_operand" "d,r,ws,md,md")))
  ...
  [(pc)]
{
  ...
  const machine_mode mode = <MODE>mode;
  emit_move_insn (simplify_gen_subreg (DFmode, operands[0], mode, hi_word),
		  operands[1]);
  emit_move_insn (simplify_gen_subreg (DFmode, operands[0], mode, lo_word),
		  operands[2]);
  DONE;
})
...
(define_expand "extendsf<mode>2"
  [(set (match_operand:IBM128 0 "gpc_reg_operand" "")
	(float_extend:IBM128 (match_operand:SF 1 "gpc_reg_operand" "")))]
