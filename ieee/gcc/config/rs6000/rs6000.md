   ...
   && TARGET_LONG_DOUBLE_128"
{
  if (FLOAT128_IEEE_P (<MODE>mode))
  ...
  else
    {
      ...
      if (<MODE>mode == TFmode)
	emit_insn (gen_extenddftf2_fprs (operands[0], operands[1], tmp));
      else if (<MODE>mode == IFmode)
	emit_insn (gen_extenddfif2_fprs (operands[0], operands[1], tmp));
      else
	gcc_unreachable ();
    }
  DONE;
...
(define_insn_and_split "extenddf<mode>2_fprs"
  [(set (match_operand:IBM128 0 "nonimmediate_operand" "=d,&d,m")
	(float_extend:IBM128
	 (match_operand:DF 1 "input_operand" "m,dm,d")))
   (use (match_operand:DF 2 "input_operand" "m,dm,d"))]
  "TARGET_HARD_FLOAT && TARGET_FPRS && TARGET_DOUBLE_FLOAT 
...
(define_insn_and_split "extenddf<mode>2_vsx"
  [(set (match_operand:IBM128 0 "gpc_reg_operand" "=d,d")
	(float_extend:IBM128
	 (match_operand:DF 1 "input_operand" "ws,m")))]
  "TARGET_LONG_DOUBLE_128 && TARGET_VSX"
  "#"
  "&& reload_completed"
  [(pc)]
