...
;; Specialized rotate-and-mask and insert acting on either GPRs or vector
;; registers with ISA 3.0
(define_insn "*rotl<mode>3_mask_p9"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r,wJwK,wK")
	(and:GPR (match_operator:GPR 4 "rotate_mask_operator"
		  [(match_operand:GPR 1 "gpc_reg_operand" "r,wJwK,wK")
		   (match_operand:SI 2 "vextract_shift_operand" "n,n,n")])
		 (match_operand:SI 3 "vextract_mask_operand" "n,wW,n")))]
  "TARGET_P9_VECTOR && TARGET_VSX_SMALL_INTEGER"
{
  ...
  else if (VSX_REGNO_P (r))
    {
      HOST_WIDE_INT shift = INTVAL (operands[2]) / 8;
      HOST_WIDE_INT mask = INTVAL (operands[3]);

      if (GET_MODE_SIZE (<MODE>mode) < GET_MODE_SIZE (DImode))
	shift += GET_MODE_SIZE (DImode) - GET_MODE_SIZE (<MODE>mode);

      operands[2] = GEN_INT (shift);
      if (mask == 0xffffffff)
	return "xxextractuw %x0,%x1,%2";
      else if (mask == 0xffff)
	return "vextractuh %0,%1,%2";
      else if (mask == 0xff)
	return "vextractub %0,%1,%2";
      else
	gcc_unreachable ();
    }
  else
  ...
  [(set_attr "type" "shift,vecperm,vecperm")])

;; Generalized rotate-and-mask and insert acting on GPRs
  ...
  [(const_int 0)]
{
  ...
  if (MEM_P (input) || altivec_register_operand (input, <QHI:MODE>mode))
    ...
    {
      rtx tmp = GET_CODE (operands[3]) == SCRATCH ? di : operands[3];
