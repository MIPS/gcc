  ...
  if (ws->chunk_size == 0)
	...
	return 1;
      ...
      /* Compute the "zero-based" start and end points.  That is, as
         if the loop began at zero and incremented by one.  We begin
         with the straight quotient and adjust that to evenly 
         distribute the remainder amongst the lowest numbered threads.  */
      s0 = q * i;
      if (r)
        s0 += i > r ? r : i;
      e0 = s0 + q;
      ...
      s = s0 * ws->incr + ws->next;
      e = e0 * ws->incr + ws->next;
      ...
      if (s == e)
	{
      ...
      *pstart = s;
      *pend = e;
      thr->ts.static_trip = (e == ws->end ? -1 : 1);
      return 0;
    }
  else
    {
      unsigned long n, s0, n0, i, c;
      long s, e;
