...
(define_insn "*mov<GPR:mode>_mfcr_shift2"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(unspec:GPR [(match_operand:GPR 1 "gpc_reg_operand" "r")
		     (match_operand:GPR 2 "const_int_operand" "n")
		    ...
		    UNSPEC_MFCRF_EXTRACT2))]
  "TARGET_MFCRF_SPLIT"
  "{rlinm|rlwinm} %0,%1,%2,%3,%3\t\t# meissner5"
  [(set_attr "type" "shift")])
...
(define_insn "*mov<GPR:mode>_mfcr<CR:mode>_setcr"
  [(set (match_operand:CC 0 "cc_reg_operand" "=x,?y")
	(compare:CC (match_operator:GPR 1 "scc_comparison_operator"
				[(match_operand:CR 2 "cc_reg_operand" "y,y")
				 (const_int 0)])
		    (const_int 0)))
   (set (match_operand:GPR 3 "gpc_reg_operand" "=r,r")
...
(define_insn "*mov<GPR:mode>_mfcr<CR:mode>_shift2"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(ashift:GPR
	 (match_operator:GPR 1 "scc_comparison_operator"
			     [(match_operand:CR 2 "cc_reg_operand" "y")
  ...
  ""
{
  int is_bit, put_bit, count, int_size, int_size_m1;

  if (TARGET_MFCRF_SPLIT)
    return "#";

  int_size = (<GPR:MODE>mode == DImode) ? 64 : 32;
  int_size_m1 = int_size - 1;
  is_bit = ccr_bit (operands[1], 1);
  put_bit = int_size_m1 - (INTVAL (operands[3]) & int_size_m1);

  if (is_bit >= put_bit)
    count = is_bit - put_bit;
  else
    count = int_size - (put_bit - is_bit);
