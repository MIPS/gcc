  ...
  "TARGET_BCP8"
{
  ...
  /* Power7 has a performance problem  if the comparison and bc+8 are back to
     back, and the bc+8 is at the end of a cache block, performance suffers,
     so in this case, we tell the assembler to insert a group ending nop.  */
  if (TARGET_BCP8_NOP)
    {
      rtx prev_insn = prev_nonnote_insn_bb (insn);
      if (prev_insn && GET_CODE (prev_insn) == INSN)
	{
	  rtx pattern = PATTERN (prev_insn);
	  if (GET_CODE (pattern) == SET
	      && GET_CODE (SET_SRC (pattern)) == COMPARE)
	    output_asm_insn (".p2alignl 5,0x60420000,4", operands);
	}
    }
