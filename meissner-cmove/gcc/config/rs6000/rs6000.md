...
(define_insn_and_split "smin<mode>3"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(smin:GPR (match_operand:GPR 1 "gpc_reg_operand" "%r")
		  (match_operand:GPR 2 "reg_or_short_operand" "r")))]
  "TARGET_IMINMAX"
  "#"
  ""
  [(const_int 0)]
  "
{
  ...
  if (TARGET_IMINMAX_POWER && <MODE>mode == SImode)
    {			/* Use power DOZ instruction.  */
      tmp = gen_reg_rtx (SImode);
      emit_insn (gen_dozsi3 (tmp, op1, op2));
      if (op2 == const0_rtx)
	emit_insn (gen_negsi2 (op0, tmp));
      else
	emit_insn (gen_subsi3 (op0, op2, tmp));
      DONE;
    }
  else if (TARGET_IMINMAX_ISEL || TARGET_IMINMAX_BCP8)
    {			/* Use ISEL or branch cond.+8.  */
      cc = gen_reg_rtx (CCmode);

      /* Special case 0 which can be loaded by ISEL, but we need to swap the
	 sense of the test.  */
      if (op2 == CONST0_RTX (<MODE>mode) && TARGET_IMINMAX_ISEL)
	{
	  cmp = gen_rtx_COMPARE (CCmode, op1, op2);
	  op2 = op1;
	  op1 = CONST0_RTX (<MODE>mode);
	  cond = gen_rtx_GT (VOIDmode, cc, const0_rtx);
	}
      else
	{
	  op2 = force_reg (<MODE>mode, op2);
	  cmp = gen_rtx_COMPARE (CCmode, op1, op2);
	  cond = gen_rtx_LT (VOIDmode, cc, const0_rtx);
	}

      emit_insn (gen_rtx_SET (VOIDmode, cc, cmp));
			      
      if (TARGET_IMINMAX_ISEL && TARGET_ISEL)
	emit_insn (gen_isel_signed_<mode> (op0, cond, op1, op2, cc));
      else if (TARGET_ISEL_LIMITED)
	emit_insn (gen_isel_limited_signed_<mode> (op0, cond, op1, op2, cc,
		   GEN_INT (ISEL_IMINMAX)));
      else if (TARGET_IMINMAX_BCP8)
	emit_insn (gen_bcp8_<mode> (op0, cond, cc, op1, op2));
      else
	gcc_unreachable ();
      DONE;
    }
  ...
  if (TARGET_IMINMAX_POWER && <MODE>mode == SImode)
    {
      ...
      op2 = force_reg (<MODE>mode, op2);
      emit_insn (gen_dozsi3 (tmp, op1, op2));
      emit_insn (gen_addsi3 (op0, tmp, op1));
      DONE;
    }
  else if (TARGET_IMINMAX_ISEL || TARGET_IMINMAX_BCP8)
    {			/* Use ISEL or branch cond.+8.  */
  ...
  if (TARGET_IABS_ISEL)		/* use ISEL instruction.  */
    {
      ...
      if (TARGET_ISEL)
	emit_insn (gen_isel_signed_<mode> (op0, lt, tmp1, op1, cc));
      else if (TARGET_ISEL_LIMITED)
	emit_insn (gen_isel_limited_signed_<mode> (op0, lt, tmp1, op1, cc,
		   GEN_INT (ISEL_IABS)));
      else
	gcc_unreachable ();
      DONE;
  ...
  if (TARGET_IABS_ISEL)		/* use ISEL instruction.  */
    {
      ...
      if (TARGET_ISEL)
	emit_insn (gen_isel_signed_<mode> (op0, lt, op1, tmp1, cc));
      else if (TARGET_ISEL_LIMITED)
	emit_insn (gen_isel_limited_signed_<mode> (op0, lt, op1, tmp1, cc,
		   GEN_INT (ISEL_IABS)));
      else
	gcc_unreachable ();
      DONE;
...
;; Isel patterns where we want to use ISEL for specific operations (abs,
;; comparison, conditional move), but we don't want to generate ISEL by
;; default.
(define_insn "isel_limited_signed_<mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r,r")
	(if_then_else:GPR
	 (match_operator 1 "scc_comparison_operator"
			 [(match_operand:CC 4 "cc_reg_operand" "y,y")
  ...
  "ISEL_LIMITED_OK_P (INTVAL (operands[5]))"
  "*
{ return output_isel (operands); }"
  [(set_attr "type" "isel")
   (set_attr "length" "4")])

(define_insn "*isel_limited_reversed_signed_<mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(if_then_else:GPR
	 (match_operator 1 "scc_rev_comparison_operator"
			 [(match_operand:CC 4 "cc_reg_operand" "y")
	 ...
	 (match_operand:GPR 2 "gpc_reg_operand" "b")
	 (match_operand:GPR 3 "gpc_reg_operand" "b")))
   (use (match_operand 5 "const_int_operand" "i"))]
  "ISEL_LIMITED_OK_P (INTVAL (operands[5]))"
  "*
{ return output_isel (operands); }"
  [(set_attr "type" "isel")
