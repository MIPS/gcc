...
; Same as GPR, but a different name allows iterators on two different types
...
(define_expand "smin<mode>3"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "")
	(smin:GPR (match_operand:GPR 1 "gpc_reg_operand" "")
		  (match_operand:GPR 2 "reg_or_short_operand" "")))]
  "TARGET_INT_MINMAX"
{
  if (!rs6000_expand_int_minmax (operands[0], LT, operands[1], operands[2]))
    FAIL;
  DONE;
})
...
(define_expand "smax<mode>3"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "")
	(smax:GPR (match_operand:GPR 1 "gpc_reg_operand" "")
		  (match_operand:GPR 2 "reg_or_short_operand" "")))]
  "TARGET_INT_MINMAX"
{
  if (!rs6000_expand_int_minmax (operands[0], GT, operands[1], operands[2]))
    FAIL;
  DONE;
})
...
(define_expand "umin<mode>3"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "")
	(umin:GPR (match_operand:GPR 1 "gpc_reg_operand" "")
		  (match_operand:GPR 2 "gpc_reg_operand" "")))]
  "TARGET_INT_MINMAX"
{
  if (!rs6000_expand_int_minmax (operands[0], LTU, operands[1], operands[2]))
    FAIL;
  DONE;
})
...
(define_expand "umax<mode>3"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "")
	(umax:GPR (match_operand:GPR 1 "gpc_reg_operand" "")
		  (match_operand:GPR 2 "gpc_reg_operand" "")))]
  "TARGET_INT_MINMAX"
{
  if (!rs6000_expand_int_minmax (operands[0], GTU, operands[1], operands[2]))
    FAIL;
  DONE;
})
...
(define_insn_and_split "abs<mode>2"
  ...
  "
{
  if (!rs6000_expand_int_abs (operands[0], operands[1], false))
    FAIL;
  DONE;
}")
...
(define_insn_and_split "*nabs<mode>2"
  ...
  "
{
  if (!rs6000_expand_int_abs (operands[0], operands[1], true))
    FAIL;
  DONE;
}")
...
(define_expand "mov<mode>cc"
   [(set (match_operand:GPR 0 "gpc_reg_operand" "")
	 (if_then_else:GPR (match_operand 1 "comparison_operator" "")
			   ...
			   (match_operand:GPR 3 "add_operand" "")))]
...
(define_expand "mov<mode>cc_internal"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "r")
	(if_then_else:GPR
  ...
  "TARGET_ISEL || TARGET_BCP8"
{
  ...
  enum machine_mode cmp_mode = GET_MODE (cmp);
  enum machine_mode cr_mode = GET_MODE (cr);
  bool int_compare_p = (cr_mode != CCFPmode);
  enum rtx_code rev_code = UNKNOWN;

  /* Isel can handle 0 as a constant in the true position.  */
  if (TARGET_ISEL)
    {
      if (scc_comparison_operator (cmp, cmp_mode))
	{
	  if (int_compare_p && op2 == const0_rtx
	      && ((rev_code = reverse_condition (cmp_code)) != UNKNOWN))
	    {
	      operands[2] = force_reg (<MODE>mode, op3);
	      operands[3] = const0_rtx;
	    }
	  else
	    {
	      if (op2 != const0_rtx)
		operands[2] = force_reg (<MODE>mode, op2);
	      operands[3] = force_reg (<MODE>mode, op3);
	    }
	}

      else if (scc_rev_comparison_operator (cmp, cmp_mode))
	{
	  if (int_compare_p && op3 == const0_rtx
	      && ((rev_code = reverse_condition (cmp_code)) != UNKNOWN))
	    {
	      operands[3] = force_reg (<MODE>mode, op2);
  ...
  /* Branch conditional + 8 can handle constants in the false position.  */
  else if (TARGET_BCP8)
    {
      if (int_compare_p && GET_CODE (op2) == CONST_INT
	  && (satisfies_constraint_I (op2)
	      || satisfies_constraint_L (op2))
	  && ((rev_code = reverse_condition (cmp_code)) != UNKNOWN))
	{
	  operands[2] = force_reg (<MODE>mode, op3);
	  operands[3] = op2;
  ...
  /* Reverse the comparison if desired.  */
  if (rev_code != UNKNOWN)
    operands[1] = gen_rtx_fmt_ee (rev_code, cmp_mode, copy_rtx_if_shared (cr),
				  const0_rtx);
})

;; Integer conditional move with the ISEL instruction
;; Use the BASE_REGS for the isel true field since r0 is replaced by 0.
...
;; We prefer to use volatile CRs (0, 1, 6, 7) over the saved CRs (2, 3, 4)
;; to reduce the chance that we will need to save and restore the CRs.

(define_insn "*bcp8<mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r,r,r,r")
	(if_then_else:GPR
	 (match_operator 1 "branch_comparison_operator"
			 [(match_operand 2 "cc_reg_operand" "wc,wc,??*y,??*y")
			  (const_int 0)])
	 (match_operand:GPR 3 "add_operand" "0,rIL,0,rIL")
	 (match_operand:GPR 4 "add_operand" "rIL,0,rIL,0")))]
  "TARGET_BCP8"
{
  bool reverse_p = (which_alternative == 1 || which_alternative == 3);
  bool emit_nop_p = rs6000_previous_compare_p (insn, operands[2]);

  return rs6000_output_bcp8 (operands[0], operands[1], operands[3],
			     operands[4], insn, reverse_p, emit_nop_p);
}
  [(set_attr "type" "isel")
   (set_attr "isel_type" "bcp8")
   (set (attr "length") (if_then_else (match_test "TARGET_BCP8_NOP")
				      (const_string "12")
				      (const_string "8")))])

;; Use the combiner to try and keep the comparison next to the jump for branch
;; conditional + 8 support.

(define_insn "*bcp8<GPR:mode>_cmp<GPR2:mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r,r,r,r")
	(if_then_else:GPR
	 (match_operator 1 "bcp8_comparison_operator"
	    [(match_operand:GPR2 2 "gpc_reg_operand" "r,r,r,r")
	     (match_operand:GPR2 3 "reg_or_cint_operand" "rIK,rIK,rIK,rIK")])
	 (match_operand:GPR 4 "add_operand" "0,rIL,0,rIL")
	 (match_operand:GPR 5 "add_operand" "rIL,0,rIL,0")))
   (clobber (match_scratch:CC 6 "=wc,wc,??*y,??*y"))]
  "TARGET_BCP8 && TARGET_BCP8_MERGE
   && ((GET_CODE (operands[3]) != CONST_INT)
       || (signed_comparison_operator (operands[1], GET_MODE (operands[2]))
	   && satisfies_constraint_I (operands[3]))
       || (unsigned_comparison_operator (operands[1], GET_MODE (operands[1]))
	   && satisfies_constraint_K (operands[3])))"
{
  ...
  if (signed_comparison_operator (cc, GET_MODE (cc)))
    output_asm_insn ("{cmp%I3|cmp<GPR2:wd>%I3} %6,%2,%3", operands);

  else if (unsigned_comparison_operator (cc, GET_MODE (cc)))
    {
      output_asm_insn ("{cmpl%I3|cmpl<GPR2:wd>%I3} %6,%2,%b3", operands);
      cr = gen_rtx_SUBREG (CCUNSmode, cr, 0);
    }

  else
    gcc_unreachable ();

  cond = gen_rtx_fmt_ee (GET_CODE (cc), GET_MODE (cr), cr, const0_rtx);
  return rs6000_output_bcp8 (operands[0], cond, operands[4], operands[5],
			     insn, reverse_p, true);
}
  [(set_attr "type" "isel")
   (set_attr "isel_type" "bcp8")
...
(define_insn "*mov<mode>_mfcr"
  [(set (match_operand:CR 0 "gpc_reg_operand" "=r")
	(unspec:CR [(match_operand:CR 1 "cc_reg_not_cr0_operand" "y")]
		   UNSPEC_MFCRF))]
  ""
  "mfcr %0%Q1\t\t# meissner1"
  [(set (attr "type")
...
(define_insn "*mov<mode>_shift"
  [(set (match_operand:CR 0 "gpc_reg_operand" "=r")
	(unspec:CR [(match_operand:CR 1 "gpc_reg_operand" "r")
		    (match_operand:CR 2 "cc_reg_not_cr0_operand" "y")]
		   UNSPEC_MFCRF_EXTRACT))]
  ""
  "{rlinm|rlwinm} %0,%1,%f2,0xf0000000\t\t# meissner2"
  [(set_attr "type" "shift")])

(define_split
  [(set (match_operand:CR 0 "gpc_reg_operand" "")
	(match_operand:CR 1 "cc_reg_not_cr0_operand" ""))]
  "reload_completed && INT_REGNO_P (REGNO (operands[0]))"
  [(set (match_dup 0) (unspec:CR [(match_dup 1)]
				 UNSPEC_MFCRF))
   (set (match_dup 0) (unspec:CR [(match_dup 0)
				  (match_dup 1)]
				 UNSPEC_MFCRF_EXTRACT))])
...
;;
;; On newer machines with mfcrf, we try to split the move from CR instruction
;; with the shift to extract the value.

(define_insn "*mov<GPR:mode>_mfcr<CR:mode>"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(match_operator:GPR 1 "scc_comparison_operator"
			    [(match_operand:CR 2 "cc_reg_operand" "y")
			     (const_int 0)]))]
  ""
{
  return (TARGET_MFCRF_SPLIT) ? "#" : "mfcr %0%Q2\;{rlinm|rlwinm} %0,%0,%J1,1";
}
  [(set (attr "type")
	(if_then_else (match_test "TARGET_MFCRF")
		      (const_string "mfcrf")
...
(define_insn "*mov<GPR:mode>_mfcr<CR:mode>_move"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(unspec:GPR [(match_operand:CR 1 "cc_reg_operand" "y")]
		    UNSPEC_MFCRF))]
  "TARGET_MFCRF_SPLIT"
  "mfcr %0%Q1\t\t# meissner3"
  [(set (attr "type")
...
(define_insn "*mov<GPR:mode>_mfcr_shift"
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(unspec:GPR [(match_operand:GPR 1 "gpc_reg_operand" "r")
		     (match_operand:GPR 2 "const_int_operand" "n")]
		    UNSPEC_MFCRF_EXTRACT))]
  "TARGET_MFCRF_SPLIT"
  "{rlinm|rlwinm} %0,%1,%2,1\t\t# meissner4"
  [(set_attr "type" "shift")])
  ...
  [(set (match_operand:GPR 0 "gpc_reg_operand" "=r")
	(match_operator:GPR 1 "scc_comparison_operator"
			    [(match_operand 2 "cc_reg_operand" "y")
			     (const_int 0)]))]
  "reload_completed && TARGET_MFCRF_SPLIT"
  [(set (match_dup 0) (unspec:GPR [(match_dup 2)]
				  UNSPEC_MFCRF))
   (set (match_dup 0) (unspec:GPR [(match_dup 0)
				   (match_dup 3)]
				  UNSPEC_MFCRF_EXTRACT))]
{
  operands[3] = GEN_INT (print_operand_cr_bits (operands[1], 'J'));
})
...
;; There is a 3 cycle delay between consecutive mfcr instructions so it is
;; useful to combine 2 scc instructions to use only one mfcr.  However for
;; MFCRF machines we don't want to do this, since it is likely each CR register
;; is internally a separate register, and doing the general MCRF instruction
;; forces the machine to wait for 8 CR's to be available before doing the move.
   ...
   (set (match_operand:SI 3 "gpc_reg_operand" "=r")
	(match_operator:SI 4 "scc_comparison_operator"
			   [(match_operand 5 "cc_reg_operand" "y")
			    (const_int 0)]))]
  "!TARGET_MFCRF && REGNO (operands[2]) != REGNO (operands[5])"
...
(define_insn ""
  ...
  "*
{
  return output_cbranch (operands[1], \"%l0\", 0, insn, true);
}"
  [(set_attr "type" "branch")])
...
(define_insn ""
  ...
  "*
{
  return output_cbranch (operands[0], NULL, 0, insn, true);
}"
  [(set_attr "type" "jmpreg")
   (set_attr "length" "4")])
...
(define_insn ""
  ...
  "*
{
  return output_cbranch (operands[1], \"%l0\", 1, insn, true);
}"
  [(set_attr "type" "branch")])
...
(define_insn ""
  ...
  "*
{
  return output_cbranch (operands[0], NULL, 1, insn, true);
}"
  [(set_attr "type" "jmpreg")
   (set_attr "length" "4")])
...
;; Possibly use a single field move instruction with a mask instead of the mfcr
;; that moves all of the registers, which can be slow on machines that provide
;; renamed registers for each CR register.

(define_expand "movesi_from_cr"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r,r")
...
(define_insn "*movesi_from_cr_internal"
  [(set (match_operand:SI 0 "gpc_reg_operand" "=r,r")
        (unspec:SI [(reg:CC CR0_REGNO) (reg:CC CR1_REGNO)
		    (reg:CC CR2_REGNO) (reg:CC CR3_REGNO)
		    (reg:CC CR4_REGNO) (reg:CC CR5_REGNO)
		    (reg:CC CR6_REGNO) (reg:CC CR7_REGNO)
		    (match_operand:SI 1 "const_int_operand" "O,i")]
		   UNSPEC_MOVESI_FROM_CR))]
  ""
  "@
   mfcr %0
