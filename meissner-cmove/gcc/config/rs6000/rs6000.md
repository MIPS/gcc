  ...
  "TARGET_BCP8"
{
  rtx xoperands[2];
  rtx op;
  int reverse_p = (which_alternative == 1);

  xoperands[0] = operands[0];
  xoperands[1] = op = (reverse_p) ? operands[3] : operands[4];

  output_asm_insn (output_cbranch (operands[1], \"1f\", reverse_p, insn, false),
		   operands);
  if (GET_CODE (op) == CONST_INT)
    {
      if (satisfies_constraint_I (op))
	output_asm_insn ("{lil|li} %0,%1", xoperands);
      else if (satisfies_constraint_L (op))
	output_asm_insn ("{liu|lis} %0,%v1", xoperands);
      else
	gcc_unreachable ();
    }
  else
    output_asm_insn ("mr %0,%1", xoperands);
  ...
  [(set_attr "type" "isel")
   ...
   (set (attr "length") (if_then_else (match_test "TARGET_COMPARE_NOP")
				      (const_string "12")
				      (const_string "8")))])
...
(define_insn "*cmp<mode>_internal1"
  ...
  ""
{
  const char *cmp_str = "{cmp%I2|cmp<wd>%I2} %0,%1,%2";
  if (TARGET_COMPARE_NOP && GET_MODE (operands[0]) != CCFPmode)
    {
      output_asm_insn (cmp_str, operands);
      return rs6000_output_compare_nop (REGNO (operands[0]), insn);
    }
  else
    return cmp_str;
}
  [(set_attr "type" "cmp")
   (set (attr "length")
	(if_then_else (match_test "TARGET_COMPARE_NOP")
		      (const_string "8")
		      (const_string "4")))])
...
(define_insn "*cmpsi_internal2"
  ...
  ""
{
  const char *cmp_str = "{cmpl%I2|cmplw%I2} %0,%1,%b2";
  if (TARGET_COMPARE_NOP && GET_MODE (operands[0]) != CCFPmode)
    {
      output_asm_insn (cmp_str, operands);
      return rs6000_output_compare_nop (REGNO (operands[0]), insn);
...
(define_insn "*cmpdi_internal2"
  ...
  ""
{
  const char *cmp_str = "cmpld%I2 %0,%1,%b2";
  if (TARGET_COMPARE_NOP && GET_MODE (operands[0]) != CCFPmode)
    {
      output_asm_insn (cmp_str, operands);
      return rs6000_output_compare_nop (REGNO (operands[0]), insn);
