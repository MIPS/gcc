...
;; Technically, BC+8 is not an isel, but it schedules like an ISEL (needing to
;; be in the first dispatch group, and having distance between the compare and
;; the branch).
  ...
  "*
{
  ...
  if (GET_CODE (operands[4]) == CONST_INT)
	...
	output_asm_insn (\"{cal %0,%4(0)|addi %0,0,%4}\", operands);
      else if (satisfies_constraint_L (operands[4]))
	output_asm_insn (\"{cau|addis} %0,0,%v4\", operands);
      else
	gcc_unreachable ();
    }
  else
