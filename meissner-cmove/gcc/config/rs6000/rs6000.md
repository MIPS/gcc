...
(define_constants
  [(STACK_POINTER_REGNUM	1)
   ...
   ; Flags for hiding ISEL and only generating it for specific ops
   ; BCP8 is not really an isel, but it looks like one at the RTL level
   ; Also some machines internally convert some/all bc+8 instructions
   ; into an ISEL
   (ISEL_GENERAL		0)
   (ISEL_BCP8			1)
   (ISEL_IABS			2)
...
;; If this is a branch conditional + 8 insn instead of an ISEL
...
;; Branch conditional+8 support.  Some machines have a special optimization
;; that can optimize a branch around a single instruction that is a move, store
;; add, and, or, or xor operation.  We don't expose this branch to flow control
;; to make sure that the compiler does not put anything else into the slot.
  ...
  "*
{
  ...
  [(set (attr "type")
	(if_then_else (eq_attr "cpu" "power7")
		      (const_string "isel")
		      (const_string "branch")))
