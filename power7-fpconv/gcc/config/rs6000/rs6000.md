...
;; On VSX, we can use xsrdpiz/xsmindp/xsmaxdp to avoid storing to memory
;; but only do it for higher optimization levels
(define_expand "roundintdfsi2"
  [(parallel [(set (match_operand:DF 0 "gpc_reg_operand" "")
		   (float:DF
...
{
  if (VECTOR_UNIT_VSX_P (DFmode) && optimize >= 3)
    {
      rtx reg1 = gen_reg_rtx (DFmode);
      rtx reg2 = gen_reg_rtx (DFmode);
      rtx rmin = force_reg (DFmode, CONST_DOUBLE_ATOF (\"-2147483648\", DFmode));
      rtx rmax = force_reg (DFmode, CONST_DOUBLE_ATOF (\"2147483647\", DFmode));
      emit_insn (gen_btruncdf2 (reg1, operands[1]));
      rs6000_emit_minmax (reg2, SMAX, reg1, rmin);
      rs6000_emit_minmax (operands[0], SMIN, reg2, rmax);
      DONE;
    }
})
