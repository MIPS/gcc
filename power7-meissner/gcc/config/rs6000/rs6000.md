  ...
  "
{
  ...
  if (rs6000_recip_div_p[SFmode] && !optimize_insn_for_size_p ())
    {
      emit_insn (gen_recipsf3 (operands[0], operands[1], operands[2]));
      DONE;
    }
}")
...
{
  /* Use reciprocal estimate if desired rather than normal division.  */
  if (rs6000_recip_div_p[DFmode] && !optimize_insn_for_size_p ())
    {
      emit_insn (gen_recipdf3 (operands[0], operands[1], operands[2]));
      DONE;
    }
}")
...
(define_insn "*rsqrtdf_internal1"
  [(set (match_operand:DF 0 "gpc_reg_operand" "=d")
	(unspec:DF [(match_operand:DF 1 "gpc_reg_operand" "d")]
		   UNSPEC_RSQRT))]
  "TARGET_RSQRT && !VECTOR_UNIT_VSX_P (DFmode)"
  "frsqrte %0,%1"
  [(set_attr "type" "fp")])
