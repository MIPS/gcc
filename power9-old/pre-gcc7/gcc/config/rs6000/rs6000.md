...
(define_mode_iterator SIGNBIT [(SF "TARGET_SF_FPR")
			       (DF "TARGET_DF_FPR")
			       (KF "FLOAT128_VECTOR_P (KFmode)")
			       (TF "FLOAT128_VECTOR_P (TFmode)")])

(define_mode_attr Fsignbit	[(SF "ww")
				 (DF "ws")
				 (KF "wa")
				 (TF "wa")])
       ...
       || (TARGET_POWERPC64 && TARGET_DIRECT_MOVE))"
{
  ...
  if (TARGET_POWERPC64 && TARGET_DIRECT_MOVE)
    {
      emit_insn (gen_trunc<mode>df2 (operands[2], operands[1]));
      emit_insn (gen_signbitdf2 (operands[0], operands[2]));
      DONE;
    }
...
(define_expand "signbit<mode>2"
  [(set (match_operand:SI 0 "gpc_reg_operand" "")
	(unspec:SI
	 [(match_operand:SFDF 1 "input_operand" "")]
	 UNSPEC_SIGNBIT))]
  "TARGET_POWERPC64 && TARGET_DIRECT_MOVE"
  "")
...
(define_insn_and_split "*signbit_<u>di_<mode>2_dm"
  [(set (match_operand:DI 0 "gpc_reg_operand" "=r,r,r")
	(any_extend:DI
	 (unspec:SI [(match_operand:SIGNBIT 1 "input_operand" "<Fsignbit>,m,r")]
		    UNSPEC_SIGNBIT)))]
  "TARGET_POWERPC64 && TARGET_DIRECT_MOVE"
  "#"
  "&& reload_completed"
...
;; Handle isinf on 64-bit machines that support direct move.  While in theory
;; it could be done on 32-bit systems or systems without direct move, it is
;; likely to be slower than testing abs(x) > HUGE_VAL that is generated if
;; we don't have isinf.
(define_expand "isinf<mode>2"
  [(use (match_operand:SI 0 "gpc_reg_operand" ""))
   (use (match_operand:SFDF 1 "gpc_reg_operand" ""))]
  "TARGET_POWERPC64 && TARGET_DIRECT_MOVE && can_create_pseudo_p ()"
{
  rtx op0 = operands[0];
  rtx op1 = operands[1];
  rtx op1_gpr = gen_reg_rtx (DImode);
  rtx op1_shift = gen_reg_rtx (DImode);
  rtx inf_const = gen_reg_rtx (DImode);
  rtx eq_reg = gen_reg_rtx (DImode);
  /* Infinity is an exponent of all 1's and all data bits 0.  The sign bit may
     be 0 or 1.  Shift left to eliminate the sign bit.  */
  unsigned HOST_WIDE_INT inf_shift = (0x7ff0000000000000UL << 1);

  if (<MODE>mode == SFmode)
    {
      rtx df_tmp = gen_reg_rtx (DFmode);
      emit_insn (gen_extendsfdf2 (df_tmp, op1));
      op1 = df_tmp;
    }

  emit_move_insn (op1_gpr, gen_lowpart (DImode, op1));
  emit_insn (gen_ashldi3 (op1_shift, op1_gpr, GEN_INT (1)));
  emit_move_insn (inf_const, GEN_INT ((HOST_WIDE_INT) inf_shift));
  emit_insn (gen_eqdi3 (eq_reg, op1_shift, inf_const));
  emit_move_insn (op0, gen_lowpart (SImode, eq_reg));
  DONE;
})

(define_expand "isinf<mode>2"
  [(use (match_operand:SI 0 "gpc_reg_operand" ""))
   (use (match_operand:FLOAT128 1 "gpc_reg_operand" ""))]
  "TARGET_POWERPC64 && TARGET_DIRECT_MOVE && can_create_pseudo_p ()"
{
  rtx op0 = operands[0];
  rtx op1 = operands[1];

  if (FLOAT128_IBM_P (<MODE>mode))
    {
      rtx df_tmp = gen_reg_rtx (DFmode);
      emit_insn (gen_trunc<mode>df2 (df_tmp, op1));
      emit_insn (gen_isinfdf2 (op0, df_tmp));
      DONE;
    }
  else
    {
      rtx op1_gpr = gen_reg_rtx (TImode);
      rtx op1_hi = gen_reg_rtx (DImode);
      rtx op1_lo = gen_reg_rtx (DImode);
      rtx op1_shift = gen_reg_rtx (DImode);
      rtx inf_const = gen_reg_rtx (DImode);
      rtx xor_hi = gen_reg_rtx (DImode);
      rtx or_lo_hi = gen_reg_rtx (DImode);
      rtx eq_reg = gen_reg_rtx (DImode);
      /* Infinity is an exponent of all 1's and all data bits 0.  The sign bit
         may be 0 or 1.  Shift left to eliminate the sign bit.  */
      unsigned HOST_WIDE_INT inf_shift = (0x7fff000000000000UL << 1);

      emit_move_insn (op1_gpr, gen_lowpart (TImode, op1));

      if (WORDS_BIG_ENDIAN)
	{
	  emit_move_insn (op1_hi, gen_highpart (DImode, op1_gpr));
	  emit_move_insn (op1_lo, gen_lowpart (DImode, op1_gpr));
	}
      else
	{
	  emit_move_insn (op1_hi, gen_lowpart (DImode, op1_gpr));
	  emit_move_insn (op1_lo, gen_highpart (DImode, op1_gpr));
	}

      emit_move_insn (inf_const, GEN_INT ((HOST_WIDE_INT) inf_shift));
      emit_insn (gen_ashldi3 (op1_shift, op1_hi, GEN_INT (1)));
      emit_insn (gen_xordi3 (xor_hi, op1_shift, inf_const));
      emit_insn (gen_iordi3 (or_lo_hi, xor_hi, op1_lo));
      emit_insn (gen_eqdi3 (eq_reg, or_lo_hi, const0_rtx));
      emit_move_insn (op0, gen_lowpart (SImode, eq_reg));
      DONE;
    }
})
