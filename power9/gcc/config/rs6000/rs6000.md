...
(define_insn_and_split "*round32<mode>2_fprs"
  ...
  "TARGET_HARD_FLOAT && TARGET_FPRS && TARGET_DOUBLE_FLOAT
   ...
   && (!TARGET_UPPER_REGS_<MODE> || !TARGET_P8_VECTOR) && can_create_pseudo_p ()"
  "#"
  ""
  [(pc)]
{
...
(define_insn_and_split "*round32<mode>2_p8"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=<Fv>")
	(float:SFDF
	 (fix:SI (match_operand:SFDF 1 "gpc_reg_operand" "<Fv>"))))
  ...
  [(pc)]
{
  ...
  emit_insn (gen_round32_xscv<Fvsx>sxws (tmp1, src));
  if (!TARGET_P9_VECTOR)
    {
      emit_insn (gen_altivec_vupkhsw (tmp2, tmp1));
      emit_insn (gen_round32_xxpermdi (tmp2, tmp2));
    }
  else
    emit_insn (gen_vsx_sign_extend_si_v2di (tmp2, tmp1));

  emit_insn (gen_round32_xscvsxddp_<mode> (dest, tmp2));
  DONE;
}
  [(set_attr "type" "fp")
   (set_attr "length" "16")])
...
;; Altivec registers.
(define_insn "round32_xscvsxddp_<mode>"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=<Fv>")
...
;; Optimization of conversion of SF/DFmode to unsigned SImode and back to
;; SF/DFmode a using vupkhuw (ISA 2.07)
(define_insn_and_split "*roundu32<mode>2_p8"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=<Fv>")
	(float:SFDF
	 (fix:SI (match_operand:SFDF 1 "gpc_reg_operand" "<Fv>"))))
   (clobber (match_scratch:V4SI 2 "=v"))
  ...
  [(pc)]
{
  ...
  emit_insn (gen_round32_xscvdpuxws_<mode> (tmp1, src));
  emit_insn (gen_altivec_vupkhsw (tmp2, tmp1));
  emit_insn (gen_round32_xxpermdi (tmp2, tmp2));
  emit_insn (gen_round32_xscvuxd<Fvsx> (dest, tmp2));
  DONE;
}
  [(set_attr "type" "fp")
   (set_attr "length" "16")])

;; Convert SF/DFmode to unsigned SImode.  Use V4SImode to access the Altivec
;; regsiters.
(define_insn "round32_xscvdpuxws_<mode>"
  [(set (match_operand:V4SI 0 "gpc_reg_operand" "=v")
	(unspec:V4SI [(match_operand:SFDF 1 "gpc_reg_operand" "<Fv>")
		      (const_int 3)]
		   UNSPEC_ROUND_EXTEND))]
...
;; Convert unsigned 64-bit integer to SF/DFmode.  V2DI is used until DImode can
;; go in Altivec registers.
(define_insn "round32_xscvuxd<Fvsx>"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=<Fv>")
	(unspec:SFDF [(match_operand:V2DI 1 "gpc_reg_operand" "wa")
		      (const_int 4)]
		     UNSPEC_ROUND_EXTEND))]
