...
;; Opitmize converting SF/DFmode to signed SImode and back to SF/DFmode.  On
;; systems before ISA 2.07 (power8) there was no simple way to sign extend the
;; 32-bit value that FCTIWZ returned into 64-bit.  This optimization prevents
;; having to store the value from the FPR/vector unit to the stack, load the
;; value into a GPR, sign extend it, store it back on the stack from the GPR,
;; load it back into the FP/vector unit to do the rounding.

;; If we have direct move, disable the optimization.  Normally for conversion
;; to signed SImode, we can do the conversion in 3-4 instructions.  Even if we
;; disable using the upper registers, it is faster to do a direct move to the
;; GPR unit than a store and load.
...
;; Optimization of conversion of SF/DFmode to signed SImode and back to
;; SF/DFmode a using vupkhsw (ISA 2.07) or vextsd2w (ISA 3.0)
;;
;; On ISA 2.07 it generates:
;; XSCVDPSXWS	 -- Convert DP value, leave 32-bit int in bits 32-63
;; VUPKHSW	 -- Sign extend value from bits 32-63 into bits 64-127
;; XXPERMDI	 -- Move value from bits 64-127 to bits 0..63
;; XSCVSXD{D,S}P -- Convert back to DP format
;;
;; On ISA 3.0 it generates:
;; XSCVDPSXWS	 -- Convert DP value, leave 32-bit int in bits 32-63
;; VEXTSW2D	 -- Sign extend value from bits 32-63 into bits 0-63
;; XSCVSXD{D,S}P -- Convert back to DP format

(define_insn_and_split "*round32<mode>2_p8"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=<Fv>")
	(float:SFDF (fix:SI (match_operand:SFDF 1 "gpc_reg_operand" "<Fv>"))))
   (clobber (match_scratch:V4SI 2 "=v"))
   (clobber (match_scratch:V2DI 3 "=v"))]
  "TARGET_UPPER_REGS_<MODE> && TARGET_P8_VECTOR"
  "#"
  ""
  [(pc)]
{
  ...
  emit_insn (gen_round32_xscvdpsxws_<mode> (tmp1, src));
  if (!TARGET_P9_VECTOR)
    {
      emit_insn (gen_altivec_vupkhsw_direct (tmp2, tmp1));
      emit_insn (gen_round32_xxpermdi (tmp2, tmp2));
    }
  else
    emit_insn (gen_vsx_sign_extend_si_v2di (tmp2, tmp1));

  emit_insn (gen_round32_xscvsxd<Fvsx> (dest, tmp2));
  DONE;
}
  [(set_attr "type" "fp")
   (set_attr "length" "16")])

;; Convert SF/DFmode to signed SImode.  Use V4SImode to access the Altivec
;; regsiters.
(define_insn "round32_xscvdpsxws_<mode>"
  [(set (match_operand:V4SI 0 "gpc_reg_operand" "=v")
	(unspec:V4SI [(match_operand:SFDF 1 "gpc_reg_operand" "<Fv>")
		      (const_int 0)]
		   UNSPEC_ROUND_EXTEND))]
  "TARGET_UPPER_REGS_<MODE> && TARGET_P8_VECTOR"
  "xscvdpsxws %x0,%x1"
...
;; The vupkhsw/vupkhuw instruction leaves the extended 64-bit value in the
;; bottom 64-bit position.  This insn moves it to the top position.
(define_insn "round32_xxpermdi"
  [(set (match_operand:V2DI 0 "gpc_reg_operand" "=wa")
	(unspec:V2DI [(match_operand:V2DI 1 "gpc_reg_operand" "wa")
		      (const_int 1)]
		     UNSPEC_ROUND_EXTEND))]
  "TARGET_P8_VECTOR"
  "xxpermdi %x0,%x1,%x1,3"
...
;; Convert 64-bit integer to SF/DFmode.  V2DI is used until DImode can go in
;; Altivec registers.
(define_insn "round32_xscvsxd<Fvsx>"
  [(set (match_operand:SFDF 0 "gpc_reg_operand" "=<Fv>")
	(unspec:SFDF [(match_operand:V2DI 1 "gpc_reg_operand" "wa")
		      (const_int 2)]
		     UNSPEC_ROUND_EXTEND))]
  "TARGET_UPPER_REGS_<MODE> && TARGET_P8_VECTOR"
  "xscvsxddp %x0,%x1"
