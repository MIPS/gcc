...
let emit_prologue chan test_name =
  ...
  Printf.fprintf chan "/* { dg-do compile } */\n";
  Printf.fprintf chan "/* { dg-require-effective-target arm32 } */\n";
  Printf.fprintf chan
...
let emit_epilogue chan features regexps =
  let no_op = List.exists (fun feature -> feature = No_op) features in
    ...
    (if not no_op then
       ...
       ()
(*
       (* If the intrinsic is a no-op, and thus yields no instructions,
          make sure no Neon instructions appear in the assembly file.  *)
       Printf.fprintf chan "/* { dg-final { scan-assembler-not \"%s\"} } */\n"
                      "^\\[ \t\\]+\\[vV\\].*" *)
    );
    Printf.fprintf chan "\n"
