...
;; The register allocator can sometimes load up constants to use as an index
;; register in GPRs since the memory address mode only allows indirect or
;; indexed addresses.  Other optimizations that break apart memory addresses
;; (such as vector initialization) may be able to incorporate these constants
;; directly.  Provide a peephole2 to cover:
;;	LI x,<constant>
;;	ADD w,y,x
;;
;; where x is dead after the add.  Note, operand3 must be a base register
;; and not R0, since ADDI doesn't support adding R0.

(define_peephole2
  [(set (match_operand:P 0 "int_reg_operand" "")
	(match_operand:P 1 "short_cint_operand" ""))
   (set (match_operand:P 2 "int_reg_operand" "")
	(plus:P (match_operand:P 3 "base_reg_operand" "")
		(match_dup 0)))]
  "peep2_reg_dead_p (2, operands[0])"
  [(set (match_dup 2)
	(plus:P (match_dup 3)
