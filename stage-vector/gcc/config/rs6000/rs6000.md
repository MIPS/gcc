...
;; Add peephole2's to use the ISA 3.0 generate byte/half word instructions
(define_peephole2
  [(match_scratch:DI 3 "b")
   (set (match_operand:SI 0 "int_reg_operand" "")
	(zero_extend:SI (match_operand:QHI 1 "memory_operand" "")))
   (set (match_operand:DI 2 "vsx_register_operand" "")
	(zero_extend:DI (match_dup 0)))]
  "TARGET_P9_VECTOR && TARGET_POWERPC64 && peep2_reg_dead_p (2, operands[0])
   && !update_address_mem (operands[1], <MODE>mode)"
  [(const_int 0)]
{
  rtx mem = operands[1];
  rtx addr = XEXP (mem, 0);

  if (!indexed_or_indirect_address (addr, <MODE>mode))
    {
      rtx tmp = operands[3];
  ...
  emit_insn (gen_p9_lxsi<wd>zx (operands[2], mem));
  DONE;
})
