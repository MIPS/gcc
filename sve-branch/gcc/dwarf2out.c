			 ...
			 machine_mode mem_mode)
{
  ...
  if (dwarf_strict && GET_MODE_SIZE (int_op_mode) > DWARF2_ADDR_SIZE)
    return NULL;
		    ...
		    enum var_init_status initialized)
{
  ...
  switch (GET_CODE (rtl))
      ...
      if (mem_loc_result != NULL)
	{
	  if (!is_a <scalar_int_mode> (mode, &int_mode)
		...
		= base_type_for_mode (mode, SCALAR_INT_MODE_P (mode));
	      if (type_die == NULL)
		return NULL;
	      deref = new_loc_descr (DW_OP_GNU_deref_type, size, 0);
    ...
    case UDIV:
      if (!dwarf_strict && is_a <scalar_int_mode> (mode, &int_mode))
	{
	  if (GET_MODE_SIZE (int_mode) > DWARF2_ADDR_SIZE)
	    {
	      op = DW_OP_div;
