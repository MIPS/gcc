			  ...
			  rtx op, machine_mode op_mode)
{
  ...
  tem = simplify_unary_operation_1 (code, mode, op);
  if (tem)
    return tem;
...
/* Perform autogenerated simplifications on a binary operation.
   This is shared by simplify_binary_operation and
   simplify_relational_operation; the arguments are the same as for
   the former.  */

static rtx
simplify_binary_autogen (enum rtx_code code, machine_mode mode,
			 rtx op0, rtx op1)
{
  rtx tem = (rtx) alloca (sizeof (rtx_def) + sizeof (rtunion));
  PUT_CODE (tem, code);
  PUT_MODE_RAW (tem, mode);
  XEXP (tem, 0) = op0;
  XEXP (tem, 1) = op1;
  return simplify_rtx_autogen (tem);
}
			   ...
			   rtx op0, rtx op1)
{
  ...
  return simplify_binary_autogen (code, mode, op0, op1);
}
			       ...
			       machine_mode cmp_mode, rtx op0, rtx op1)
{
  ...
  if (GET_MODE_CLASS (cmp_mode) != MODE_CC && !CC0_P (op0))
    {
      trueop0 = avoid_constant_pool_reference (op0);
      trueop1 = avoid_constant_pool_reference (op1);
      tem = simplify_relational_operation_1 (code, mode, cmp_mode,
					     trueop0, trueop1);
      if (tem)
	return tem;
    }
			    ...
			    rtx op2)
{
  ...
  tem = (rtx) alloca (sizeof (rtx_def) + sizeof (rtunion) * 2);
  PUT_CODE (tem, code);
  PUT_MODE_RAW (tem, mode);
  XEXP (tem, 0) = op0;
  XEXP (tem, 1) = op1;
		 ...
		 machine_mode innermode, poly_int64 byte)
{
  ...
  rtx tem = (rtx) alloca (sizeof (rtx_def) + sizeof (rtunion));
  PUT_CODE (tem, SUBREG);
  PUT_MODE_RAW (tem, outermode);
  SUBREG_REG (tem) = op;
  SUBREG_BYTE (tem) = byte;
  return simplify_rtx_autogen (tem);
}
