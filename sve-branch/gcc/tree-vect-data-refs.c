 ...
 /* For interleaved data accesses the step in the loop must be multiplied by
  ...
  if (known_alignment_for_access_p (dr)
      && known_alignment_for_access_p (dr_peel))
    {
      ...
      misal &= (vect_data_ref_required_alignment (dr) / BITS_PER_UNIT) - 1;
      ...
      if (do_peeling)
        {
          if (known_alignment_for_access_p (dr))
            {
	      unsigned int npeel_tmp, vecalign, dr_size;
	      ...
	      npeel_tmp = (mis & (vecalign - 1)) / dr_size;
  ...
  if (do_peeling)
    {
	  ...
	  bool negative = tree_int_cst_compare (DR_STEP (dr0),
	      ...
	      unsigned int vecalign
		= vect_data_ref_required_alignment (dr0) / BITS_PER_UNIT;
	      npeel = (mis & (vecalign - 1)) / vect_get_dr_size (dr0);
            }
      ...
      if (do_peeling)
        {
		  ...
		  unsigned int vecalign
		    = vect_data_ref_required_alignment (dr0) / BITS_PER_UNIT;
		  max_peel = (vecalign / vect_get_dr_size (dr0)) - 1;
                }
			      ...
			      loop_vec_info loop_vinfo)
{
  ...
  stmt_vec_info stmtinfo_a = vinfo_for_stmt (DR_STMT (dra));
  stmt_vec_info stmtinfo_b = vinfo_for_stmt (DR_STMT (drb));
  int dra_size = vect_get_dr_size (dra);
  int drb_size = vect_get_dr_size (drb);
  lambda_vector dist_v;
  unsigned int loop_depth;
...
vect_prune_runtime_alias_test_list (loop_vec_info loop_vinfo)
{
  ...
  FOR_EACH_VEC_ELT (may_alias_ddrs, i, ddr)
	...
	{
	  bool no_alias_p = vect_no_alias_p (dr_a, dr_b, segment_length_a,
					     segment_length_b, access_size_a,
					     access_size_b);
	  if (dump_enabled_p ())
	    {
	      dump_printf_loc (MSG_NOTE, vect_location,
...
vect_duplicate_ssa_name_ptr_info (tree name, data_reference *dr)
{
  ...
  if (misalign == -1)
    ...
    {
      unsigned int align = (vect_data_ref_required_alignment (dr)
			    / BITS_PER_UNIT);
					...
					vect_addr_base_info *addr_info)
{
  ...
  tree dest;
  gimple_seq seq = NULL;
  tree base_offset = addr_info->dr_offset;
  tree vect_ptr_type;
...
static void
maybe_init_vect_data_ref_cache (loop_vec_info loop_vinfo)
{
  if (!LOOP_VINFO_ADDR_CACHE (loop_vinfo))
    LOOP_VINFO_ADDR_CACHE (loop_vinfo)
      = new hash_table<vect_addr_base_hasher> (37);
}
...
static bool
fill_vect_addr_base_info (gimple *stmt, tree offset, struct loop *loop,
  ...
  if (loop_vinfo && loop && loop != (gimple_bb (stmt))->loop_father)
    {
      ...
      info->dr_base = unshare_expr (STMT_VINFO_DR_BASE_ADDRESS (stmt_info));
      info->dr_init = unshare_expr (STMT_VINFO_DR_OFFSET (stmt_info));
      info->dr_offset = unshare_expr (STMT_VINFO_DR_INIT (stmt_info));
    }
  else
    {
      struct data_reference *dr = STMT_VINFO_DATA_REF (stmt_info);
      info->dr_base = unshare_expr (DR_BASE_ADDRESS (dr));
      info->dr_init = unshare_expr (DR_INIT (dr));
      info->dr_offset = unshare_expr (DR_OFFSET (dr));
    }

  if (!loop_vinfo)
    {
      info->dr_offset = ssize_int (0);
      info->dr_init = ssize_int (0);
    }
				      ...
				      tree byte_offset)
{
  ...
  if (!fill_vect_addr_base_info (stmt, offset, loop, byte_offset, &info))
    return vect_create_addr_base_for_vector_ref_1 (stmt, new_stmt_list, &info);

  stmt_vec_info stmt_info = vinfo_for_stmt (stmt);
  loop_vec_info loop_vinfo = STMT_VINFO_LOOP_VINFO (stmt_info);
  maybe_init_vect_data_ref_cache (loop_vinfo);
