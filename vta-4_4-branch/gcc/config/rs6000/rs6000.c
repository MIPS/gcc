...
static int rs6000_legitimate_address (enum machine_mode, rtx, int);
static int rs6000_debug_legitimate_address (enum machine_mode, rtx, int);
int (*rs6000_legitimate_address_ptr) (enum machine_mode, rtx, int)
...
rs6000_override_options (const char *default_cpu)
{
  ...
  if (rs6000_debug_name)
	...
	{
	  rs6000_legitimate_address_ptr = rs6000_debug_legitimate_address;
...
/* Debug version of rs6000_legitimate_address.  */
static int
rs6000_debug_legitimate_address (enum machine_mode mode, rtx x,
				 int reg_ok_strict)
{
  int ret = rs6000_legitimate_address (mode, x, reg_ok_strict);
  fprintf (stderr,
	   "\nrs6000_legitimate_address: return = %s, mode = %s, "
	   "strict = %d, code = %s\n",
	   ret ? "true" : "false",
	   GET_MODE_NAME (mode),
	   reg_ok_strict,
...
rs6000_emit_int_cmove (rtx dest, rtx op, rtx true_cond, rtx false_cond)
{
  ...
  enum machine_mode mode = GET_MODE (rs6000_compare_op0);
